...
- [x] Store explicit backlink (reverse) edges alongside forward edges for O(1) "what links here" queries
- [x] Add rule: flag dead code and unreferenced callable symbols with 0 inbound calls/usages in `graphify lint`
- [x] Deduplicate nodes and parallel edges in `extract()` before returning AST extraction bundles
...
- [x] Optimize `graphify watch` / daemon mode:
...
- [x] Parse `createSlice(...)` definitions in TypeScript/JavaScript (`.ts`, `.tsx`, `.js`, `.jsx`):
...
- [x] Parse `createApi(...)` declarations:
...
- [x] Cross-Layer Frontend-to-Backend Resolution:
...
- [x] State & Cache Linting Rules (`graphify lint`):
...
## Phase 16 — Local NLP Doc-to-Code Semantic Cross-Referencing (.md & .txt)

**Zero-LLM Local Doc-to-Code Entity Linking**
- [x] Multi-Format Document Ingestion (.md, .txt, .rst, .mdx, .qmd, .skill):
  - Route plain text (`.txt`) and documentation files through `extract_markdown` without external LLM dependencies
  - Extract document-level nodes, structural sections, and inline symbol mentions
- [x] Cross-File Symbol & Path Resolution (`graphify/markdown_resolution.py`):
  - Extend `MARKDOWN_MENTION_SUFFIXES` to include `.txt`, `.rst`, `.markdown` alongside `.md`
  - Resolve path-qualified mentions (`path/to/file.py::Symbol`) with exact matching (`EXTRACTED`, confidence: 1.0)
  - Resolve bare and dotted symbol citations (`Symbol`, `Class.method()`) against unique callable code AST entities (`INFERRED`, confidence: 0.95)
  - Filter language built-in globals to prevent false positive cross-references
- [x] Unit & Regression Testing:
  - Add test coverage in `tests/test_markdown_code_spans.py` verifying `.txt` files resolve symbol citations into `references` edges

---

## Phase 17 — Claude Code Deep Integration & Context Window Token Optimization

- [x] Claude Code Native MCP Integration:
  - Document stdio server configuration (`graphify --mcp`) for Claude Code / Claude Desktop
  - Wire AST symbol navigation (`find_definitions`, `get_callers`, `get_callees`, `find_references`) to avoid reading whole files
  - Expose blast radius & impact tools (`get_impact_analysis`, `get_blast_radius`, `get_execution_flow`) for automated pre-edit safety checks
- [x] Large Codebase Token Economy & Hierarchical Retrieval:
  - Add agent-crawlable community wiki navigation (`graphify . --wiki`) to reduce context ingestion by ~85%
  - Add FTS5/BM25 targeted query mode (`graphify query "<concept>" --terse`)
- [x] Repository Agent Configuration:
  - Create CLAUDE.md with prompt rules: query graphify first, check blast radius before modifying, use graph-first code reuse, keep graph updated with `graphify update .`

---

## Phase 18 — Local PDF Ingestion, Markdown Conversion & AST Extraction

**Zero-LLM Local PDF-to-Markdown & Code Graph Integration**
- [x] Local PDF-to-Markdown Converter (`graphify/detect.py`):
  - Convert `.pdf` documents to structured Markdown text locally using pure Python / `pypdf`
  - Auto-structure document hierarchy: parse `/Title` and `/Author` into YAML frontmatter, detect section/page headings, and format lists
  - Enforce decompression and raw size bounds (`_OFFICE_MAX_RAW_BYTES`) to prevent memory exhaustion
- [x] Multi-Format AST Graph Integration (`graphify/extract.py` & `graphify/extractors/markdown.py`):
  - Register `.pdf` in `_DISPATCH` routing through `extract_pdf`
  - Parse converted markdown into structural AST nodes (`page`, `heading`) and document hierarchy edges (`contains`)
  - Cross-reference code mentions in PDF prose (`Widget`, `func()`) to source code AST symbols via `markdown_resolution.py`
- [x] Unit & Acceptance Testing:
  - Add test coverage in `tests/test_pdf_extraction.py` validating PDF conversion, markdown AST generation, and code mention resolution

---

## Phase 19 — High-Density Code Repo RAG & AST Skeletonization

**Tree-sitter Code Skeletons & Elision (80–90% Token Reduction)**
- [x] Implement `graph_fy/skeleton.py`: Tree-sitter code elision engine:
  - Extract classes, method signatures, parameter types, return type annotations, and docstrings.
  - Elide function bodies to `...` across supported languages (Python, TypeScript/JavaScript, Go, Rust, Java, C/C++).
  - Support token-budgeted output to pack entire module interfaces into compact context windows.

**Personalized PageRank (PPR) Query-Biased Subgraph Reranking**
- [x] Implement query-seeded Personalized PageRank in `graph_fy/retrieval.py`:
  - Run random walk with restart (`compute_pagerank(subG, personalization=seed_weights)`) seeded by FTS5 BM25 matches without external scipy dependency.
  - Prioritize structural ego-networks of the query topic while suppressing unrelated global utility hubs.

**Compact Stub Formatting (`.pyi` / `.d.ts` / Repo-Map output)**
- [x] Add `--skeleton` flag to `graph_fy query`:
  - Emit ranked code skeletons instead of verbose JSON metadata arrays.
  - Group declarations by file path with syntax headers.

**1-Hop Type & Interface Expansion**
- [x] Automated type dependency expansion in retrieval & MCP definitions:
  - Follow AST `references`, `inherits`, and `implements` edges for type annotations to bundle dependent class/type definitions into the retrieved context.

**MCP Tooling for Surgical Code Navigation**
- [x] Expose `get_code_skeleton` and `get_symbol_implementation` tools in MCP server (`graph_fy/mcp.py` & `graph_fy/serve.py`):
  - Allow AI coding agents to inspect compact interface skeletons first, then pull exact symbol body slices on demand.
