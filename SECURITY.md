...
Security fixes are applied directly to the active `main` branch.

## Reporting a Vulnerability

Report security vulnerabilities via GitHub private vulnerability reporting. Please include:
...
`graph_fy` is a **local development tool**. It runs as an assistant skill and local MCP stdio server. It makes zero network calls during graph analysis.
...
| Path traversal in MCP server | `security.validate_graph_path()` resolves paths and requires them to be inside `graph_fy_out/`. Also requires the `graph_fy_out/` directory to exist. |
