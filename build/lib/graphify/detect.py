...
def classify_file(path: Path) -> FileType | None:
    ...
    from graphify.extractors.docker import is_docker_compose
    ...
    from graphify.extractors.github_actions import is_github_workflow
    if is_github_workflow(path):
        return FileType.CODE
    # OpenAPI, AsyncAPI, and JSON Schema
    from graphify.extractors.api_schema import is_api_or_schema
    if is_api_or_schema(path):
        return FileType.CODE
    # Argo CD GitOps manifests
    from graphify.extractors.argocd import is_argocd_manifest
    if is_argocd_manifest(path):
        return FileType.CODE
    # Bazel and Monorepo configuration
    from graphify.extractors.bazel import is_bazel_file
    from graphify.extractors.monorepo import is_monorepo_file
...
def extract_pdf_text(path: Path) -> str:
    """PDF parsing is removed in fully-local no-LLM build."""
    return ""
