"""graph_fy CLI - `graph_fy install` sets up the Claude Code skill."""
...
# Output directory — override with graph_fy_OUT env var for worktrees or shared-output setups.
# Accepts a relative name ("graph_fy_out-feature") or an absolute path ("/shared/graph_fy_out").
# Defined once in graph_fy.paths so the security/callflow path guards honour the
# same override (#1423).
from graph_fy.paths import graph_fy_OUT as _graph_fy_OUT

# Install/uninstall subsystem moved to graph_fy/install.py; re-exported here so
# `from graph_fy.__main__ import <name>` keeps working unchanged.
from graph_fy.install import (  # noqa: E402,F401
    ...
    _resolve_graph_fy_exe,
    _skill_registration,
    _strip_graph_fy_hook,
    _strip_graph_fy_md_section,
...
from graph_fy.cli import (  # noqa: E402,F401
    dispatch_command,
    _StageTimer,
    _clone_repo,
    _default_graph_path,
...
def __getattr__(name: str) -> str:
    ...
    # brick the CLI on `import graph_fy.__main__` (#1121 follow-up).
...
def _check_skill_version(skill_dst: Path, platform_names: "list[str] | None" = None) -> None:
    """Warn if the installed skill is from an older graph_fy version.
    ...
    a plain `graph_fy install` only refreshes the detected platform, and a
    ...
    try:
        ...
        return
    if not skill_exists:
        print("  warning: skill dir exists but SKILL.md is missing. Run 'graph_fy install' to repair.", file=sys.stderr)
        return
    ...
    if "references/" in body and not (skill_dst.parent / "references").exists():
        print("  warning: skill references/ sidecar is missing. Run 'graph_fy install' to repair.", file=sys.stderr)
    try:
    ...
    if installed != __version__:
        if _version_tuple(installed) > _version_tuple(__version__):
            # The skill on disk is NEWER than the running package. `graph_fy install`
            ...
            print(
                f"  warning: skill at {skill_dst.parent} is from graph_fy {installed}, but the package is "
                f"{__version__} (older). Upgrade the package "
                f"(e.g. 'uv tool upgrade graph_fy' or 'pip install -U graph_fy'); "
                f"running 'graph_fy install' would downgrade the skill.",
                file=sys.stderr,
            )
        else:
            _cmd = (
                f"graph_fy install --platform {platform_names[0]}"
                if platform_names else "graph_fy install"
            )
            print(
                f"  warning: skill at {skill_dst.parent} is from graph_fy {installed}, "
                f"package is {__version__}. Run '{_cmd}' to update it "
                f"(a plain 'graph_fy install' refreshes only the detected platform).",
                file=sys.stderr,
            )
...
# `graph_fy hook-guard` subcommand (see _run_hook_guard). The previous hooks
...
# silently vanished — users had to invoke /graph_fy by hand (#522). Moving the
...
# The always-on instruction blocks are packaged markdown under graph_fy/always_on/,
...
# when a graph exists. Emitted by `graph_fy hook-guard gemini`. The old hook was a
...
_KIRO_STEERING_MARKER = "graph_fy: A knowledge graph of this project"
        ...
        "PreToolUse": [
            {
                ...
                "hooks": [
                    {
                        "type": "command",
                        # Use the graph_fy CLI itself so the hook is shell-agnostic:
...
def main() -> None:
    ...
    stdout early, graph_fy treats it as success instead of crashing with an
...
def _run_cli() -> None:
    ...
    if len(sys.argv) >= 2 and sys.argv[1] in ("-v", "--version", "version"):
        print(f"graph_fy {__version__}")
        return

    if len(sys.argv) < 2 or sys.argv[1] in ("-h", "--help", "-?"):
        print("Usage: graph_fy <command>")
        print("Commands:")
        print("  install [--platform P]  copy skill to platform config dir (claude|copilot)")
        print("  uninstall               remove graph_fy from all detected platforms in one shot")
        print("    --purge                 also delete graph_fy_out/ directory")
        print("  path \"A\" \"B\"            shortest path between two nodes in graph.json")
        print("    --graph <path>          path to graph.json (default graph_fy_out/graph.json)")
        print("  flow <entry_point>      trace execution flow from entry point to terminal sinks")
        print("    --sink <sink>           find specific execution path(s) to sink")
        ...
        print("                            (default graph_fy_out/graph.json)")
        print("    --json                  emit machine-readable JSON")
        ...
        print("  clone <github-url>      clone a GitHub repo locally and print its path for /graph_fy")
        ...
        print("    --out <path>            output path (default: graph_fy_out/merged-graph.json)")
        print("    --branch <branch>       checkout a specific branch (default: repo default)")
        print("    --out <dir>             clone to a custom directory (default: ~/.graph_fy/repos/<owner>/<repo>)")
        ...
        print("                            (also: graph_fy_FORCE=1 env var; use after refactors that delete code)")
        print("    --no-cluster            skip clustering, write raw extraction only")
        print("  cluster-only <path>     rerun clustering on an existing graph.json and regenerate report")
        print("    --no-viz                skip graph.html generation (useful for >5000 node graphs / CI)")
        print("    --graph <path>          path to graph.json (default <path>/graph_fy_out/graph.json)")
        ...
        print("    --graph <path>          path to graph.json (default graph_fy_out/graph.json)")
        print("  impact \"X\"               predictive impact analysis and blast radius for target X")
        print("    --depth N               downstream traversal depth (default 10)")
        print("    --json                  emit JSON output")
        print("    --terse                 emit compact 1-line blast radius summary")
        print("    --graph <path>          path to graph.json (default graph_fy_out/graph.json)")
        ...
        print("  save-result             save a Q&A result to graph_fy_out/memory/ for graph feedback loop")
        ...
        print("    --memory-dir DIR        memory directory (default: graph_fy_out/memory)")
        print("  reflect                 aggregate graph_fy_out/memory/ outcomes into a deterministic lessons doc")
        print("    --memory-dir DIR        memory directory (default: graph_fy_out/memory)")
        print("    --out FILE              output path (default: graph_fy_out/reflections/LESSONS.md)")
        print("    --graph PATH            graph.json, for community grouping + dropping stale nodes (optional)")
        print("    --analysis PATH         .graph_fy_analysis.json (optional, auto-detected next to --graph)")
        print("    --labels PATH           .graph_fy_labels.json (optional, auto-detected next to --graph)")
        ...
        print("                            manifest gate and semantic cache reads (env: graph_fy_FORCE=1)")
        ...
        print("    --out DIR, --output DIR output dir (default: <path>); writes <DIR>/graph_fy_out/")
        print("    --google-workspace      export .gdoc/.gsheet/.gslides shortcuts via gws before extraction")
        print("    --no-gitignore         ignore .gitignore and .git/info/exclude (prioritizes .graph_fyignore)")
        print("    --no-cluster            skip clustering, write raw extraction only")
        ...
        print("  global add <graph.json>  add/update a project graph in the global graph (~/.graph_fy/global-graph.json)")
        ...
        print("  claude install          write graph_fy section to CLAUDE.md + PreToolUse hook (Claude Code)")
        print("  claude uninstall        remove graph_fy section from CLAUDE.md + PreToolUse hook")
        print(
            "  copilot install         copy graph_fy skill to ~/.copilot/skills (GitHub Copilot CLI)"
        )
        print("  copilot uninstall       remove graph_fy skill from ~/.copilot/skills")
        print(
