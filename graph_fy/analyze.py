...
from graph_fy.detect import CODE_EXTENSIONS, DOC_EXTENSIONS, PAPER_EXTENSIONS, IMAGE_EXTENSIONS
...
def _file_category(path: str) -> str:
    ...
    if ext in CODE_EXTENSIONS:
        return "code"
    if ext in PAPER_EXTENSIONS or ext == ".pdf":
        return "paper"
    if ext in IMAGE_EXTENSIONS or ext in _IMAGE_EXTS:
        return "image"
    return "doc"
...
def _cross_community_surprises(
    ...
    if not communities:
        ...
        if G.number_of_nodes() > 500:
            k = min(50, max(20, int(G.number_of_nodes() * 0.01)))
            betweenness = nx.edge_betweenness_centrality(G, k=k, seed=42)
        else:
...
def suggest_questions(
    ...
    if G.number_of_edges() > 0:
        ...
        k = min(100, max(20, int(n_nodes * 0.02))) if n_nodes > 1000 else None
