...
# Hyperedge member lists are canonically keyed `nodes` (see graph_fy/llm.py
...
def _fold_node_aliases(node: dict) -> None:
    ...
    label-keyed merge/dedup — a permanent ghost that ``graph_fy update``
...
def _infer_merge_root(graph_path: Path) -> str | None:
    ...
    Prefers the committed ``graph_fy_out/.graph_fy_root`` marker — the authoritative
    scan root graph_fy records at build/watch time (#686/#1423) — then falls back to
    the directory that contains the output dir (``graph.json``'s grandparent, i.e.
    ``<root>/graph_fy_out/graph.json`` -> ``<root>``). The grandparent heuristic is
    applied only when ``graph.json``'s own directory actually looks like a graph_fy
    ...
    try:
        ...
        pass
    from .paths import graph_fy_OUT
    try:
        if (
            parent.name == Path(graph_fy_OUT).name
            or (parent / ".graph_fy_root").exists()
...
def _derive_prune_root(prune_sources: "list[str]", stored_sfs: "set[str]") -> "str | None":
    ...
    layouts (``graph.json`` not under ``<root>/graph_fy_out/``), so every prune
...
def _semantic_id_remap(nodes: list, root: str | None) -> dict:
    ...
    from graph_fy.extractors.base import _file_stem  # local: avoid import cost at module load
...
def graph_has_legacy_ids(nodes: list, root: str | Path | None = None, sample: int = 300) -> bool:
    ...
    from graph_fy.extractors.base import _file_stem
...
def _doc_twin_remap(nodes: list) -> dict[str, str]:
    ...
    the same document. A ``graph_fy update`` after a semantic build leaves both,
...
def build_from_json(extraction: dict, *, directed: bool = False, root: str | Path | None = None) -> nx.Graph:
    ...
    for node in extraction.get("nodes", []):
        ...
        if "source" in node and "source_file" not in node:
            ...
            print(
                f"[graph_fy] WARNING: node '{node_id}' uses field 'source' instead of "
        ...
        # graph_fy versions that didn't always populate file_type) don't
    ...
    try:
        ...
        resolve_cross_format_dependencies(extraction.get("nodes", []), extraction.get("edges", []))
    except Exception:
        pass

    # Apply labeling improvements (Phase 8: slugs, categories, synonym mappings, typed edge aliases)
    try:
        from graph_fy.labeling import apply_labeling_improvements
        apply_labeling_improvements(extraction.get("nodes", []), extraction.get("edges", []), root=_root)
    except Exception:
        pass
    ...
    if real_errors:
        ...
        print(
            f"[graph_fy] Extraction warning ({len(real_errors)} issues): {breakdown}",
            file=sys.stderr,
        )
    ...
    for node in extraction.get("nodes", []):
        ...
        if isinstance(node, dict):
            ...
            try:
                ...
                print(
                    f"[graph_fy] WARNING: skipping node with non-hashable id "
    ...
    for nid in sorted(node_set):
        ...
        if attrs.get("source_location") or is_ast:
            ...
            if is_ast:
                ...
                # saved graph_fy_out/memory/*.md query answer) mints a NEW
    ...
    # document that only *mentions* a file (a saved graph_fy_out/memory/*.md
    ...
    from graph_fy.extractors.base import _file_stem as _fs
    ...
    for edge in sorted(
        ...
        try:
            ...
            print(
                f"[graph_fy] WARNING: skipping edge with non-hashable endpoint "
        ...
        # `calls` and `uses` after everything. On graph_fy's own corpus that
    ...
    if hyperedges:
        ...
        for he in hyperedges:
            ...
            if isinstance(he, dict) and isinstance(he.get("nodes"), list):
                ...
                if not valid_members:
                    print(
                        f"[graph_fy] WARNING: dropping hyperedge "
        ...
        if kept_hyperedges:
            ...
            G.graph["hyperedges"] = []
            print(
                f"[graph_fy] WARNING: all {len(hyperedges)} hyperedge(s) were "
...
def build(
    ...
    *,
    ...
    """
    from graph_fy.dedup import deduplicate_entities
...
def deduplicate_by_label(nodes: list[dict], edges: list[dict]) -> tuple[list[dict], list[dict]]:
    ...
    print(f"[graph_fy] Deduplicated {len(remap)} duplicate node(s) by label.", file=sys.stderr)
...
def build_merge(
    ...
    *,
    ...
    if new_sources:
        ...
        if replaced_n:
            print(
                f"[graph_fy] Replaced {replaced_n} node(s) from re-extracted "
                f"source file(s).",
                file=sys.stderr,
            )
    ...
    if prune_sources:
        ...
        if n_nodes:
            print(
                f"[graph_fy] Pruned {n_nodes} node(s) from {n_files} deleted or "
                f"excluded source file(s).",
                file=sys.stderr,
            )
        if edges_to_remove:
            print(
                f"[graph_fy] Pruned {len(edges_to_remove)} edge(s) from deleted or "
                f"excluded source file(s).",
                file=sys.stderr,
            )
        ...
        if not n_nodes and not edges_to_remove:
            if (prune_set or prune_abs) and not _matched_prune_entries:
                ...
                print(
                    f"[graph_fy] WARNING: {len(prune_sources)} prune source(s) "
                ...
                print(
                    f"[graph_fy] {len(prune_sources)} source file(s) deleted or "
    ...
    if had_graph and not dedup:
        ...
        if unexplained:
            raise ValueError(
                f"graph_fy: build_merge would drop {len(unexplained)} node(s) "
...
def distinct_repo_tags(graph_paths: "list[Path]") -> "list[str]":
    ...
    The naive tag (the ``graph_fy_out`` parent dir name) is NOT unique across
    inputs: ``src/graph_fy_out`` and ``frontend/src/graph_fy_out`` both yield
    ...
    """
    repo_dirs = [p.parent.parent for p in graph_paths]  # graph_fy_out/.. → repo dir
