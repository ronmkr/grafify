...
# Output directory name — override with graph_fy_OUT env var for worktrees or
# shared-output setups. Accepts a relative name ("graph_fy_out-feature") or an
# absolute path ("/shared/graph_fy_out"). Single source of truth in graph_fy.paths
# (#1423); re-exported here as _graph_fy_OUT for the existing call sites.
...
try:
    ...
    try:
        _EXTRACTOR_VERSION = _pkg_version("graph_fy")
    except Exception:
        _EXTRACTOR_VERSION = _pkg_version("graph_fyy")
except Exception:
...
def _cleanup_stale_ast_entries(ast_base: Path, current_dir: Path) -> None:
    """Remove AST cache entries left behind by other graph_fy versions.
...
def prompt_fingerprint(prompt: "str | Path") -> str:
    ...
    its system prompt, :func:`graph_fy.llm._extraction_system`) or a Path to the
...
# `graph_fy extract --force` / `graph_fy update --force` (or graph_fy_FORCE=1)
...
# read anyway. Override with graph_fy_MTIME_GRANULARITY_MS (0 disables the
...
def _mtime_granularity_ns() -> int:
    ...
    """
    raw = os.environ.get("graph_fy_MTIME_GRANULARITY_MS", "").strip()
    if raw:
        try:
            ms = float(raw)
        except ValueError:
...
def _stat_sig_fresh(entry: object, st: "os.stat_result") -> bool:
    ...
    Entries written by an older graph_fy carry no ``indexed_at_ns``; they are
...
def _stat_key_to_relative(key: str, anchor: Path) -> str:
    ...
    Local duplicate of :func:`graph_fy.detect._to_relative_for_storage` —
...
def _stat_key_to_absolute(key: str, anchor: Path) -> str:
    ...
    written by an older graph_fy remains readable.
    """
    p = Path(key)
    if p.is_absolute():
        return str(p)
...
def _ensure_stat_index(root: Path, cache_root: "Path | None" = None) -> None:
    ...
    # graph_fy_out/ (#1747). _stat_index_anchor is the separate KEY anchor:
...
def _flush_stat_index() -> None:
    ...
    for k, v in _stat_index.items():
        ...
        on_disk[dk] = v
    # Never resurrect a corpus that was deleted while graph_fy was running
    # (#2974): a hook-launched `graph_fy update . &` in a short-lived worktree
    ...
    # written. Creating graph_fy_out/cache/ under a root that still exists is
...
def file_hash(path: Path, root: Path = Path("."), cache_root: "Path | None" = None) -> str:
    ...
    # graph_fy_out/cache/stat-index.json inside the analyzed source tree even when
...
def _relativize_source_files_in(payload: dict, root: Path) -> None:
    ...
    Mirror of :func:`graph_fy.watch._relativize_source_files` so cached
    ...
    :func:`graph_fy.detect._to_relative_for_storage`.
    """
    try:
...
def _normalize_source_file_value(src: "str | Path", root_resolved: Path) -> str:
    ...
    ``graph_fy update`` runs with a machine-specific ``source_file`` (#2197).
...
def _id_anchor(path_str: str, rel_str: str) -> str:
    ...
    """
    from graph_fy.ids import normalize_id  # ids imports only re/unicodedata: no cycle
...
def _portability_anchors(path: "str | Path", root: "str | Path") -> tuple[list[str], str, list[str], str]:
    ...
    """
    from graph_fy.ids import normalize_id
...
def _absolutize_ids_in(payload: dict, path: "str | Path", root: Path) -> None:
    ...
    stored form (``$graph_fy-root$_pkg_mod_base``) is a storage encoding, not a valid id,
...
def cache_dir(root: Path = Path("."), kind: str = "ast",
    ...
    AST entries live in graph_fy_out/cache/ast/v{version}-s{schema}/, namespaced
    by graph_fy version and cache-key schema because they depend on extractor
    code and key semantics, not just file contents. Semantic entries are still
    NOT version-namespaced (re-extraction
    costs LLM calls, #1252): they live in graph_fy_out/cache/semantic/, with
    deep-mode entries beside them in graph_fy_out/cache/semantic-deep/.
...
def load_cached(path: Path, root: Path = Path("."), kind: str = "ast",
    ...
    Cache value: stored as graph_fy_out/cache/{kind}/{hash}.json (AST entries
    ...
    AST entries written by other graph_fy versions — including the legacy
...
def save_cached(path: Path, result: dict, root: Path = Path("."), kind: str = "ast",
    ...
    Stores as graph_fy_out/cache/{kind}/{hash}.json where hash = SHA256 of current file contents.
...
def cached_files(root: Path = Path(".")) -> set[str]:
    ...
    base = Path(root).resolve() / _graph_fy_OUT / "cache"
    hashes: set[str] = set()
    # Legacy flat entries
    if base.is_dir():
...
def prune_semantic_cache(root: Path, live_hashes: set[str]) -> int:
    ...
    extraction-spec) can share one graph_fy_out/, and a wholesale sweep would
...
def check_semantic_cache(
    ...
    if legacy:
        warnings.warn(
            ...
            "vintages. Re-run with --force (or graph_fy_FORCE=1) to re-extract them "
    ...
    if corrupt:
        warnings.warn(
            ...
            "run with --force (or graph_fy_FORCE=1) to rewrite them, or clear the "
