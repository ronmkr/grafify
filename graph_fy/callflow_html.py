#!/usr/bin/env python3
"""
callflow_html.py — Generate call-flow architecture HTML from graph_fy knowledge graph outputs.

Reads graph.json plus optional GRAPH_REPORT.md, .graph_fy_labels.json, and sections JSON,
...
Usage:
  python3 -m graph_fy export callflow-html
  python3 -m graph_fy export callflow-html /path/to/project/graph_fy_out/graph.json
  python3 -m graph_fy export callflow-html --graph /path/to/graph.json --output docs/architecture.html
"""
...
h4 { font-size: 1.05rem; font-weight: 500; margin: 20px 0 8px; color: var(--warn); }
p { margin: 8px 0; color: var(--muted); }
.subtitle { color: var(--muted); font-size: 1.05rem; margin-bottom: 32px; font-family: "Roboto Mono", monospace; }
.mermaid {
...
.mermaid-toolbar button, .mermaid-toolbar .zoom-level {
  height: 28px;
  min-width: 30px;
  ...
  color: var(--text-sub);
  font: 500 0.82rem "Roboto Mono", monospace;
  display: inline-flex;
  align-items: center;
...
.mermaid-toolbar button:hover { background: #f1f3f4; color: var(--accent); }
.mermaid-toolbar .zoom-level { min-width: 52px; color: var(--muted); }
.call-table { width: 100%; border-collapse: collapse; margin: 16px 0; font-size: 0.9rem; background: var(--surface); border: 1px solid var(--border); border-radius: 8px; overflow: hidden; box-shadow: 0 1px 2px rgba(60,64,67,0.04); }
.call-table th { background: #f8f9fa; color: var(--muted); text-align: left; padding: 10px 14px; border-bottom: 2px solid var(--border); font-size: 0.8rem; font-weight: 600; text-transform: uppercase; letter-spacing: 0.04em; }
.call-table td { padding: 10px 14px; border-bottom: 1px solid #e0e0e0; vertical-align: top; font-family: "Roboto Mono", monospace; font-size: 0.86rem; color: var(--text); }
.call-table tr:hover td { background: #f8fafd; }
.tag {
...
.tag-async { background: #f3e8fd; color: #7c3aed; border-color: #e9d5ff; }
.tag-class { background: #e6f4ea; color: var(--ok); border-color: #ceead6; }
.tag-func { background: #e8f0fe; color: var(--accent); border-color: #d2e3fc; }
.tag-cmd { background: #fef7e0; color: var(--warn); border-color: #feefc3; }
.tag-endpoint { background: #fce8e6; color: var(--err); border-color: #fad2cf; }
.tag-hook { background: #fdf2f8; color: #db2777; border-color: #fbcfe8; }
.card {
  background: var(--card);
  border: 1px solid var(--border);
  border-radius: 8px;
...
.arrow-chain { font-family: "Roboto Mono", monospace; font-size: 0.85rem; color: var(--accent); padding: 10px; background: #e8f0fe; border: 1px solid #d2e3fc; border-radius: 6px; }
code { font-family: "Roboto Mono", monospace; background: #f1f3f4; padding: 2px 6px; border-radius: 4px; font-size: 0.88em; border: 1px solid #dadce0; color: #202124; }
...
.nav a { color: var(--muted); padding: 5px 12px; border-radius: 8px; transition: all 0.15s; font-weight: 500; }
.nav a:hover { color: var(--accent); background: #f1f3f4; text-decoration: none; }
...
def normalize_node(raw: dict, index: int) -> dict:
    """Normalize a graph_fy node across common graph.json schema variants."""
    node = dict(raw)
...
def normalize_edge(raw: dict, index: int) -> dict | None:
    """Normalize graph_fy edges while preserving original fields."""
    edge = dict(raw)
...
def _node_link_payload(data: dict) -> tuple[list, list] | None:
    """Read current graph_fy graph.json via NetworkX's node-link parser."""
...
def load_labels(path: str | Path | None) -> dict:
    """Load community labels from .graph_fy_labels.json, tolerating wrapper keys."""
...
def infer_project_name(graph_path: str, meta: dict) -> str:
    ...
    if path.parent.name == graph_fy_OUT_NAME and len(path.parents) > 1:
        return path.parents[1].name
    return path.parent.name or "Project"


def resolve_graph_fy_paths(args) -> dict:
    """Resolve project root, graph_fy output dir, and optional files."""
    base = Path(args.project).expanduser() if args.project else Path.cwd()
    if args.graph_fy_out:
        graph_fy_out = Path(args.graph_fy_out).expanduser()
    elif args.graph:
        graph_fy_out = Path(args.graph).expanduser().parent
    elif (base / "graph.json").exists():
        graph_fy_out = base
    elif (base / graph_fy_OUT).exists():
        graph_fy_out = base / graph_fy_OUT
    elif (base / "graph_fy_out").exists():
        graph_fy_out = base / "graph_fy_out"
    else:
        graph_fy_out = base / graph_fy_OUT

    project_root = graph_fy_out.parent if graph_fy_out.name in (graph_fy_OUT_NAME, "graph_fy_out", "graph_fy_out") else base
    graph = Path(args.graph).expanduser() if args.graph else graph_fy_out / "graph.json"
    report = Path(args.report).expanduser() if args.report else graph_fy_out / "GRAPH_REPORT.md"
    labels = Path(args.labels).expanduser() if args.labels else graph_fy_out / ".graph_fy_labels.json"
    sections = Path(args.sections).expanduser() if args.sections else None
    return {
        "base": project_root,
        "graph_fy_out": graph_fy_out,
        "graph": graph,
        "report": report,
        "labels": labels,
        "sections": sections,
...
def _community_text(nodes: list, label: str = "") -> str:
    ...
    for node in nodes[:80]:
        ...
        # Strip project package prefix so 'graph_fy' doesn't spuriously score as 'graph'
        src = re.sub(r"\b(graph_fy|graphify)\b", "", str(node.get("source_file", "")))
        parts.append(src)
...
def node_importance(node: dict) -> float:
    """Use graph_fy centrality fields when available."""
...
def generate_header(sections: list, meta: dict, lang: str) -> str:
    ...
    if lang.startswith("zh"):
        ...
        subtitle = (
            f"由 graph_fy 知识图谱生成：{meta.get('node_count', '?')} 个节点、"
        ...
        subtitle = (
            f"Generated from graph_fy knowledge graph: {meta.get('node_count', '?')} nodes, "
...
def section_keywords(nodes: list, limit: int = 5) -> list:
    ...
    stopwords = {
        ...
        "ts", "tsx", "js", "jsx", "go", "rs", "java", "html", "css", "fy",
        "graphify",
    }
    for node in nodes:
        text = f"{node.get('label', '')} {node.get('source_file', '')}".replace("/", " ").replace("_", " ").replace("-", " ").replace(".", " ")
...
def generate_section_cards(sec: dict, nodes: list, section_edges: list, lang: str) -> str:
    ...
    note = pick_text(
        lang,
        f"本节由 graph_fy 社区聚类生成。关系概况：{relation_text}。图表优先展示高置信、跨节点调用或使用关系，完整节点清单位于表格中。",
        f"This section comes from graph_fy community clustering. Relationship summary: {relation_text}. The diagram prioritizes high-confidence calls or usage relationships; the table keeps the broader node inventory.",
    )
...
class CallflowOptions:
    ...
    def __init__(
        ...
        *,
        graph_fy_out: str | Path | None = None,
        ...
        self.graph_fy_out = str(graph_fy_out) if graph_fy_out is not None else None
...
def write_callflow_html(
    ...
    *,
    ...
    """Generate call-flow architecture HTML from graph_fy output files."""
    ...
    paths = resolve_graph_fy_paths(args)
    if not paths["graph"].exists():
        raise FileNotFoundError(
            f"graph_fy output not found: {paths['graph']}. "
            "Run graph_fy first or pass --graph /path/to/graph.json."
        )
    ...
    if args.output:
        ...
        output_path = paths["graph_fy_out"] / f"{safe_filename(meta['project_name'])}-callflow.html"
...
""")
  ...
  <p>Generated: {datetime.now(timezone.utc).strftime('%Y-%m-%d %H:%M UTC')} · graph_fy callflow-html</p>
</div>
""")
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate call-flow architecture HTML from graph_fy knowledge graph outputs"
    )
    parser.add_argument("project", nargs="?", default=None, help="Project root or graph_fy output directory")
    parser.add_argument("--graph_fy_out", default=None, help="Path to graph_fy output directory")
    ...
    parser.add_argument("--labels", default=None, help="Path to .graph_fy_labels.json")
    ...
    try:
        write_callflow_html(
            args.project,
            graph_fy_out=args.graph_fy_out,
            graph=args.graph,
            report=args.report,
            labels=args.labels,
