"""graph_fy command dispatch — every non-install subcommand.
...
module. The path-redirect (`graph_fy <path>` -> extract) re-enters via a lazy
...
_SEARCH_NUDGE = json.dumps({
    "hookSpecificOutput": {
        ...
        "additionalContext": (
            'MANDATORY: graph_fy_out/graph.json exists. You MUST run '
            '`graph_fy query "<question>"` before grepping raw files. Only grep '
            'after graph_fy has oriented you, or to modify/debug specific lines.'
        ),
    }
...
_READ_NUDGE = json.dumps({
    "hookSpecificOutput": {
        "hookEventName": "PreToolUse",
        "additionalContext": (
            'MANDATORY: graph_fy_out/graph.json exists. You MUST run graph_fy '
            'before reading source files. Use: `graph_fy query "<question>"` '
            '(scoped subgraph), `graph_fy explain "<concept>"`, or '
            '`graph_fy path "<A>" "<B>"`. Only read raw files after graph_fy has '
...
_READ_NUDGE_STALE = json.dumps({
    "hookSpecificOutput": {
        "hookEventName": "PreToolUse",
        "additionalContext": (
            'graph_fy_out/graph.json exists but may be STALE for this file (the file '
            'changed after the last build). Prefer `graph_fy query "<question>"` for '
            'orientation, and run `graph_fy update` to refresh the graph. Reading the '
...
_READ_DENY = json.dumps({
    "hookSpecificOutput": {
        ...
        "permissionDecisionReason": (
            'graph_fy strict mode: this project has a fresh knowledge graph that covers '
            'this file. Run `graph_fy query "<your question>"` (or `graph_fy explain` / '
            '`graph_fy path`) FIRST to orient yourself, then re-issue this Read — it '
...
_GEMINI_NUDGE_TEXT = (
    'graph_fy: knowledge graph at graph_fy_out/. For focused questions, run '
    '`graph_fy query "<question>"` (scoped subgraph, usually much smaller than '
...
def _stamped_manifest_files(
    ...
    Mirrors the #1890 path normalization in graph_fy.llm.
...
def _handle_unverified_semantic_shrink(
    ...
    *,
    ...
    print(
        f"[graph_fy extract] semantic extraction is incomplete: unverified semantic "
...
def _stale_graph_sources(
    ...
    (.graph_fyignore/.gitignore/--exclude changed) without being listed in the
    ...
    """
    from graph_fy.paths import nfc
    try:
    ...
    try:
        ...
        root_res = scan_root
    # <out>/graph_fy_out/graph.json — relative source_files may be anchored here.
    ...
    def _provably_excluded(c: Path) -> bool:
        ...
        for s in spellings:
            ...
            if any(s.startswith(pref) for pref in excluded_prefixes):
                return True
        try:
            from graph_fy.detect import _is_sensitive as _det_sensitive
            if _det_sensitive(c):
                return True
        except Exception:
            pass
    ...
    if kept_alive:
        print(
            f"[graph_fy] fail-closed: kept node(s) from {len(kept_alive)} "
...
def _zero_node_stamped_code_sources(
    ...
    documented recovery was deleting graph_fy_out/. A stamped file that the
    ...
    try:
        ...
        root_res = scan_root
    # <out>/graph_fy_out/graph.json — legacy relative source_files may be
...
def _prune_graph_json_sources(graph_path: Path, stale_sources: list[str]) -> int:
    ...
    from graph_fy.export import backup_if_protected as _backup
    _backup(graph_path.parent)
    from graph_fy.paths import write_json_atomic
...
class _StageTimer:
    ...
    Monotonic (perf_counter), diagnostic-only: emits ``[graph_fy timing] <stage>:
    ...
    def mark(self, stage: str) -> None:
        now = self._now()
        if self.enabled:
            print(f"[graph_fy timing] {stage}: {now - self._last:.1f}s", file=sys.stderr)
        self._last = now

    def total(self) -> None:
        if self.enabled:
            print(f"[graph_fy timing] total: {self._now() - self.start:.1f}s", file=sys.stderr)
def _enforce_graph_size_cap_or_exit(gp: Path) -> None:
    """Reject oversized graph files before parsing (CLI exit-on-fail flavor).

    Delegates to ``graph_fy.security.check_graph_file_size_cap`` and turns the
...
def _hook_strict_enabled(flag: bool) -> bool:
    """Resolve strict mode: graph_fy_HOOK_STRICT env overrides the baked-in flag
    (truthy forces on without a reinstall, falsy is the kill switch); unset defers
    to the flag the installed hook command carried."""
    v = os.environ.get("graph_fy_HOOK_STRICT", "").strip().lower()
...
def _touch_query_stamp(graph_path: "Path") -> None:
    """Record that graph_fy oriented the agent recently, next to the queried graph.
...
def _query_stamp_fresh() -> bool:
    """True if a query/explain/path ran within graph_fy_HOOK_STRICT_TTL (default
    1800s) — recent orientation, so strict mode does not block this read."""
    from graph_fy.paths import out_path
    try:
        ttl = float(os.environ.get("graph_fy_HOOK_STRICT_TTL", "1800"))
...
def _bash_invokes_search(cmd_str: str) -> bool:
    ...
    grep. Every false positive injects a nudge where graph_fy has nothing to
...
def _run_hook_guard(kind: str, strict: bool = False) -> None:
    ...
    exists, nudges the agent to use graph_fy instead of grepping/reading raw files.
    ...
    in-project, fresh code per session is DENIED with a redirect to `graph_fy query`
    ...
    """
    from graph_fy.paths import out_path, graph_fy_OUT_NAME
    ...
    try:
        if kind == "search":
            ...
            under_out = "graph_fy_out/" in j or (graph_fy_OUT_NAME.lower() + "/") in j
...
def _clone_repo(
    ...
    Clones into ~/.graph_fy/repos/<owner>/<repo> by default so repeated
    ...
    if out_dir:
        ...
        dest = Path.home() / ".graph_fy" / "repos" / owner / repo
...
def dispatch_command(cmd: str) -> None:
    if cmd == "describe":
        ...
        from graph_fy.prs import cmd_prs
        cmd_prs(sys.argv[2:])
    elif cmd == "hook":
        from graph_fy.hooks import (
        ...
        if subcmd == "install":
            ...
            print("Usage: graph_fy hook [install|uninstall|status]", file=sys.stderr)
            sys.exit(1)
    elif cmd == "diff":
        from graph_fy.retrieval import diff_context
        base_ref = "HEAD"
        budget = 2000
        root_dir = "."
        args = sys.argv[2:]
        ...
        while i < len(args):
            if args[i] in ("--budget", "-b") and i + 1 < len(args):
                ...
                i += 1
        res = diff_context(base_ref=base_ref, root_dir=root_dir, token_budget=budget)
        if "--json" in sys.argv:
            print(json.dumps(res, indent=2))
        else:
            print(res.get("prompt_context", ""))
        sys.exit(0)
    elif cmd == "refine":
        from graph_fy.retrieval import refine_query
        code_draft = ""
        budget = 2000
        args = sys.argv[2:]
        i = 0
        while i < len(args):
            if args[i] in ("--code", "-c") and i + 1 < len(args):
                code_draft = args[i + 1]
                i += 2
            elif args[i].startswith("--code="):
                code_draft = args[i].split("=", 1)[1]
                i += 1
            elif args[i] in ("--file", "-f") and i + 1 < len(args):
                fpath = Path(args[i + 1])
                if fpath.is_file():
                    code_draft = fpath.read_text(encoding="utf-8", errors="replace")
                i += 2
            elif args[i].startswith("--file="):
                fpath = Path(args[i].split("=", 1)[1])
                if fpath.is_file():
                    code_draft = fpath.read_text(encoding="utf-8", errors="replace")
                i += 1
            elif args[i] in ("--budget", "-b") and i + 1 < len(args):
                try:
                    budget = int(args[i + 1])
                except ValueError:
                    pass
                ...
                i += 1
        if not code_draft and not sys.stdin.isatty():
            code_draft = sys.stdin.read()
        if not code_draft:
            print("Usage: graph_fy refine --code \"<draft>\" | --file <path> [--budget N]", file=sys.stderr)
            sys.exit(1)
        res = refine_query(code_draft=code_draft, token_budget=budget)
        if "--json" in sys.argv:
            print(json.dumps(res, indent=2))
        else:
            print(res.get("prompt_context", ""))
        sys.exit(0)
    elif cmd in ("context", "prebrief"):
        from graph_fy.retrieval import task_context
        task_str = ""
        budget = 1500
        include_diagram = "--no-diagram" not in sys.argv
        args = sys.argv[2:]
        i = 0
        while i < len(args):
            if args[i] in ("--budget", "-b") and i + 1 < len(args):
                ...
                try:
                    ...
                    pass
                i += 1
            elif args[i] in ("--task", "-t") and i + 1 < len(args):
                task_str = args[i + 1]
                i += 2
            elif args[i].startswith("--task="):
                task_str = args[i].split("=", 1)[1]
                i += 1
            elif not args[i].startswith("-"):
                if not task_str:
                    task_str = args[i]
                ...
                i += 1
        if not task_str and not sys.stdin.isatty():
            task_str = sys.stdin.read().strip()
        if not task_str:
            print("Usage: graph_fy context \"<task description>\" [--budget N] [--no-diagram] [--json]", file=sys.stderr)
            sys.exit(1)
        res = task_context(task=task_str, token_budget=budget, include_diagram=include_diagram)
        if "--json" in sys.argv:
            print(json.dumps(res, indent=2))
        else:
            print(res.get("prompt_context", ""))
        ...
        if len(sys.argv) < 3:
            print("Usage: graph_fy query \"<text>\" [--limit N] [--hops N] [--dfs] [--context C] [--budget N] [--graph path] [--terse] [--caveman]", file=sys.stderr)
            sys.exit(1)
        from graph_fy.serve import _query_graph_text
        from graph_fy.security import sanitize_label
        from networkx.readwrite import json_graph
        from graph_fy import querylog
        ...
        terse = "--terse" in sys.argv or "--caveman" in sys.argv
        verbose = "--verbose" in sys.argv or "-v" in sys.argv
        use_skeleton = "--skeleton" in sys.argv
        as_rag_prompt = "--rag-prompt" in sys.argv
        if as_rag_prompt:
            use_skeleton = True
        args = sys.argv[3:]
        i = 0
        while i < len(args):
            if args[i] in ("--terse", "--caveman"):
                terse = True
                i += 1
            elif args[i] == "--skeleton":
                use_skeleton = True
                i += 1
            elif args[i] == "--rag-prompt":
                as_rag_prompt = True
                use_skeleton = True
                i += 1
                ...
                try:
                    ...
                    sys.exit(1)
                i += 1
            elif args[i] == "--hops" and i + 1 < len(args):
                try:
                    hops = int(args[i + 1])
                except ValueError:
                    print("error: --hops must be an integer", file=sys.stderr)
                    sys.exit(1)
                i += 2
            elif args[i].startswith("--hops="):
                try:
                    hops = int(args[i].split("=", 1)[1])
                except ValueError:
                    print("error: --hops must be an integer", file=sys.stderr)
                    sys.exit(1)
                i += 1
        ...
        try:
            ...
            try:
                from graph_fy.build import graph_has_legacy_ids as _legacy
                if _legacy(_raw.get("nodes", [])):
                    print(
                        "[graph_fy] note: this graph uses the pre-#1504 node-ID scheme; "
                        "rebuild with `graph_fy extract --force` to get path-qualified IDs "
        ...
        _is_legacy_text = bool(
            (not (as_rag_prompt or use_skeleton))
            and (
                context_filters
                or use_dfs
                or budget != 2000
                or force_text
                or "who calls " in question.lower()
            )
            or not (gp.parent / "index.db").exists()
        )
        if _is_legacy_text:
            ...
            if terse:
                from graph_fy.retrieval import format_query_results
                print(format_query_results(_result, terse=True))
            else:
                print(_result)
            if verbose:
                try:
                    from graph_fy.mcp import estimate_token_savings
                    nodes_data = []
                    for line in _result.splitlines():
                        if line.startswith("NODE "):
                            parts = line.split()
                            node_dict = {"label": parts[1] if len(parts) > 1 else ""}
                            for part in parts[2:]:
                                if part.startswith("src="):
                                    node_dict["source_file"] = part[4:]
                                elif part.startswith("loc="):
                                    node_dict["source_location"] = part[4:]
                            nodes_data.append(node_dict)
                    root_dir = str(gp.parent.parent) if gp.parent.name == "graph_fy_out" else str(gp.parent)
                    savings = estimate_token_savings(nodes_data, root_dir=root_dir)
                    print(
                        f"\n[verbose] Token savings: {savings['tokens_saved']} tokens saved | "
                        f"{savings['file_reads_avoided']} file reads avoided | "
                        f"{savings['compression_ratio']}x compression"
                    )
                except Exception as exc:
                    print(f"\n[verbose] Could not estimate token savings: {exc}")
        else:
            from graph_fy.retrieval import query as _retrieval_query
            idx_path = gp.parent / "index.db"
            results = _retrieval_query(
                question,
                graph=G,
                db_path=idx_path,
                limit=limit,
            ...
            if verbose:
                try:
                    from graph_fy.mcp import estimate_token_savings
                    root_dir = str(gp.parent.parent) if gp.parent.name == "graph_fy_out" else str(gp.parent)
                    nodes_list = results.get("nodes", []) if isinstance(results, dict) else results
                    savings = estimate_token_savings(nodes_list, root_dir=root_dir)
                    if isinstance(results, dict):
                        results["token_savings"] = savings
                    if terse:
                        from graph_fy.retrieval import format_query_results
                        print(format_query_results(results, terse=True, skeleton=use_skeleton))
                    else:
                ...
                if as_rag_prompt and isinstance(results, list):
                    from graph_fy.retrieval import format_rag_prompt
                    print(format_rag_prompt(results, question, token_budget=budget))
                elif terse:
                    from graph_fy.retrieval import format_query_results
                    print(format_query_results(results, terse=True, skeleton=use_skeleton))
                else:
        ...
        if len(sys.argv) < 3:
            print("Usage: graph_fy affected \"<node-or-label>\" [--relation R] [--depth N] [--graph path]", file=sys.stderr)
            sys.exit(1)
        from graph_fy.affected import DEFAULT_AFFECTED_RELATIONS, format_affected, load_graph
        query = sys.argv[2]
        graph_path = _default_graph_path()
        depth = 2
        relations: list[str] = []
        ...
        # (#2706). The graph is written to <root>/<graph_fy_OUT_NAME>/graph.json,
        # so the root is the output dir's parent; a graph pointed at directly by
        # --graph falls back to its own directory.
        from graph_fy.paths import graph_fy_OUT_NAME
        graph_root = gp.parent.parent if gp.parent.name == graph_fy_OUT_NAME else gp.parent
        print(
            format_affected(
                graph,
                query,
    ...
    elif cmd in ("impact", "blast"):
        if len(sys.argv) < 3:
            print("Usage: graph_fy impact <target> [--depth N] [--json] [--terse] [--graph path]", file=sys.stderr)
            sys.exit(1)
        from graph_fy.affected import load_graph
        from graph_fy.impact import compute_impact, format_impact_report
        target = sys.argv[2]
        graph_path = _default_graph_path()
        depth = 10
        as_json = False
        ...
        while i < len(args):
            if args[i] == "--graph" and i + 1 < len(args):
                ...
                graph_path = args[i].split("=", 1)[1]
                i += 1
            elif args[i] in ("--depth", "-d") and i + 1 < len(args):
                try:
                    depth = int(args[i + 1])
                except ValueError:
                    print("error: --depth must be an integer", file=sys.stderr)
        ...
        from graph_fy.paths import graph_fy_OUT_NAME
        graph_root = gp.parent.parent if gp.parent.name == graph_fy_OUT_NAME else gp.parent
        impact_data = compute_impact(graph, target, max_depth=depth, root=graph_root)
        if as_json:
            print(json.dumps(impact_data, indent=2))
        else:
            print(format_impact_report(impact_data, terse=terse))
    elif cmd == "flow":
        if len(sys.argv) < 3:
            print("Usage: graph_fy flow <entry_point> [--sink <sink>] [--json] [--graph <path>] [--max-depth N]", file=sys.stderr)
            sys.exit(1)
        from graph_fy.affected import load_graph
        from graph_fy.flow import format_flow_diagram, trace_execution_flow
        ...
        i = 0
        while i < len(args):
            if args[i] == "--sink" and i + 1 < len(args):
                sink = args[i + 1]
                i += 2
            elif args[i].startswith("--sink="):
                sink = args[i].split("=", 1)[1]
                i += 1
            elif args[i] == "--json":
                as_json = True
                i += 1
                ...
                graph_path = args[i].split("=", 1)[1]
                i += 1
            elif args[i] in ("--max-depth", "--depth") and i + 1 < len(args):
                try:
                    ...
                    sys.exit(1)
                i += 2
            elif args[i].startswith("--max-depth=") or args[i].startswith("--depth="):
                try:
        ...
        try:
            ...
            print(f"error: could not load graph: {exc}", file=sys.stderr)
            sys.exit(1)
        flow_data = trace_execution_flow(graph, entry_point=entry_point, sink=sink, max_depth=max_depth)
        if as_json:
            print(json.dumps(flow_data, indent=2))
        else:
            print(format_flow_diagram(flow_data))
    elif cmd in ("god-nodes", "god_nodes"):
        # god_nodes has long been an analyzer (analyze.py), an MCP tool, and a
        # README-advertised capability, but never a CLI subcommand — `graph_fy
        # god_nodes` fell through to "unknown command" (#2004). Wire it as a
        # read-only graph query, mirroring `affected`.
        from graph_fy.affected import load_graph
        from graph_fy.analyze import god_nodes as _god_nodes
        from graph_fy.security import sanitize_label as _sanitize_label
        graph_path = _default_graph_path()
        top_n = 10
        ...
        # graph_fy save-result --question Q --answer A [--type T] [--nodes N1 N2 ...]
        #                      [--outcome useful|dead_end|corrected] [--correction TEXT]
        import argparse as _ap

        p = _ap.ArgumentParser(prog="graph_fy save-result")
        ...
        p.add_argument("--memory-dir", default=str(Path(_graph_fy_OUT) / "memory"))
        ...
        from graph_fy.ingest import save_query_result as _sqr
        ...
        p = _ap.ArgumentParser(prog="graph_fy reflect")
        p.add_argument("--memory-dir", default=str(Path(_graph_fy_OUT) / "memory"))
        p.add_argument(
            "--out",
            default=str(Path(_graph_fy_OUT) / "reflections" / "LESSONS.md"),
        )
        ...
        opts = p.parse_args(sys.argv[2:])
        from graph_fy.reflect import reflect as _reflect, lessons_fresh as _lessons_fresh

        graph_arg = opts.graph
        if graph_arg is None:
            default_graph = Path(_graph_fy_OUT) / "graph.json"
        ...
        if _gp is not None:
            _analysis_path = Path(opts.analysis) if opts.analysis else (
                _gp.parent / ".graph_fy_analysis.json")
            _labels_path = Path(opts.labels) if opts.labels else (
                _gp.parent / ".graph_fy_labels.json")
    ...
    elif cmd in ("path", "trace"):
        if len(sys.argv) < 4:
            print(
                f'Usage: graph_fy {cmd} "<source>" "<target>" [--graph path] '
                "[--directed|--undirected]",
                file=sys.stderr,
            )
            sys.exit(1)
        from graph_fy.serve import _pick_scored_endpoint, _score_nodes
        from networkx.readwrite import json_graph
        import networkx as _nx
        ...
        segments = []
        from graph_fy.build import edge_datas
        ...
        if len(sys.argv) < 3:
            print('Usage: graph_fy explain "<node>" [--graph path] [--terse] [--caveman]', file=sys.stderr)
            sys.exit(1)
        from graph_fy.serve import _find_node, find_node_ambiguity
        from networkx.readwrite import json_graph
        ...
        if terse:
            ...
            sloc = d.get("source_location", "")
            if sfile and sloc:
                loc_str = f"{sfile}:{sloc}"
            else:
                loc_str = sfile or sloc or ""
            print(f"[{nid}] {lbl}{' ' + loc_str if loc_str else ''}".strip())
            for direction, nb, edata in connections[:20]:
                rel = edata.get("relation", "connected")
                if direction == "out":
                    print(f"  {rel} -> {nb}")
                else:
                    print(f"  {rel} <- {nb}")
            ...
            # Work-memory overlay: a derived experiential hint from `graph_fy reflect`,
            # merged in display-only from the .graph_fy_learning.json sidecar next to
            # graph.json. No line when the node has no overlay entry.
            try:
                from graph_fy.reflect import load_learning_overlay as _llo
                from graph_fy.security import sanitize_label as _sl
                _overlay = _llo(gp)
        ...
        report_path = Path("graph_fy_out/GRAPH_REPORT.md")
        if len(sys.argv) > 2 and not sys.argv[2].startswith("-"):
            arg_p = Path(sys.argv[2])
            if arg_p.is_dir():
                cand = arg_p / "graph_fy_out" / "GRAPH_REPORT.md"
                report_path = cand if cand.exists() else arg_p / "GRAPH_REPORT.md"
            else:
                report_path = arg_p
        if not report_path.exists():
            print(f"Error: report file not found at {report_path}. Run `graph_fy build` first.", file=sys.stderr)
            sys.exit(1)
        print(report_path.read_text(encoding="utf-8"))
        sys.exit(0)

    elif cmd == "lint" and not (len(sys.argv) > 2 and sys.argv[2] == "multigraph"):
        from graph_fy.lint import run_lint
        ...
        if target_path and target_path.is_file() and target_path.suffix == ".json":
            graph_path = target_path

        if not graph_path.exists():
            print(f"error: graph file not found at {graph_path}. Run `graph_fy build` first.", file=sys.stderr)
            sys.exit(1)
        ...
        try:
            ...
            print(f"error: failed to read {graph_path}: {exc}", file=sys.stderr)
            sys.exit(1)

        result = run_lint(data)
        if json_output:
            print(json.dumps(result, indent=2))
        else:
            summary = result["summary"]
            print(f"graph_fy Lint: {result['status'].upper()}")
    ...
    elif cmd in ("diagnose", "lint"):
        subcmd = sys.argv[2] if len(sys.argv) > 2 and not sys.argv[2].startswith("-") else ""
        if subcmd != "multigraph":
            usage_cmd = "diagnose multigraph"
            print(
                f"Usage: graph_fy {usage_cmd} "
        ...
        i = 2 if (len(sys.argv) > 2 and sys.argv[2].startswith("-")) else 3
        while i < len(sys.argv):
            arg = sys.argv[i]
            if arg == "--graph":
                i += 1
        ...
        from graph_fy.diagnostics import (
            diagnose_file,
        ...
        if len(sys.argv) < 3:
            print(
                "Usage: graph_fy add <url> [--author Name] [--contributor Name] [--dir ./raw]",
                file=sys.stderr,
            )
            sys.exit(1)
        from graph_fy.ingest import ingest as _ingest
        ...
        try:
            ...
            print("Run /graph_fy --update in your AI assistant to update the graph.")
        except Exception as exc:
            print(f"error: {exc}", file=sys.stderr)
            sys.exit(1)
        ...
        while i_arg < len(sys.argv):
            ...
            if a == "--hot":
                ...
                try:
                    ...
                    pass
                i_arg += 1
            elif a == "--debounce" and i_arg + 1 < len(sys.argv):
                try:
                    debounce = float(sys.argv[i_arg + 1])
                except ValueError:
                    pass
                i_arg += 2
            elif not a.startswith("--") and watch_path is None:
                watch_path = Path(a)
                i_arg += 1
            else:
                i_arg += 1
        ...
        if not watch_path.exists():
            ...
            sys.exit(1)
        from graph_fy.watch import watch as _watch

        try:
            _watch(watch_path, debounce=debounce, hot=hot)
        ...
        # the configured backend, even when a .graph_fy_labels.json already exists.
        ...
        if graph_override is not None:
            graph_json = graph_override
        else:
            graph_json = watch_path / _graph_fy_OUT / "graph.json"
            if not graph_json.exists() and (watch_path / "graph_fy_out" / "graph.json").exists():
                graph_json = watch_path / "graph_fy_out" / "graph.json"
        if not graph_json.exists():
            print(
                f"error: no graph found at {graph_json} — run /graph_fy first",
                file=sys.stderr,
            )
            sys.exit(1)
        from networkx.readwrite import json_graph as _jg
        from graph_fy.build import build_from_json
        from graph_fy.cluster import cluster, score_all, remap_communities_to_previous
        from graph_fy.analyze import (
            god_nodes,
            surprising_connections,
            suggest_questions,
        )
        from graph_fy.report import generate
        from graph_fy.export import to_json, to_html
        ...
        from graph_fy.security import check_graph_file_size_cap as _check_cap
        try:
        ...
        # node-overlap so the existing .graph_fy_labels.json keeps attaching
        ...
        # graph_fy_out/ dir (another project/tenant's output), write beside it,
        # not into a stray graph_fy_out/ in the CWD (#1747). But when `--graph`
        # points at an arbitrary path — e.g. a `backup/graph.json` archived
        # before re-clustering (#934) — fall back to the CWD's graph_fy_out/,
        # which is the restore-into-place workflow that test pins. The default
        # (no --graph) case already has graph_json under watch_path/graph_fy_out.
        _out_name = Path(_graph_fy_OUT).name
        if graph_json.parent.name in (_out_name, "graph_fy_out", "graph_fy_out"):
            out = graph_json.parent
        else:
            out = watch_path / _graph_fy_OUT
        out.mkdir(parents=True, exist_ok=True)
        if _graph_fy_OUT == "graph_fy_out":
            compat_link = out.parent / "graph_fy_out"
            if not compat_link.exists() and not compat_link.is_symlink():
                try:
                    compat_link.symlink_to("graph_fy_out", target_is_directory=True)
                except Exception:
                    pass
        labels_path = out / ".graph_fy_labels.json"
        ...
        if labels_path.exists() and not force_relabel:
            ...
            if _ignored_label_flags:
                print(
                    f"[graph_fy] warning: {'/'.join(_ignored_label_flags)} ignored: "
                    f"reusing saved labels at {labels_path}. "
                    f"Run `graph_fy label` to relabel, or delete the labels file.",
                    file=sys.stderr,
                )
            ...
            # is told to `graph_fy label` for fresh LLM names. Unchanged communities keep
            ...
            from graph_fy.cluster import community_member_sigs, label_communities_by_hub
            ...
            # No labels file yet (or `graph_fy label` forced a refresh). When run
            ...
            from graph_fy.cluster import label_communities_by_hub
            from graph_fy.llm import generate_community_labels
        ...
        if not _commit:
            from graph_fy.watch import _git_head as _gh
        ...
        from graph_fy.export import backup_if_protected as _backup
        from graph_fy.exporters.html import _HTML_STALE_MARKER
        _backup(out)
        ...
        if not to_json(G, communities, str(out / "graph.json"),
            ...
            print(
                ...
                "GRAPH_REPORT.md, .graph_fy_labels.json and .graph_fy_analysis.json "
                "left untouched.",
                file=sys.stderr,
            )
            sys.exit(1)
        tokens = label_token_usage
        from graph_fy.report import load_learning_for_report as _llfr
        ...
        (out / ".graph_fy_analysis.json").write_text(
        ...
        if not placeholder_only:
            from graph_fy.paths import write_json_atomic as _wja
            ...
            # (see reuse above).
            from graph_fy.cluster import community_member_sigs as _cms
        ...
        if no_viz:
            ...
            try:
                from graph_fy.exporters.html import _viz_node_limit
                viz_limit = _viz_node_limit()
                if viz_limit <= 0:
                    html_target.unlink(missing_ok=True)
                    _clear_html_stale_marker()
                    skip_reason = "graph_fy_VIZ_NODE_LIMIT=0 disables HTML visualization"
                else:
        ...
        force = os.environ.get("graph_fy_FORCE", "").lower() in ("1", "true", "yes")
        no_cluster = False
        args = sys.argv[2:]
        ...
        if watch_arg is not None:
            ...
            saved = Path(_graph_fy_OUT) / ".graph_fy_root"
            if saved.exists():
                ...
                # (#3028). Match the other .graph_fy_root readers.
        ...
        if not watch_path.exists():
            print(f"error: path not found: {watch_path}", file=sys.stderr)
            sys.exit(1)
        from graph_fy.watch import _rebuild_code

        print(f"Re-extracting code files in {watch_path} (no LLM needed)...")
        # Interactive CLI: block on the per-repo lock rather than skip, so the
        # user sees their explicit `graph_fy update` complete instead of
        ...
        # toward graph_fy when a fresh in-project graph exists; always exits 0. In
        ...
        if len(sys.argv) < 3:
            print("Usage: graph_fy check-update <path>", file=sys.stderr)
            sys.exit(1)
        from graph_fy.watch import check_update
        ...
        from graph_fy.tree_html import write_tree_html, DEFAULT_MAX_CHILDREN
        graph_path = Path(_graph_fy_OUT) / "graph.json"
        ...
        while i_arg < len(args):
            ...
            if a == "--graph" and i_arg + 1 < len(args):
                ...
                print("Usage: graph_fy tree [--graph PATH] [--output HTML]")
                print("  --graph PATH         path to graph.json (default graph_fy_out/graph.json)")
                print("  --output HTML        output path (default graph_fy_out/GRAPH_TREE.html)")
        ...
        # Usage: graph_fy merge-driver %O %A %B  (set in .git/config merge driver)
        if len(sys.argv) < 5:
            print("Usage: graph_fy merge-driver <base> <current> <other>", file=sys.stderr)
            sys.exit(1)
        ...
        def _load_graph(p: str):
            ...
            from graph_fy.paths import load_node_link_graph as _lnlg
            return _lnlg(data), data
        try:
            G_cur, _ = _load_graph(_current_path)
            G_oth, _ = _load_graph(_other_path)
        except Exception as exc:
            print(f"[graph_fy merge-driver] error loading graphs: {exc}", file=sys.stderr)
        ...
        if merged.number_of_nodes() > _MERGE_MAX_NODES:
            print(
                f"[graph_fy merge-driver] merged graph has {merged.number_of_nodes()} nodes, "
        ...
        # graph_fy merge-graphs graph1.json graph2.json ... --out merged.json
        args = sys.argv[2:]
        graph_paths: list[Path] = []
        out_path = Path(_graph_fy_OUT) / "merged-graph.json"
        i = 0
        while i < len(args):
        ...
        if len(graph_paths) < 2:
            print(
                "Usage: graph_fy merge-graphs <graph1.json> <graph2.json> [...] [--out merged.json]",
                file=sys.stderr,
            )
            sys.exit(1)
        import networkx as _nx
        from networkx.readwrite import json_graph as _jg
        from graph_fy.build import prefix_graph_for_global as _prefix, distinct_repo_tags as _repo_tags
        graphs = []
        for gp in graph_paths:
            ...
            # Normalize edges/links key before loading — graph_fy writes "links"
        ...
        # (the graph_fy default) by converting MultiGraphs with nx.Graph().
        def _to_simple(g: "_nx.Graph") -> "_nx.Graph":
            ...
            if type(g) is not _nx.Graph:
                return _nx.Graph(g)
            return g
        # Unique repo tag per graph. The bare `graph_fy_out/..` dir name is not
        # unique across inputs (src/graph_fy_out and frontend/src/graph_fy_out both
        ...
        from graph_fy.cross_repo_types import link_shared_type_declarations as _link_shared
        ...
        from graph_fy.cross_repo_calls import link_cross_repo_member_calls as _link_calls
        ...
        if collected_hyperedges:
            from graph_fy.export import attach_hyperedges as _attach
        ...
        if len(sys.argv) < 3:
            print(
                "Usage: graph_fy clone <github-url> [--branch <branch>] [--out <dir>]",
                file=sys.stderr,
            )
            sys.exit(1)
        url = sys.argv[2]
        ...
        if subcmd not in ("html", "callflow-html", "obsidian", "wiki", "svg", "graphml", "neo4j", "falkordb"):
            print("Usage: graph_fy export <format>", file=sys.stderr)
        ...
        # Parse shared args
        args = sys.argv[3:]
        out_dir = Path(_graph_fy_OUT)
        if not out_dir.exists() and Path("graph_fy_out").exists():
            out_dir = Path("graph_fy_out")
        graph_path = out_dir / "graph.json"
        graph_path_explicit = False
        labels_path = out_dir / ".graph_fy_labels.json"
        ...
        analysis_path = out_dir / ".graph_fy_analysis.json"
        node_limit = 5000
        no_viz = False
        ...
        while i < len(args):
            ...
            if a == "--graph" and i + 1 < len(args):
                ...
                print("Usage: graph_fy export callflow-html [GRAPH|DIR] [--graph PATH] [--labels PATH]")
                print("  --report PATH          path to GRAPH_REPORT.md")
                print("  --sections PATH        JSON section definitions")
                print("  --output HTML          output path (default graph_fy_out/<project>-callflow.html)")
                ...
                if candidate.name == "graph.json" or candidate.suffix.lower() == ".json":
                    ...
                    graph_path = candidate / _graph_fy_OUT / "graph.json"
                graph_path_explicit = True
                i += 1
            else:
                i += 1
        ...
        if graph_path_explicit:
            ...
            if not labels_path_explicit:
                labels_path = graph_out_dir / ".graph_fy_labels.json"
        ...
        if not graph_path.exists():
            print(f"error: graph not found: {graph_path}. Run /graph_fy <path> first.", file=sys.stderr)
            sys.exit(1)

        if subcmd == "callflow-html":
            from graph_fy.callflow_html import write_callflow_html as _write_callflow_html
        ...
        from networkx.readwrite import json_graph as _jg
        from graph_fy.build import build_from_json as _bfj
        from graph_fy.security import check_graph_file_size_cap as _check_cap
        ...
        # that happens, `graph_fy export html` previously bailed with
        ...
        if subcmd == "html":
            from graph_fy.export import to_html as _to_html
            if no_viz:
            ...
            from graph_fy.export import to_obsidian as _to_obsidian, to_canvas as _to_canvas
            ...
            from graph_fy.wiki import to_wiki as _to_wiki
            from graph_fy.analyze import god_nodes as _god_nodes
            if not communities:
                print(
                    "error: .graph_fy_analysis.json is missing or empty — refusing to export wiki to prevent data loss.\n"
                    "Run `graph_fy extract .` (or `graph_fy cluster-only .`) to regenerate community data first.",
                    file=sys.stderr,
                )
                sys.exit(1)
            if not gods_data:
            ...
            if push_uri:
                ...
                from graph_fy.export import to_cypher as _to_cypher
            ...
            if push_uri:
                from graph_fy.export import push_to_falkordb as _push
                result = _push(G, uri=push_uri, user=push_user,
                               password=push_password, communities=communities)
                ...
                print(f"cypher.txt written ({out_dir}/cypher.txt) - statements are OpenCypher. "
                      ...
                      f"import), so load a graph with: graph_fy export falkordb --push "
                      f"falkordb://localhost:6379")

    elif cmd == "benchmark":
        from graph_fy.benchmark import run_benchmark, print_benchmark

        graph_path = sys.argv[2] if len(sys.argv) > 2 else _default_graph_path()
        _enforce_graph_size_cap_or_exit(Path(graph_path))
        # Try to load corpus_words from detect output
        corpus_words = None
        detect_path = Path(".graph_fy_detect.json")
        ...
        subcmd = sys.argv[2] if len(sys.argv) > 2 else ""
        from graph_fy.global_graph import (
        ...
        if subcmd == "add":
            # graph_fy global add <graph.json> [--as <tag>]
            args = sys.argv[3:]
            source = None
            tag = None
            i = 0
            while i < len(args):
                if args[i] == "--as" and i + 1 < len(args):
                    ...
                    i += 1
            if not source:
                print("Usage: graph_fy global add <graph.json> [--as <repo-tag>]", file=sys.stderr)
                sys.exit(1)
            if not tag:
                ...
                from graph_fy.build import distinct_repo_tags
            ...
            if len(sys.argv) <= 3:
                print("Usage: graph_fy global remove <repo-tag>", file=sys.stderr); sys.exit(1)
            tag = sys.argv[3]
            try:
            ...
            repos = _global_list()
            if not repos:
                print("Global graph is empty. Use 'graph_fy global add' to add a project.")
            else:
            ...
            print("Usage: graph_fy global [add|remove|list|path]", file=sys.stderr); sys.exit(1)

    elif cmd in ("extract", "build"):
        # Full-pipeline local graph build:
        # Runs detect -> AST extraction on code and docs -> build -> cluster -> FTS5 index -> write outputs.
        # Fully local, zero LLM calls, zero network access.
        has_path = True
        ...
        # --force parity with `graph_fy update`: the flag or graph_fy_FORCE=1
        ...
        while i < len(args):
            ...
            if a == "--backend" and i + 1 < len(args):
                ...
                # before, and `graph_fy tree` already documents --output, so the
        ...
        if not has_path and cli_postgres_dsn is None:
            print("Usage: graph_fy extract <path> [--code-only] [options]", file=sys.stderr)
        ...
        if deep_mode:
            print("[graph_fy extract] deep mode enabled: richer semantic extraction")

        # CLI flag wins over env var. Setting graph_fy_API_TIMEOUT here so
        # _call_openai_compat picks it up without needing a new kwarg path.
        if cli_api_timeout is not None:
            os.environ["graph_fy_API_TIMEOUT"] = str(cli_api_timeout)
        if cli_max_workers is not None:
            os.environ["graph_fy_MAX_WORKERS"] = str(cli_max_workers)

        # Resolve output dir. The user-facing contract is "<out>/graph_fy_out/"
        # so a fresh checkout writes graph_fy_out/ at the project root, matching
        # the skill.md pipeline.
        out_root = (out_dir.resolve() if out_dir else target)
        graph_fy_out = out_root / _graph_fy_OUT
        graph_fy_out.mkdir(parents=True, exist_ok=True)
        if _graph_fy_OUT == "graph_fy_out":
            compat_link = out_root / "graph_fy_out"
            if not compat_link.exists() and not compat_link.is_symlink():
                try:
                    compat_link.symlink_to("graph_fy_out", target_is_directory=True)
                except Exception:
        ...
        # flag-less `graph_fy extract` must NOT clobber it back to True, which
        ...
        _effective_gitignore = False if no_gitignore else _read_build_gi(graph_fy_out)
        # An explicit list replaces the persisted one; omission reuses it.
        _effective_excludes = cli_excludes or _read_build_ex(graph_fy_out)
        _write_build_cfg(
            graph_fy_out,
        ...
        from graph_fy.detect import (
            detect as _detect,
            detect_incremental as _detect_incremental,
            save_manifest as _save_manifest,
        )
        manifest_path = graph_fy_out / "manifest.json"
        existing_graph_path = graph_fy_out / "graph.json"
        ...
        if force and code_only and existing_graph_path.exists():
            print(
                "[graph_fy extract] --force --code-only: full AST re-scan, "
                "existing semantic layer preserved (no semantic pass this run)"
            )
        if force:
            print("[graph_fy extract] --force: full re-scan, semantic cache reads skipped")
        elif incremental_mode and not manifest_path.exists():
            print(
                "[graph_fy extract] manifest.json missing; using existing "
        ...
        if not has_path:
            ...
            print(f"[graph_fy extract] incremental scan of {target}")
            ...
            if _healed_sources:
                print(
                    f"[graph_fy extract] re-queuing {len(_healed_sources)} "
            ...
            if _healed_sem_sources:
                print(
                    f"[graph_fy extract] re-queuing {len(_healed_sem_sources)} "
            ...
            print(f"[graph_fy extract] scanning {target}")
            detection = _detect(
                target,
                google_workspace=google_workspace or None,
                extra_excludes=_effective_excludes or None,
        ...
        if code_only:
            _skipped_semantic = [*doc_files, *paper_files, *image_files]
            if _skipped_semantic:
                _s_names = ", ".join(sorted({p.name for p in _skipped_semantic})[:6])
                _s_more = f" (+{len(_skipped_semantic) - 6} more)" if len(_skipped_semantic) > 6 else ""
                print(f"[graph_fy extract] --code-only: skipping {len(_skipped_semantic)} non-code file(s): {_s_names}{_s_more}")
            semantic_files = []
        elif deep_mode:
            semantic_files = [
                Path(p) for _k in ("document", "paper", "image")
                for p in files_by_type.get(_k, [])
            ]
        else:
        ...
        if incremental_mode:
            ...
            print(
                f"[graph_fy {cmd}] {len(code_files) + len(semantic_files)} code and doc file(s) changed; "
                f"{unchanged_total} unchanged; {len(deleted_files)} deleted"
                f"{_excl_note}"
            )
        else:
            print(
                f"[graph_fy {cmd}] found {len(code_files) + len(semantic_files)} code and doc file(s)"
            )
        ...
        # are no longer invisible in graph_fy's own output (#1692).
        ...
        if _unclassified:
            ...
            print(
                f"[graph_fy extract] {len(_unclassified)} file(s) not classified "
        ...
        if _sec:
            ...
            print(
                f"[graph_fy extract] {len(_sec)} file(s) skipped as potentially sensitive "
        ...
        if needs_llm:
            if backend is None:
                ...
                sys.exit(1)
            if backend == "ollama":
                from graph_fy.llm import _validate_ollama_base_url
        ...
        if incremental_mode:
            from graph_fy.extractors.terraform import refresh_terraform_paths
        ...
        if code_files:
            from graph_fy.extract import extract as _ast_extract
            # Anchor the cache at the output root, not the scanned project:
            # with --out, a <target>/graph_fy_out/cache/ would leak a
            # graph_fy_out/ dir into a project that asked for external output.
            ...
            # #2437/#2438 (the `graph_fy update` twin of watch's #2406 fix): an
            ...
            if incremental_mode and existing_graph_path.exists():
                ...
                try:
                    from graph_fy.build import _is_ast_tier as _ctx_is_ast_tier
            ...
            print(f"[graph_fy extract] AST extraction on {len(code_files)} code files...")
            try:
                ast_result = _ast_extract(code_files, **ast_kwargs)
            except Exception as exc:
                print(f"[graph_fy extract] AST extraction failed: {exc}", file=sys.stderr)
        ...
        from graph_fy.llm import _extraction_system as _sem_prompt_for
        ...
        if semantic_files:
            ...
            if sem_cache_hits:
                print(f"[graph_fy extract] semantic cache: {sem_cache_hits} hit / {sem_cache_misses} miss")

            if uncached_paths:
                print(f"[graph_fy extract] semantic extraction on {len(uncached_paths)} files via {backend}...")
                corpus_kwargs: dict = {
                    "backend": backend,
                    "model": model,
                    "root": target,
                ...
                def _progress(idx: int, total: int, _result: dict) -> None:
                    ...
                    print(
                        f"[graph_fy extract] chunk {idx + 1}/{total} done",
                        flush=True,
                    )
                ...
                try:
                    ...
                    print(
                        f"[graph_fy extract] semantic extraction failed: {exc}",
                        file=sys.stderr,
                    )
                ...
                if uncached_paths and _chunk_stats["succeeded"] == 0:
                    print(
                        f"[graph_fy extract] error: all semantic chunks failed "
                ...
                if _dropped_files:
                    print(
                        f"[graph_fy extract] dropped {_dropped_items} out-of-scope "
                ...
                if _omitted_files or _partial_semantic_files:
                    _extraction_incomplete = True
                    print(
                        f"[graph_fy extract] semantic extraction is incomplete: "
                ...
                try:
                    ...
                    print(f"[graph_fy extract] warning: could not write semantic cache: {exc}", file=sys.stderr)
        ...
        try:
            from graph_fy.cache import file_hash as _file_hash
            ...
            print(f"[graph_fy extract] warning: could not prune semantic cache: {exc}", file=sys.stderr)
        stages.mark("semantic extract")

        pg_result: dict = {"nodes": [], "edges": []}
        if cli_postgres_dsn is not None:
            from graph_fy.pg_introspect import introspect_postgres
            print(f"[graph_fy extract] introspecting PostgreSQL schema...")
            try:
                pg_result = introspect_postgres(cli_postgres_dsn)
            except (ConnectionError, ImportError) as exc:
                print(f"error: {exc}", file=sys.stderr)
                sys.exit(1)
            print(f"[graph_fy extract] PostgreSQL: {len(pg_result['nodes'])} nodes, "
                  f"{len(pg_result['edges'])} edges")

        cargo_result: dict = {"nodes": [], "edges": []}
        if cli_cargo:
            from graph_fy.cargo_introspect import introspect_cargo
            print("[graph_fy extract] introspecting Cargo workspace...")
            try:
                cargo_result = introspect_cargo(target)
            except (ConnectionError, ImportError, OSError) as exc:
                print(f"error: {exc}", file=sys.stderr)
                sys.exit(1)
            print(f"[graph_fy extract] Cargo: {len(cargo_result['nodes'])} nodes, "
        ...
        graph_json_path = graph_fy_out / "graph.json"
        analysis_path = graph_fy_out / ".graph_fy_analysis.json"
        ...
        if no_cluster:
            ...
            from graph_fy.build import dedupe_edges as _dedupe_edges, dedupe_nodes as _dedupe_nodes
            from graph_fy.export import (
            ...
            if (
                ...
                if graph_stale_sources:
                    ...
                    if _n_pruned:
                        print(
                            f"[graph_fy extract] pruned {_n_pruned} node(s) from "
                            f"{len(graph_stale_sources)} source file(s) no longer "
                            "in the scan (deleted or excluded)."
                        )
                print(
                    "[graph_fy extract] no incremental changes detected "
                ...
                try:
                    ...
                    print(f"[graph_fy extract] warning: could not write manifest: {exc}", file=sys.stderr)
                stages.total()
                sys.exit(0)
            ...
            if merge_existing_graph:
                ...
                from graph_fy.build import merge_raw_extraction as _merge_raw_extraction
            ...
            from graph_fy.build import disambiguate_file_labels_in_nodes as _disamb_labels
            ...
            if _extraction_incomplete and not cli_allow_partial:
                from graph_fy.export import MALFORMED_GRAPH as _MALFORMED_GRAPH
                ...
                if _malformed or _shrinks:
                    ...
                    print(
                        "[graph_fy extract] error: extraction was incomplete (an AST/"
                    ...
                    sys.exit(1)
            _backup(graph_fy_out)
            _invalidate_file_manifest_for_db_graph()
            from graph_fy.paths import write_json_atomic as _write_json_atomic
            ...
            try:
                ...
                (graph_fy_out / ".graph_fy_root").write_text(
            ...
            stages.mark("write")
            try:
                from graph_fy.build import build as _build_bare
                _bare_G = _build_bare([merged], root=target)
                from graph_fy.retrieval import build_index as _build_fts_index
                _build_fts_index(_bare_G, graph_fy_out / "index.db")
                print(f"[graph_fy extract] wrote {graph_fy_out / 'index.db'} (FTS5 search index)")
            except Exception as _idx_err:
                print(f"[graph_fy extract] warning: could not build FTS5 index: {_idx_err}", file=sys.stderr)
            cost = _estimate_cost(
                backend, merged["input_tokens"], merged["output_tokens"]
            )
            print(
                f"[graph_fy extract] wrote {graph_json_path} — "
                f"{len(merged['nodes'])} nodes, {len(merged['edges'])} edges "
                f"(no clustering)"
            )
            if merged["input_tokens"] or merged["output_tokens"]:
                print(
                    f"[graph_fy extract] tokens: "
            ...
            try:
                ...
                print(f"[graph_fy extract] warning: could not write manifest: {exc}", file=sys.stderr)
            if global_merge:
                from graph_fy.global_graph import global_add as _global_add
                _tag = global_repo_tag or target.name
                try:
                    result = _global_add(graph_fy_out / "graph.json", _tag)
                    if result["skipped"]:
                        print(f"[graph_fy global] '{_tag}' unchanged since last add - skipped.")
                    else:
                        print(f"[graph_fy global] '{_tag}' merged into global graph "
                              f"(+{result['nodes_added']} nodes, -{result['nodes_removed']} pruned).")
                except Exception as exc:
                    print(f"[graph_fy global] warning: failed to merge into global graph: {exc}", file=sys.stderr)
            stages.total()
            sys.exit(0)

        # Build graph + cluster + score + write.
        from graph_fy.build import (
            build as _build,
            build_from_json as _build_from_json,
            build_merge as _build_merge,
        )
        from graph_fy.cluster import cluster as _cluster, score_all as _score_all
        from graph_fy.export import to_json as _to_json
        from graph_fy.analyze import god_nodes as _god_nodes, surprising_connections as _surprising
        ...
        if merge_existing_graph:
            ...
            try:
                ...
                print(f"[graph_fy extract] {exc}", file=sys.stderr)
                sys.exit(1)
        else:
        ...
        if G.number_of_nodes() == 0:
            print(
                "[graph_fy extract] graph is empty — extraction produced no nodes. "
        ...
        # command was invoked from, so `graph_fy extract <target>` run from
        ...
        from graph_fy.cluster import label_communities_by_hub as _label_by_hub
        hub_labels = _label_by_hub(G, communities)
        from graph_fy.watch import _git_head as _gh_target
        _wrote = _to_json(G, communities, str(graph_json_path), force=_force_write,
                          built_at_commit=_gh_target(cwd=Path(target).resolve()),
                          community_labels=hub_labels)
        if not _wrote:
            ...
            # re-attempts.
            print(
                "[graph_fy extract] error: extraction was incomplete (an AST/semantic "
        ...
        if merged.get("output_tokens", 0) > 0:
            (graph_fy_out / ".graph_fy_semantic_marker").write_text(
        ...
        # Build SQLite FTS5 search index
        try:
            from graph_fy.retrieval import build_index as _build_fts_index
            _build_fts_index(G, graph_fy_out / "index.db")
            print(f"[graph_fy extract] wrote {graph_fy_out / 'index.db'} (FTS5 search index)")
        except Exception as _idx_err:
            print(f"[graph_fy extract] warning: could not build FTS5 index: {_idx_err}", file=sys.stderr)
        ...
        try:
            from graph_fy.report import generate as _generate_report, load_learning_for_report as _llfr
            from graph_fy.analyze import suggest_questions
            questions = suggest_questions(G, communities, hub_labels)
            _commit = _gh_target(cwd=Path(target).resolve())
            _det = detection if 'detection' in locals() and isinstance(detection, dict) else {}
            report = _generate_report(
                G, communities, cohesion, hub_labels, gods, surprises,
                _det, {"input": 0, "output": 0}, str(Path(target).resolve()),
                suggested_questions=questions,
                built_at_commit=_commit,
                learning=_llfr(graph_json_path),
            )
            (graph_fy_out / "GRAPH_REPORT.md").write_text(report, encoding="utf-8")
            print(f"[graph_fy extract] wrote {graph_fy_out / 'GRAPH_REPORT.md'}")
        except Exception as _rep_err:
            print(f"[graph_fy extract] warning: could not generate GRAPH_REPORT.md: {_rep_err}", file=sys.stderr)

        # Save labels sidecar
        try:
            (graph_fy_out / ".graph_fy_labels.json").write_text(
                json.dumps({str(k): v for k, v in hub_labels.items()}, indent=2, ensure_ascii=False),
                encoding="utf-8",
            )
        except Exception:
            pass
        ...
        # Generate graph.html
        if not ("--no-viz" in sys.argv):
            html_target = graph_fy_out / "graph.html"
            try:
                from graph_fy.export import to_html as _to_html
                from graph_fy.exporters.html import _viz_node_limit
                viz_limit = _viz_node_limit()
                if viz_limit > 0:
                    _to_html(G, communities, str(html_target), community_labels=hub_labels, node_limit=viz_limit)
                    print(f"[graph_fy extract] wrote {html_target}")
            except Exception as _viz_err:
                print(f"[graph_fy extract] warning: could not write {html_target}: {_viz_err}", file=sys.stderr)
        ...
        print(f"[graph_fy extract] wrote {analysis_path}")
        if incremental_mode:
            _excl_note = f", {len(excluded_files)} excluded" if excluded_files else ""
            print(
                f"[graph_fy extract] incremental summary: "
            ...
            print(f"[graph_fy extract] semantic cache: {sem_cache_hits} cached, {sem_cache_misses} re-extracted")
        if merged["input_tokens"] or merged["output_tokens"]:
            print(
                f"[graph_fy extract] tokens: "
                f"{merged['input_tokens']:,} in / "
        ...
        # graph_fy cache-check <files_from> [--root <dir>] [--mode <m> | --deep]
        #                       [--prompt-file <path>]
        ...
        # Writes:
        #   graph_fy_out/.graph_fy_cached.json   — already-cached nodes/edges/hyperedges
        #   graph_fy_out/.graph_fy_uncached.txt  — paths that need extraction
        # Stdout: "Cache: N hit, M miss"
        from graph_fy.cache import check_semantic_cache
        if len(sys.argv) < 3:
            print("Usage: graph_fy cache-check <files_from> [--root <dir>] "
        ...
        out = root / _graph_fy_OUT
        out.mkdir(parents=True, exist_ok=True)
        if cached_nodes or cached_edges or cached_hyperedges:
            (out / ".graph_fy_cached.json").write_text(
                json.dumps({"nodes": cached_nodes, "edges": cached_edges, "hyperedges": cached_hyperedges},
                           ensure_ascii=False),
                encoding="utf-8",
            )
        (out / ".graph_fy_uncached.txt").write_text("\n".join(uncached), encoding="utf-8")
        print(f"Cache: {len(files) - len(uncached)} hit, {len(uncached)} miss")

    elif cmd == "merge-chunks":
        # graph_fy merge-chunks <chunk_glob_or_files...> --out <path>
        # Concatenates .graph_fy_chunk_*.json files written by semantic subagents.
        # Deduplicates nodes by id (first writer wins). Sums token counts.
        import glob as _glob
        if len(sys.argv) < 3:
            print("Usage: graph_fy merge-chunks <chunk_files...> --out <path>", file=sys.stderr)
            sys.exit(1)
        ...
        from graph_fy.semantic_cleanup import load_validated_semantic_fragment
        for cf in chunk_files:
            chunk, _chunk_errs = load_validated_semantic_fragment(Path(cf))
            if _chunk_errs:
                print(
                    f"[graph_fy merge-chunks] warning: skipping invalid chunk {cf}: "
        ...
        if not valid_chunks:
            print(
                f"[graph_fy merge-chunks] error: no valid chunks to merge; "
        ...
        # graph_fy merge-semantic --cached <path> --new <path> --out <path>
        ...
        if len(sys.argv) < 3:
            print("Usage: graph_fy merge-semantic --cached <path> --new <path> --out <path>", file=sys.stderr)
            sys.exit(1)
        ...
        # User ran `graph_fy <path>` directly — treat as `graph_fy extract <path>`.
        # Common when following the PowerShell note in README (`graph_fy .`) or
        ...
        print("Run 'graph_fy --help' for usage.", file=sys.stderr)
        sys.exit(1)
