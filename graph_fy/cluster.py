...
def _suppress_output():
    ...
    the call prevents this without losing any graph_fy output.
    """
...
def _native_leiden(stable: nx.Graph, resolution: float) -> dict[str, int] | None:
    ...
    in graph_fy_BUILD_PERF.md for the measurements this is based on.
    ...
    # _IdentityMapper guards against). graph_fy's own node IDs are already
...
def community_member_sigs(communities: dict[int, list[str]]) -> dict[int, str]:
    ...
    Persisted next to ``.graph_fy_labels.json`` so a later ``cluster-only`` can tell
