"""Entity deduplication pipeline for graph_fy knowledge graphs.
...
from graph_fy._minhash import MinHash, MinHashLSH
    ...
    for loser in losers:
        ...
        if lose_file == keep_file:
            if _norm(lose_label) != _norm(keep_label):
                print(
                    f"[graph_fy] note: node '{nid}' was extracted twice from "
            ...
            print(
                f"[graph_fy] WARNING: node '{nid}' is minted by two different files — "
                ...
                f"dropped node is lost. To keep them distinct, run 'graph_fy extract' "
                f"per subfolder and merge with 'graph_fy merge-graphs'.",
                file=sys.stderr,
            )
...
def deduplicate_entities(
    ...
    *,
    ...
    total = len(remap)
    msg = f"[graph_fy] Deduplicated {total} node(s)"
...
def _llm_tiebreak(
    ...
    *,
    ...
    """Batch-resolve ambiguous pairs via LLM (disabled in local no-LLM build)."""
    return
    ...
    try:
        from graph_fy.llm import _call_llm
    except ImportError as exc:
        # F-038: previously this silent fallback hid the fact that `_call_llm`
        # didn't exist in `graph_fy.llm` at all, so `--dedup-llm` was a no-op.
        # Surface the import failure so future regressions are visible.
        print(
            f"[graph_fy] --dedup-llm: cannot import _call_llm ({exc}); skipping LLM tiebreaker.",
            flush=True,
        )
        return
    ...
    for batch_start in range(0, len(ambiguous), batch_size):
        ...
        try:
            ...
            print(f"[graph_fy] --dedup-llm batch failed: {exc}", flush=True)
