...
from graph_fy.google_workspace import (
...
CODE_EXTENSIONS = {'.py', '.ts', '.tsx', '.mts', '.cts', '.js', '.jsx', '.mjs', '.cjs', '.ejs', '.ets', '.go', '.rs', '.cbl', '.cob', '.cobol', '.cpy', '.java', '.groovy', '.gradle', '.cpp', '.cc', '.cxx', '.c', '.h', '.hpp', '.cu', '.cuh', '.metal', '.rb', '.rake', '.swift', '.kt', '.kts', '.cs', '.scala', '.php', '.lua', '.luau', '.toc', '.zig', '.ps1', '.psm1', '.psd1', '.ex', '.exs', '.m', '.mm', '.ml', '.mli', '.jl', '.vue', '.svelte', '.astro', '.dart', '.v', '.sv', '.svh', '.sql', '.r', '.f', '.F', '.f90', '.F90', '.f95', '.F95', '.f03', '.F03', '.f08', '.F08', '.pas', '.pp', '.dpr', '.dpk', '.lpr', '.inc', '.dfm', '.lfm', '.lpk', '.sh', '.bash', '.json', '.tf', '.tfvars', '.hcl', '.dm', '.dme', '.dmi', '.dmm', '.dmf', '.sln', '.slnx', '.csproj', '.fsproj', '.vbproj', '.xaml', '.razor', '.cshtml', '.cls', '.trigger', '.lisp', '.cl', '.lsp', '.asd', '.robot', '.resource', '.proto', '.avsc', '.bzl', '.bazel'}
DOC_EXTENSIONS = {'.md', '.mdx', '.qmd', '.skill', '.txt', '.rst'}
...
# attacker-controllable (graph_fy runs on cloned/shared folders), and .docx/.xlsx
...
def classify_file(path: Path) -> FileType | None:
    ...
    from graph_fy.manifest_ingest import is_package_manifest_path
    if is_package_manifest_path(path):
        return FileType.CODE
    # Compound extensions must be checked before simple suffix lookup
    if path.name.lower().endswith(".blade.php"):
        return FileType.CODE
    # Infrastructure-as-code and config formats (Kubernetes, Helm, CloudFormation, Kafka)
    try:
        from graph_fy.extractors.iac import (
        ...
        if (
            is_helm_chart_file(path)
            or is_kubernetes_manifest(path)
            or is_cloudformation_template(path)
            or is_kafka_config(path)
        ):
            return FileType.CODE
    except Exception:
        pass
    # Dockerfiles and Docker Compose
    from graph_fy.extractors.docker import is_docker_compose
    if path.name.lower() == "dockerfile" or path.name.lower().startswith("dockerfile.") or path.suffix.lower() == ".dockerfile" or is_docker_compose(path):
        return FileType.CODE
    # GitHub Actions workflows
    from graph_fy.extractors.github_actions import is_github_workflow
    if is_github_workflow(path):
        return FileType.CODE
    # OpenAPI, AsyncAPI, and JSON Schema
    from graph_fy.extractors.api_schema import is_api_or_schema
    if is_api_or_schema(path):
        return FileType.CODE
    # Argo CD GitOps manifests
    from graph_fy.extractors.argocd import is_argocd_manifest
    if is_argocd_manifest(path):
        return FileType.CODE
    # Bazel and Monorepo configuration
    from graph_fy.extractors.bazel import is_bazel_file
    from graph_fy.extractors.monorepo import is_monorepo_file
    if is_bazel_file(path) or is_monorepo_file(path):
        return FileType.CODE
...
def pdf_to_markdown(path: Path) -> str:
    """Convert a .pdf file to markdown text locally without external APIs."""
    if not _file_within_size_cap(path):
        return ""
    try:
        ...
        if meta:
            ...
            if title and str(title).strip():
                fm_items.append(f"title: {str(title).strip()}")
            if author and str(author).strip():
                fm_items.append(f"author: {str(author).strip()}")
            if fm_items:
                lines.append("---")
                lines.extend(fm_items)
                lines.append("---")
                lines.append("")
        ...
        for idx, page in enumerate(reader.pages):
            ...
            if num_pages > 1:
                ...
                lines.append("")
            # Clean and normalize lines (standardize bullet points and code backticks)
            for page_line in text.splitlines():
                stripped = page_line.strip()
                if stripped.startswith(("•", "·", "–", "—")):
                    stripped = "- " + stripped.lstrip("•·–— ").strip()
                # Normalize PDF smart quotes / typographic backticks often generated for `
    ...
    # Fallback: regex scan for literal strings in raw PDF byte streams
    try:
        raw_bytes = path.read_bytes()
        text_matches = re.findall(rb"\(([^)\r\n]+)\)\s*Tj", raw_bytes)
        if text_matches:
            extracted = [m.decode("latin1", errors="ignore").strip() for m in text_matches]
...
def xlsx_extract_structure(path: Path) -> dict:
    ...
    Returns a nodes/edges dict compatible with the graph_fy extract pipeline.
...
def convert_office_file(path: Path, out_dir: Path, root: "Path | None" = None) -> Path | None:
    ...
    # byte-identical sidecars — unbounded duplicates when graph_fy_out/ is
    ...
    import unicodedata
    if root is None:
        # Default layout: out_dir is <root>/<graph_fy_out>/converted.
        root = out_dir.parent.parent
    try:
        key = path.resolve().relative_to(Path(root).resolve()).as_posix()
    except (ValueError, OSError):
        # Not under the scan root (custom graph_fy_OUT layouts, --include
...
def count_words(path: Path) -> int:
    try:
        ...
        # cannot help, and `graph_fy update` hangs with no output. os.stat
...
_SKIP_DIRS = {
    ...
    "graph_fy_out", "graph_fy_out",  # never treat the default output as source input (#524)
    ...
    ".graph_fy",  # graph_fy's own extraction cache — never index self-generated data
...
def _parse_gitignore_line(raw: str) -> str:
    """Parse one raw line from a .graph_fyignore file per gitignore spec.
...
def _git_info_exclude(vcs_root: Path) -> Path | None:
    ...
    a directory without any ``.gitignore`` entry. graph_fy only read
    ``.gitignore``/``.graph_fyignore``, so it walked into those worktree copies and
...
def _read_ignore_text(path: Path) -> str:
    ...
    for enc in (fallback, "latin-1"):
        ...
        if key not in _warned_ignore_encodings:
            ...
            print(
                f"[graph_fy] WARNING: {path} is not valid UTF-8; read it as "
...
def _load_dir_own_ignore(d: Path, *, gitignore: bool = True) -> list[tuple[Path, str]]:
    """Read .gitignore/.graph_fyignore directly inside *d* (not its ancestors).

    Merges .gitignore and .graph_fyignore for this one directory (#1363):
    .gitignore is read first and .graph_fyignore last, so .graph_fyignore
    patterns (including `!` negations) win on conflict via last-match-wins;
    adding a .graph_fyignore can only ever exclude MORE, never re-include a
    .gitignore-excluded file (#945 kept: a dir with only a .gitignore still
    gets sensible defaults).

    Shared by `_load_graph_fyignore` (ancestor chain, loaded once before the
    ...
    for fname in ((".gitignore", ".graph_fyignore") if gitignore else (".graph_fyignore",)):
        ignore_file = d / fname
...
def _load_graph_fyignore(root: Path, *, gitignore: bool = True) -> list[tuple[Path, str]]:
    """Read .graph_fyignore files and return (anchor_dir, pattern) pairs.
    ...
    # per-directory .gitignore/.graph_fyignore — so load it first (lowest priority
...
# `graph_fy watch` process spanning many repos could still accumulate keys over
...
def _is_ignored(
    ...
    *,
    ...
    """Return True if the path should be ignored per .graph_fyignore patterns.
...
def _is_scan_ignored(
    ...
    *,
    ...
    contains only .graph_fyignore/--exclude rules, which remain authoritative
...
def ignored_predicate(
    ...
    *,
    ...
    .graph_fyignore/.gitignore chain (_load_graph_fyignore), CLI/persisted
    ...
    """
    root = root.resolve()
    patterns = _load_graph_fyignore(root, gitignore=gitignore)
    explicit_patterns = _load_graph_fyignore(root, gitignore=False)
    # Only shell out to git when .gitignore actually contributes patterns beyond
    # the explicit (.graph_fyignore/--exclude) set: with no .gitignore in play,
    ...
    cache: dict[Path, bool] = {}
    explicit_cache: dict[Path, bool] = {}
    # root's own ignore file is the last entry of _load_graph_fyignore's chain.
...
def detect(root: Path, *, follow_symlinks: bool | None = None, google_workspace: bool | None = None, extra_excludes: list[str] | None = None, cache_root: Path | None = None, gitignore: bool = True) -> dict:
    root = root.resolve()
    configured_out_dir = root / graph_fy_OUT
    ...
    # .graph_fyinclude support was removed (#2112): its loader and matchers had
    ...
    if (root / ".graph_fyinclude").is_file():
        import sys as _sys
        print(
            "[graph_fy] WARNING: .graph_fyinclude is no longer supported "
    ...
    def _wc(path: Path) -> int:
        ...
        from graph_fy import cache as _cache
        return _cache.cached_word_count(path, root, count_words, cache_root=cache_root)

    skipped_sensitive: list[str] = []
    unclassified: list[str] = []
    # Files/dirs dropped by a .gitignore/.graph_fyignore rule. Recorded so an
    ...
    ignore_patterns = _load_graph_fyignore(root, gitignore=gitignore)
    explicit_ignore_patterns = _load_graph_fyignore(root, gitignore=False)
    ...
    # so they win over any .graph_fyignore/.gitignore rules (#947).
    if extra_excludes:
        for pat in extra_excludes:
            line = _parse_gitignore_line(pat)
            if line:
    ...
    # Always include graph_fy_out/memory/ - query results filed back into the graph
    memory_dir = root / graph_fy_OUT / "memory"
    scan_paths = [root]
    ...
    def _on_walk_error(err: OSError) -> None:
        ...
        print(
            f"[graph_fy] WARNING: could not scan {target} ({err}); "
    ...
    converted_dir = out_base / graph_fy_OUT / "converted"
        ...
        if ftype:
            if p.suffix.lower() in GOOGLE_WORKSPACE_EXTENSIONS:
                if not google_workspace:
                    skipped_sensitive.append(
                        ...
                        "or set graph_fy_GOOGLE_WORKSPACE=1]"
                    )
                    continue
                try:
                ...
                if md_path:
                    ...
                    # graph_fy_out/ -- an ignore check here would reject the
            ...
            if p.suffix.lower() in OFFICE_EXTENSIONS:
                ...
                if md_path:
                    ...
                    skipped_sensitive.append(str(p) + " [office conversion failed - pip install graph_fyy[office]]")
                continue
    ...
    return {
        ...
        "graph_fyignore_patterns": len(ignore_patterns),
...
def _to_relative_for_storage(key: str, root: Path) -> str:
    ...
    fallback in :func:`graph_fy.watch._relativize_source_files` so the
...
def load_manifest(
    ...
    *,
    ...
    manifests with absolute keys pass through unchanged, so a graph_fy_out/
...
def save_manifest(
    ...
    *,
    ...
    kind="ast"      — written by `graph_fy update` (AST-only rebuild). Stamps
                      ast_hash; preserves an existing semantic_hash only when
                      the file content is unchanged (mtime + hash match).
    kind="semantic" — written by `graph_fy extract` after semantic extraction.
    ...
    .graph_fyignore/.gitignore/--exclude) are dropped instead of surviving
...
def detect_incremental(
    ...
    *,
    ...
    kind="semantic" (default for extract): a file is "changed" when its
        ...
        semantic extraction pass. Use this for `graph_fy extract` so that
        files touched by `graph_fy update` (AST-only) are re-extracted
        semantically.
    kind="ast": a file is "changed" when its ast_hash is missing or its
        content has changed. Use this for `graph_fy update`.
