...
from graph_fy.exporters.graphdb import push_to_falkordb, push_to_neo4j  # noqa: E402,F401
...
_BACKUP_ARTIFACTS = [
    "graph.json",
    "GRAPH_REPORT.md",
    ".graph_fy_labels.json",
    ".graph_fy_analysis.json",
    "manifest.json",
    ".graph_fy_semantic_marker",
    "cost.json",
]
...
def backup_if_protected(out_dir: Path) -> "Path | None":
    ...
    - .graph_fy_semantic_marker is present (graph cost real LLM tokens), or
    - .graph_fy_labels.json contains at least one non-default community label
    ...
    Set graph_fy_NO_BACKUP=1 to disable.
    """
    if os.environ.get("graph_fy_NO_BACKUP"):
        return None
    out = Path(out_dir)
    if not (out / "graph.json").exists():
        return None

    is_semantic = (out / ".graph_fy_semantic_marker").exists()
    is_curated = False
        ...
        for name in _BACKUP_ARTIFACTS:
            ...
            if src.exists():
                try:
                    ...
                    pass
        if copied:
            print(f"[graph_fy] backed up {reason} graph ({copied} files) -> {backup_dir.name}/")
        return backup_dir
    except Exception as exc:
        import sys
        print(f"[graph_fy] warning: backup failed ({exc}) - continuing with overwrite", file=sys.stderr)
        return None
...
def _yaml_str(s: str) -> str:
    ...
    See `graph_fy.ingest._yaml_str` for the full rationale; duplicated here to
...
def to_json(G: nx.Graph, communities: dict[int, list[str]], output_path: str, *, force: bool = False, built_at_commit: str | None = None, community_labels: dict[int, str] | None = None) -> bool:
        ...
        if not oversized:
            ...
            if not raw.strip():
                ...
                try:
                    ...
                    import sys as _sys
                    print(
                        f"[graph_fy] WARNING: existing {existing_path} could not be "
            ...
            if new_n < existing_n:
                import sys as _sys
                print(
                    f"[graph_fy] WARNING: new graph has {new_n} nodes but existing "
                    ...
                    f"Run a full rebuild (/graph_fy .) to be safe, or pass force=True "
    ...
    if "hyperedges" not in getattr(G, "graph", {}):
        ...
        if _prev_hyperedges:
            print(
                f"[graph_fy] WARNING: graph carries no hyperedge metadata but "
    ...
    # (output_path lives in <target>/graph_fy_out/), never the shell's cwd —
...
def to_cypher(G: nx.Graph, output_path: str) -> None:
    lines = ["// Neo4j Cypher import - generated by /graph_fy", ""]
...
# A frontmatter tag entry in graph_fy's own namespace, e.g. "  - graph_fy/document".
_graph_fy_TAG_RE = re.compile(r"^\s*-\s+graph_fy/\S")
...
# Community notes carry no frontmatter; graph_fy identifies its own by the
# Dataview query it writes into every one of them.
_COMMUNITY_QUERY_MARKER = "FROM #community/"


def _is_graph_fy_note(path: Path) -> bool:
    """Whether a vault note carries graph_fy's own frontmatter signature.

    Every note graph_fy writes opens with a YAML frontmatter block tagging it in
    the ``graph_fy/`` namespace::
    ...
    user's note that merely mentions graph_fy in its prose is not adopted.

    Community overview notes are recognised separately: they carry no
    frontmatter at all, so they are identified by graph_fy's own filename prefix
    ...
    for line in head.splitlines()[1:]:
        if line.strip() == "---":
            return False  # frontmatter closed without a graph_fy tag
        if _graph_fy_TAG_RE.match(line):
            return True
    return False


def _adopt_pre_manifest_notes(out: Path) -> set[str]:
    """Names of notes in *out* that graph_fy itself wrote before manifests existed.

    Deliberately limited to top-level ``*.md``: those are the only files graph_fy
    can identify as its own from their content. ``.obsidian/graph.json`` is NOT
    adopted — graph_fy writes one, but so does Obsidian, and with no manifest
    ...
    try:
        ...
        return set()
    return {p.name for p in candidates if _is_graph_fy_note(p)}
...
def to_obsidian(
    ...
    # .obsidian/ config. Track the files graph_fy owns in a manifest; a pre-existing
    # file NOT in the manifest is the user's and is never overwritten.
    _manifest_path = out / ".graph_fy_obsidian_manifest.json"
    try:
    ...
    if not _manifest_existed:
        ...
        # graph_fy owns, so every note it wrote last time reads as the user's and
        # is skipped. The re-export then writes fresh notes BESIDE the stale ones
        # and the vault carries two generations, with a warning claiming graph_fy
        # "did not create" files it did (#2863). Adopt the notes that carry
        # graph_fy's own frontmatter, once, so the manifest starts out honest.
        _owned |= _adopt_pre_manifest_notes(out)
    _written: list[str] = []
    _skipped: list[str] = []

    def _owned_write(rel_name: str, content: str) -> bool:
        """Write a graph_fy-owned file, refusing to overwrite a pre-existing file
        graph_fy didn't create. Returns True if written."""
        target = out / rel_name
    ...
    # Map file_type → graph_fy tag
    _FTYPE_TAG = {
        "code": "graph_fy/code",
        "document": "graph_fy/document",
        "paper": "graph_fy/paper",
        "image": "graph_fy/image",
    }
    ...
    for node_id, data in G.nodes(data=True):
        ...
        ftype_tag = _FTYPE_TAG.get(ftype, f"graph_fy/{ftype}" if ftype else "graph_fy/document")
        dom_conf = _dominant_confidence(node_id)
        conf_tag = f"graph_fy/{dom_conf}"
    ...
    # view — but never clobber an existing .obsidian/graph.json graph_fy doesn't own
    ...
    # manifest says graph_fy owns are candidates, and anything written or skipped
    ...
    if pruned:
        print(
            f"[graph_fy] pruned {pruned} note(s) for nodes no longer in the graph",
            file=sys.stderr,
        )

    # Persist the manifest of files graph_fy owns, so a re-run can safely update its
    ...
    if _skipped:
        ...
        print(
            f"[graph_fy] WARNING: skipped {len(_skipped)} pre-existing file(s) graph_fy "
            f"did not create, to avoid overwriting your notes: {shown}. "
            f"Export into an empty directory (or the default graph_fy_out/obsidian) "
