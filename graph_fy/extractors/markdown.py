"""Markdown extractor. Moved verbatim from graph_fy/extract.py."""
...
from graph_fy.detect import CODE_EXTENSIONS
from graph_fy.extractors.base import _file_stem, _make_id
...
_MD_LINKABLE_EXTS = {".md", ".mdx", ".qmd", ".markdown", ".rst", ".txt", ".pdf"}
...
# Hashtags: #tag, #nested/tag, #my-tag. Must contain at least one letter;
# headings (# Heading), numeric issue refs (#123), hex colors, and anchors are ignored.
_MD_TAG_RE = re.compile(r'(?<![A-Za-z0-9_&/:#\\])#([A-Za-z0-9_\-/]*[A-Za-z][A-Za-z0-9_\-/]*)')
...
# not a declared dependency of graph_fy (see manifest_ingest._parse_apm for the
...
def _active_scan_root() -> "Path | None":
    ...
    fallback stays off.
    """
    import graph_fy.extract as _extract
...
def _build_link_index(root: Path) -> "dict[str, list[tuple[int, str, Path]]]":
    ...
    index dot-folders.
    """
    from graph_fy.detect import _SKIP_DIRS
...
def extract_markdown(path: Path, source_text: str | None = None) -> dict:
    ...
    ``graph_fy.markdown_resolution``). That pass emits
    ...
    def add_tag(tag_str: str, line: int) -> None:
        clean = tag_str.strip().lstrip("#").rstrip(".,;:!?)'\"")
        if not clean or not any(c.isalpha() for c in clean):
            return
        if _HEX_COLOR_RE.match(clean):
            return
        tag_nid = _make_id("tag", clean.lower())
        add_node(tag_nid, f"Tag:#{clean}", line, file_type="concept", node_kind="tag", extra={"display_label": f"Tag:#{clean}"})
        if tag_nid not in seen_tags:
            seen_tags.add(tag_nid)
            add_edge(file_nid, tag_nid, "tagged_as", line)
    ...
    fm_extra: dict = {}
    if frontmatter:
        fm_extra["frontmatter"] = frontmatter
        if "aliases" in frontmatter:
            aliases = frontmatter["aliases"]
            fm_extra["aliases"] = aliases if isinstance(aliases, list) else [str(aliases)]
        if "type" in frontmatter:
            fm_extra["doc_type"] = str(frontmatter["type"])
        if "category" in frontmatter:
            fm_extra["category"] = str(frontmatter["category"])
        if "title" in frontmatter:
            fm_extra["title"] = str(frontmatter["title"])

    title_val = frontmatter.get("title") if frontmatter else None
    disp_lbl = f"Note:{title_val or path.stem}"
    fm_extra["display_label"] = disp_lbl

    add_node(file_nid, path.name, 1, node_kind="page",
             extra=fm_extra if fm_extra else None)

    # Frontmatter tags
    fm_tags = frontmatter.get("tags") if frontmatter else None
    if not fm_tags and frontmatter:
        fm_tags = frontmatter.get("tag")
    if fm_tags:
        if isinstance(fm_tags, list):
            for t in fm_tags:
                add_tag(str(t), 1)
        elif isinstance(fm_tags, str):
            for t in fm_tags.replace(",", " ").split():
                add_tag(t, 1)
    ...
    def add_link(raw: str, line: int, wikilink: bool = False) -> None:
        ...
        try:
            ...
            pass
        rel = "references_heading" if ("#" in raw and not raw.startswith("#")) else "references"
        add_edge(file_nid, tgt_nid, rel, line, target_file=target_file)

    # Track heading stack for nesting: [(level, nid), ...]
    heading_stack: list[tuple[int, str]] = []
    in_code_block = False
    current_nid = file_nid
    section_lines: dict[str, list[str]] = {file_nid: []}
    ...
    for line_num_0, line_text in enumerate(lines):
        ...
        if stripped.startswith("```"):
            ...
            section_lines.setdefault(current_nid, []).append(line_text)
            continue
        ...
        # Extract tags from body lines (strip code spans and links first)
        scan_text = _MD_CODE_SPAN_RE.sub(" ", line_text)
        scan_text = _MD_INLINE_LINK_RE.sub(" ", scan_text)
        scan_text = _MD_WIKILINK_RE.sub(" ", scan_text)
        for m in _MD_TAG_RE.finditer(scan_text):
            add_tag(m.group(1), line_num)
        ...
        if heading_match:
            ...
            add_node(h_nid, title, line_num, extra={"display_label": f"Heading:{title}", "heading_level": level})
            current_nid = h_nid
            section_lines.setdefault(h_nid, []).append(line_text)
    ...
    for node in nodes:
        nid = node["id"]
        if nid in section_lines:
            node["text"] = "\n".join(section_lines[nid]).strip()
...
def extract_pdf(path: Path) -> dict:
    """Extract structural nodes and edges from a PDF file.

    Converts the PDF to Markdown text locally via pypdf (with stream regex fallback),
    then routes the markdown through extract_markdown with source_text.
    Extracts pages, headings, document metadata frontmatter, and resolves inline
    code span mentions against code symbols across the corpus.
    """
    from graph_fy.detect import pdf_to_markdown
    md_text = pdf_to_markdown(path)
    return extract_markdown(path, source_text=md_text)
