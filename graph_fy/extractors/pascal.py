"""pascal — moved verbatim from graph_fy/extract.py."""
from __future__ import annotations

import re
from graph_fy.extractors.base import _file_stem, _make_id
from graph_fy.extractors.resolution import _pascal_resolve_class, _pascal_resolve_unit
    ...
    for caller_nid, caller_line, body_text, _container, _name_lower in impl_records:
        for cm in _PAS_CALL_RE.finditer(body_text):
            ...
            if not target_nid:
                ...
                # cross-file resolver (graph_fy.pascal_resolution) instead of
...
def extract_pascal(path: Path) -> dict:
    ...
    def _emit_or_report(caller_nid: str, name_lower: str, line: int) -> None:
        ...
        if not target:
            ...
            # resolver (graph_fy.pascal_resolution) instead of guessing or
