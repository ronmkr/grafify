"""resolution — moved verbatim from graph_fy/extract.py."""
...
def _match_tsconfig_alias(raw: str, pattern: str) -> "tuple[tuple[int, int], str, bool] | None":
    ...
    rule. The final branch preserves graph_fy's existing support for treating a
...
def _find_js_project_anchor(start_dir: Path) -> Path:
    ...
    """
    from graph_fy.detect import _find_vcs_root
...
def _pascal_resolve_unit(from_path: Path, unit_name: str) -> str:
    """Resolve a Pascal unit name to the graph_fy node ID of its source file.
