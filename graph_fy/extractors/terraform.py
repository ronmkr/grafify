...
def refresh_terraform_paths(
    ...
    """
    if not any(p.suffix == ".tf" or p.name in (".gitignore", ".graph_fyignore")
    ...
    for block in body.children:
        ...
        if btype == "resource" and len(labels) >= 2:
            ...
            if res_type.startswith("aws_"):
                display_label = f"AWS:{res_type}.{res_name}"
            elif res_type.startswith("kubernetes_"):
                display_label = f"K8s:{res_type}.{res_name}"
            elif res_type.startswith("helm_"):
                display_label = f"Helm:{res_type}.{res_name}"
            else:
                display_label = f"{res_type}.{res_name}"
            owner = _add_node(f"{res_type}.{res_name}", display_label, line)
            node_dict = nodes_by_id[owner]
            node_dict["type"] = "terraform_resource"
            node_dict["resource_type"] = res_type
            node_dict["resource_name"] = res_name
            if blk_body is not None:
                for child in blk_body.named_children:
                    if child.type == "attribute" and len(child.named_children) >= 2:
                        attr_name = _read(child.named_children[0]).strip()
                        val_node = child.named_children[-1]
                        while val_node.type in ("expression", "literal_value") and len(val_node.named_children) == 1:
                            val_node = val_node.named_children[0]
                        attr_val = _read(val_node).strip().strip('"\'')
                        if attr_name in ("chart", "image", "name", "table_name", "bucket", "repository"):
            ...
            nodes_by_id[owner]["type"] = "terraform_module"
            nodes_by_id[owner]["resource_name"] = labels[0]
