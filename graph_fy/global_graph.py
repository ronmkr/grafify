...
_GLOBAL_DIR = Path.home() / ".graph_fy"
    ...
    if _GLOBAL_MANIFEST.exists():
        try:
            ...
            try:
                ...
                print(
                    f"[graph_fy global] manifest at {_GLOBAL_MANIFEST} failed to parse ({exc}); "
...
def global_add(source_path: Path, repo_tag: str) -> dict:
    ...
    """
    from graph_fy.build import prefix_graph_for_global, prune_repo_from_graph
    ...
    if existing_path and existing_path != str(source_path.resolve()):
        print(
            f"[graph_fy global] warning: repo tag '{repo_tag}' previously pointed to "
    ...
    from graph_fy.cross_repo_calls import link_cross_repo_member_calls
