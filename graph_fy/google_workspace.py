...
googleworkspace CLI (`gws`) so graph_fy can extract their actual contents.
"""
...
def google_workspace_enabled(value: str | None = None) -> bool:
    ...
    raw = value if value is not None else os.environ.get("graph_fy_GOOGLE_WORKSPACE", "")
...
def _run_gws_export(file_id: str, mime_type: str, output: Path, resource_key: str | None = None) -> None:
    ...
    timeout = int(os.environ.get("graph_fy_GOOGLE_WORKSPACE_TIMEOUT", "120"))
...
def _sidecar_path(path: Path, out_dir: Path, root: "Path | None" = None) -> Path:
    ...
    # sidecars, each ingested as a distinct source doc when graph_fy_out/ is
    ...
    *,
    ...
    if ext == ".gsheet":
        if xlsx_to_markdown is None:
            raise RuntimeError("Google Sheets export requires the office extra: pip install graph_fyy[office,google]")
