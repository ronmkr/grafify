# git hook integration - install/uninstall graph_fy post-commit and post-checkout hooks
...
_HOOK_MARKER = "# graph_fy-hook-start"
_HOOK_MARKER_END = "# graph_fy-hook-end"
_CHECKOUT_MARKER = "# graph_fy-checkout-hook-start"
_CHECKOUT_MARKER_END = "# graph_fy-checkout-hook-end"

# __PINNED_PYTHON__ is replaced at install time with the absolute path of the
# Python interpreter that ran `graph_fy hook install`.  For uv-tool and pipx
...
# when the graph_fy launcher is not on PATH (common in GUI clients and CI).
#
# Probes check availability with importlib.util.find_spec instead of importing
# the package: a probe that imports graph_fy wholesale executes the full package
...
_GFY_PROBE="import importlib.util, sys; sys.exit(0 if importlib.util.find_spec('graph_fy') else 1)"
graph_fy_PYTHON=""
_PINNED='__PINNED_PYTHON__'
if [ -n "$_PINNED" ] && [ -x "$_PINNED" ] && "$_PINNED" -c "$_GFY_PROBE" 2>/dev/null; then
    graph_fy_PYTHON="$_PINNED"
fi
# Second probe: read graph_fy_out/.graph_fy_python (written by the skill and
# CLI; survives uv-tool reinstalls and is the same source the README documents).
if [ -z "$graph_fy_PYTHON" ]; then
    _GFY_PYTHON_FILE="graph_fy_out/.graph_fy_python"
    if [ -f "$_GFY_PYTHON_FILE" ]; then
        ...
        if [ -n "$_FROM_FILE" ] && [ -x "$_FROM_FILE" ] && "$_FROM_FILE" -c "$_GFY_PROBE" 2>/dev/null; then
            graph_fy_PYTHON="$_FROM_FILE"
        fi
    fi
fi
# Third probe: resolve via the graph_fy launcher on PATH.
if [ -z "$graph_fy_PYTHON" ]; then
    graph_fy_BIN=$(command -v graph_fy 2>/dev/null || command -v graph_fy 2>/dev/null)
    if [ -n "$graph_fy_BIN" ]; then
        # Windows pip layout: Scripts/graph_fy(.exe) sits beside ..\\python.exe
    ...
    if [ -z "$graph_fy_PYTHON" ] && [ -n "$graph_fy_BIN" ]; then
        ...
        case "$graph_fy_BIN" in
            *.exe) _GFY_HEAD="" ;;
            *)     _GFY_HEAD=$(head -c 256 "$graph_fy_BIN" 2>/dev/null | tr -d '\\000') ;;
        esac
        case "$_GFY_HEAD" in
        ...
        case "$_SHEBANG" in
            */env\\ *) graph_fy_PYTHON="${_SHEBANG#*/env }" ;;
            *)         graph_fy_PYTHON="$_SHEBANG" ;;
        esac
        # Allowlist: only keep characters valid in a filesystem path to prevent
        # injection if the shebang contains shell metacharacters.
        case "$graph_fy_PYTHON" in
            *[!a-zA-Z0-9/_.@:\\\\-]*) graph_fy_PYTHON="" ;;
        esac
        if [ -n "$graph_fy_PYTHON" ] && ! "$graph_fy_PYTHON" -c "$_GFY_PROBE" 2>/dev/null; then
            graph_fy_PYTHON=""
        fi
    fi
fi
# Fourth probe: uv tool environments. `uv tool install` (the README's
# recommended method) puts graph_fy in an isolated venv that no ambient
...
# co-installed tool without graph_fy never satisfies it.
#
...
if [ -z "$graph_fy_PYTHON" ]; then
    if command -v python3 >/dev/null 2>&1 && python3 -c "$_GFY_PROBE" 2>/dev/null; then
        graph_fy_PYTHON="python3"
    elif command -v python >/dev/null 2>&1 && python -c "$_GFY_PROBE" 2>/dev/null; then
        graph_fy_PYTHON="python"
    else
        echo "[graph_fy hook] could not locate a Python with graph_fy installed. Add the graph_fy bin dir to PATH or re-run 'graph_fy hook install' from the env where graph_fy lives." >&2
        exit 0
    fi
fi
"""
...
changed_raw = os.environ.get('graph_fy_CHANGED', '')
changed = [Path(f.strip()) for f in changed_raw.strip().splitlines() if f.strip()]

if not changed:
    sys.exit(0)

print(f'[graph_fy hook] {len(changed)} file(s) changed - rebuilding graph...')

try:
    from graph_fy.watch import _rebuild_code, _apply_resource_limits
    _apply_resource_limits()
    _timeout = int(os.environ.get('graph_fy_REBUILD_TIMEOUT', '600'))
    if _timeout > 0:
        if hasattr(signal, 'SIGALRM'):
            def _sigalrm_bail(*_a):
                ...
                for _child in multiprocessing.active_children():
                    _child.kill()
                raise TimeoutError(f'graph_fy rebuild exceeded {_timeout}s')
            signal.signal(signal.SIGALRM, _sigalrm_bail)
            signal.alarm(_timeout)
        else:
            def _bail():
                print(f'[graph_fy hook] graph_fy rebuild exceeded {_timeout}s', flush=True)
                for _child in multiprocessing.active_children():
                    _child.kill()
                os._exit(1)
            ...
            _watchdog.start()
    _force = os.environ.get('graph_fy_FORCE', '').lower() in ('1', 'true', 'yes')
    _root = Path('.')
    _out = os.environ.get('graph_fy_OUT', 'graph_fy_out')
    _saved = Path(_out) / '.graph_fy_root'
    if _saved.exists():
        ...
        if _txt:
            ...
            if _in_repo:
                ...
                print(f'[graph_fy hook] ignoring out-of-repo .graph_fy_root: {_txt}')
    ...
    try:
        ...
        if _md.is_dir() and any(_md.glob('*.md')):
            from graph_fy.reflect import reflect as _reflect
    ...
    print(f'[graph_fy hook] {exc}')
    sys.exit(1)
except Exception as exc:
    print(f'[graph_fy hook] Rebuild failed: {exc}')
    sys.exit(1)
"""
...
try:
    ...
    if _timeout > 0:
        if hasattr(signal, 'SIGALRM'):
            ...
            def _bail():
                print(f'[graph_fy] graph_fy rebuild exceeded {_timeout}s', flush=True)
                for _child in multiprocessing.active_children():
                    _child.kill()
                os._exit(1)
            _watchdog = threading.Timer(_timeout, _bail)
    ...
    if _saved.exists():
        ...
        if _txt:
            ...
            if _in_repo:
                ...
                print(f'[graph_fy] ignoring out-of-repo .graph_fy_root: {_txt}')
    ...
    print(f'[graph_fy] {exc}')
    sys.exit(1)
except Exception as exc:
    print(f'[graph_fy] Rebuild failed: {exc}')
    sys.exit(1)
"""

# Cross-platform detached-launch shim (#1161). The hooks used to background the
# rebuild with `nohup "$graph_fy_PYTHON" -c "..." &`, but Git for Windows' bundled
...
# still returned 0, so the graph just went stale with no signal. graph_fy already
...
__REBUILD_BODY__
'''
_log = os.environ.get('graph_fy_REBUILD_LOG') or os.path.join(os.path.expanduser('~'), '.cache', 'graph_fy-rebuild.log')
try:
...
def _detached_launch(rebuild_body: str) -> str:
    ...
    Python process via ``$graph_fy_PYTHON``.
    ...
    The launcher writes the child's output to ``$graph_fy_REBUILD_LOG`` and
    ...
    return '"$graph_fy_PYTHON" -c "' + launcher + '"\n'
...
# fires these hooks; the canonical graph_fy_out/ belongs to the primary checkout,
...
# rebuild ("failed to remove graph_fy_out/: Directory not empty") (#1809, #1806).
...
# anything after graph_fy's end marker - on every root commit (HEAD~1 does
# not exist), every rebase/merge, every linked worktree, every
# graph_fy_SKIP_HOOK=1 (#2986). Inside the subshell an `exit` ends only
# graph_fy's section; the detached rebuild launch is unaffected, and the
# hook's own exit status stays 0 as before.
_HOOK_SCRIPT = """\
# graph_fy-hook-start
# Auto-rebuilds the knowledge graph after each commit (code files only, no LLM needed).
# Installed by: graph_fy hook install
(
...
# churn run-to-run. Pinning it makes graph_fy_out reproducible.
...
# explicit graph_fy_MAX_WORKERS still wins for users who want parallelism.
if [ -n "${WINDIR:-}" ] || [ -n "${MSYSTEM:-}" ]; then
    export graph_fy_MAX_WORKERS="${graph_fy_MAX_WORKERS:-1}"
fi
...
# outputs are tracked in git). The dir is whatever graph_fy_OUT names, the same
# source the rebuild body reads (#1423): a literal graph_fy_out/ here let a
# commit touching only a renamed output dir's graph.json trigger a full rebuild.
_GFY_OUT="${graph_fy_OUT:-graph_fy_out}"
_GFY_OUT="${_GFY_OUT%/}"
...
""" + _PYTHON_DETECT + """
export graph_fy_CHANGED="$CHANGED"
...
_graph_fy_LOG="${HOME}/.cache/graph_fy-rebuild.log"
mkdir -p "$(dirname "$_graph_fy_LOG")"
export graph_fy_REBUILD_LOG="$_graph_fy_LOG"
echo "[graph_fy hook] launching background rebuild (log: $_graph_fy_LOG)"
...
# from graph_fy_OUT like the rebuild body does (#1423): a literal graph_fy_out/
...
# Honor the same opt-out as post-commit: without this, graph_fy_SKIP_HOOK=1
...
mkdir -p "$(dirname "$_graph_fy_LOG")"
export graph_fy_REBUILD_LOG="$_graph_fy_LOG"
echo "[graph_fy] Branch switched - launching background rebuild (log: $_graph_fy_LOG)"
""" + _detached_launch(_REBUILD_BODY_CHECKOUT) + """)
# graph_fy-checkout-hook-end
"""


def _load_graph_fyrc(root: Path) -> dict[str, str | int]:
    """Load key/value options from <root>/.graph_fyrc if present.

    Supported options:
      viz_node_limit: integer >= 0 (e.g. viz_node_limit=0)
    """
    rc_path = root / ".graph_fyrc"
...
def _hooks_dir(root: Path) -> Path:
    ...
    try:
        ...
        if res.returncode != 0:
            ...
            print(
                f"[graph_fy hooks] git could not resolve the hooks path for "
...
def _install_hook(
    ...
    an existing graph_fy block in-place."""
...
def _uninstall_hook(hooks_dir: Path, name: str, marker: str, marker_end: str) -> str:
    """Remove graph_fy section from a git hook using start/end markers."""
    hook_path = hooks_dir / name
    if not hook_path.exists():
        return f"no {name} hook found - nothing to remove."
    content = hook_path.read_text(encoding="utf-8")
    if marker not in content:
        return f"graph_fy hook not found in {name} - nothing to remove."
    new_content = re.sub(
    ...
    return f"graph_fy removed from {name} at {hook_path} (other hook content preserved)"
...
def _pinned_python() -> str:
    ...
    means callers must fall back to the `graph_fy` launcher on PATH — safe
    degradation.
    """
...
def _merge_attr_line() -> str:
    """The .gitattributes line assigning the graph_fy merge driver to graph.json.

    The graph lives under the configured output directory (graph_fy.paths,
    graph_fy_OUT env override). gitattributes patterns are repo-relative, so an
    absolute output-dir override cannot be expressed there — fall back to the
    default name in that case.
    """
    from graph_fy.paths import graph_fy_OUT
    out = graph_fy_OUT
    if not out or Path(out).is_absolute() or "\\" in out:
        out = "graph_fy_out"
    return f"{out.rstrip('/')}/graph.json merge=graph_fy"


def _has_merge_attr(content: str) -> bool:
    """True if a (non-comment) `<...>graph.json ... merge=graph_fy` line exists."""
    for raw in content.splitlines():
        line = raw.strip()
        if not line or line.startswith("#"):
            continue
        fields = line.split()
        if fields and fields[0].endswith("graph.json") and "merge=graph_fy" in fields[1:]:
            return True
    return False
...
def _register_merge_driver(root: Path) -> str:
    ...
    README and CHANGELOG 0.7.0 document `graph_fy merge-driver` as being set up
    ...
    same way the hook scripts pin it, so the driver works even when the graph_fy
    ...
    if pinned:
        ...
        driver = f'"{pinned}" -m graph_fy merge-driver %O %A %B'
    else:
        driver = "graph_fy merge-driver %O %A %B"
    try:
        for key, value in (
            ("merge.graph_fy.name", "graph_fy graph.json union merge"),
            ("merge.graph_fy.driver", driver),
        ):
            _sp.run(
...
def _unregister_merge_driver(root: Path) -> str:
    ...
    import subprocess as _sp
    for key in ("merge.graph_fy.name", "merge.graph_fy.driver"):
        try:
...
def _merge_driver_status(root: Path) -> str:
    ...
    try:
        res = _sp.run(
            ["git", "-C", str(root), "config", "--get", "merge.graph_fy.driver"],
            capture_output=True, text=True,
        )
...
def install(path: Path = Path(".")) -> str:
    """Install graph_fy post-commit and post-checkout hooks in the nearest git repo."""
    root = _git_root(path)
    if root is None:
    ...
    cfg = _load_graph_fyrc(root)
    viz_limit = cfg.get("viz_node_limit")
    if viz_limit is not None:
        # Use the `:-` default form (like graph_fy_MAX_WORKERS below) so an
        # explicit `graph_fy_VIZ_NODE_LIMIT=... git commit` still wins over the
        # baked project default — persisting config must not clobber a per-run
        # override.
        viz_export = f'export graph_fy_VIZ_NODE_LIMIT="${{graph_fy_VIZ_NODE_LIMIT:-{viz_limit}}}"\n'
    else:
        viz_export = ""
...
def uninstall(path: Path = Path(".")) -> str:
    """Remove graph_fy post-commit and post-checkout hooks."""
    root = _git_root(path)
    if root is None:
        raise RuntimeError(f"No git repository found at or above {path.resolve()}")
...
def status(path: Path = Path(".")) -> str:
    """Check if graph_fy hooks are installed."""
    root = _git_root(path)
    if root is None:
        return "Not in a git repository."
    hooks_dir = _user_hooks_dir(_hooks_dir(root))
    # status is a read-only diagnostic: a malformed .graph_fyrc must not turn it
    ...
    def _check(name: str, marker: str) -> str:
        ...
        if marker not in text:
            return "not installed (hook exists but graph_fy not found)"
        if cfg_limit is not None:
            # Baked as `"${graph_fy_VIZ_NODE_LIMIT:-<n>}"` so a per-run override
            # wins; match the default <n>, and still accept the older bare
            # `"<n>"` form from hooks installed before that change.
            m = re.search(
                r'export graph_fy_VIZ_NODE_LIMIT="(?:\$\{graph_fy_VIZ_NODE_LIMIT:-(\d+)\}|(\d+))"',
                text,
            )
