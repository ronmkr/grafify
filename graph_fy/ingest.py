...
from graph_fy.security import safe_fetch, safe_fetch_text, validate_url
...
def ingest(url: str, target_dir: Path, author: str | None = None, contributor: str | None = None) -> Path:
    """
    Fetch a URL and save it into target_dir as a graph_fy-ready file.
    ...
    try:
        if url_type in ("pdf", "image", "youtube"):
            raise ValueError(f"{url_type} ingestion is not supported in the local no-LLM build.")
...
def save_query_result(
    ...
    Files are stored in memory_dir (typically graph_fy_out/memory/) with YAML frontmatter
    that graph_fy's extractor reads as node metadata. This closes the feedback loop:
    ...
    signals: they are written both to the frontmatter (so `graph_fy reflect` can
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Fetch a URL into a graph_fy /raw folder")
    ...
    print(f"Ready for graph_fy: {out}")
