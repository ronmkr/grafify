# Gemini, and OpenAI.
# Used by `graph_fy extract . --backend gemini` and the benchmark scripts.
# The default graph_fy pipeline uses Claude Code subagents via skill.md;
...
from graph_fy.file_slice import (
    FileSlice,
    bisect_slice,
...
BACKENDS: dict[str, dict] = {
    ...
    "gemini": {
        ...
        "model_env_key": "graph_fy_GEMINI_MODEL",
    ...
    "openai": {
        ...
        # model. graph_fy_OPENAI_MODEL still wins over OPENAI_MODEL when both
        # are set (via model_env_key).
        "base_url": os.environ.get("OPENAI_BASE_URL", "https://api.openai.com/v1"),
        "default_model": os.environ.get("OPENAI_MODEL", "gpt-4.1-mini"),
        "env_key": "OPENAI_API_KEY",
        "model_env_key": "graph_fy_OPENAI_MODEL",
        "max_tokens": 16384,
        ...
        # automatically by _resolve_temperature; graph_fy_LLM_TEMPERATURE
    ...
    "deepseek": {
        ...
        "model_env_key": "graph_fy_DEEPSEEK_MODEL",
        ...
        # graph_fy_DISABLE_THINKING=1 to turn it off (tradeoff documented on the flag).
        "temperature": 0,
        "max_tokens": 16384,
    },
    "azure": {
        ...
        #           AZURE_OPENAI_DEPLOYMENT or graph_fy_AZURE_MODEL (deployment name).
        ...
        "default_model": os.environ.get("AZURE_OPENAI_DEPLOYMENT", os.environ.get("graph_fy_AZURE_MODEL", "gpt-4o")),
        "env_key": "AZURE_OPENAI_API_KEY",
        "model_env_key": "graph_fy_AZURE_MODEL",
    ...
    "bedrock": {
        ...
        "model_env_key": "graph_fy_BEDROCK_MODEL",
        "pricing": {"input": 3.0, "output": 15.0},  # USD per 1M tokens
        "temperature": 0,
        "max_tokens": 16384,
        "vision": True,
...
def _custom_providers_path(global_: bool = True) -> Path:
    if global_:
        return Path.home() / ".graph_fy" / "providers.json"
    return Path(".graph_fy") / "providers.json"
...
def provider_base_url_ok(base_url: str, name: str, *, warn: bool = True) -> bool:
    ...
    config is the graph_fy_ALLOW_LOCAL_PROVIDERS gate on project-local files.
    """
    ...
    try:
        ...
        if warn:
            print(f"[graph_fy] WARNING: provider {name!r} has an unparseable base_url; ignoring.", file=sys.stderr)
        return False
    if parsed.scheme not in ("http", "https"):
        if warn:
            print(
                f"[graph_fy] WARNING: provider {name!r} base_url scheme {parsed.scheme!r} is not "
                "http/https; ignoring.",
                file=sys.stderr,
            )
        return False
    ...
    if warn and parsed.scheme == "http" and not is_loopback:
        print(
            f"[graph_fy] WARNING: provider {name!r} sends your corpus to {host!r} over plaintext "
...
def _load_custom_providers() -> dict[str, dict]:
    # A project-local ./.graph_fy/providers.json travels with a cloned or shared
    ...
    # the user's own global ~/.graph_fy/providers.json stays trusted.
    local_path = _custom_providers_path(global_=False)
    global_path = _custom_providers_path(global_=True)
    allow_local = os.environ.get("graph_fy_ALLOW_LOCAL_PROVIDERS", "").strip().lower() in ("1", "true", "yes")
    if local_path.is_file() and not allow_local:
        print(
            f"[graph_fy] WARNING: ignoring project-local {local_path} (custom providers control "
            "where your corpus and API key are sent). Set graph_fy_ALLOW_LOCAL_PROVIDERS=1 to load it.",
            file=sys.stderr,
        )
...
def _resolve_max_tokens(default: int) -> int:
    """Honour graph_fy_MAX_OUTPUT_TOKENS env var override, else use backend default."""
    raw = os.environ.get("graph_fy_MAX_OUTPUT_TOKENS", "").strip()
    if raw:
        try:
            v = int(raw)
            if v > 0:
...
def _resolve_temperature(default: float | None, model: str = "") -> float | None:
    """Resolve the temperature to send, honouring graph_fy_LLM_TEMPERATURE.

    Precedence (issue #1191):
      1. graph_fy_LLM_TEMPERATURE env var, if set:
    ...
    """
    raw = os.environ.get("graph_fy_LLM_TEMPERATURE", "").strip()
    if raw:
        ...
        try:
            ...
            print(
                f"[graph_fy] graph_fy_LLM_TEMPERATURE={raw!r} is not a number or "
...
def _bedrock_inference_config(max_tokens: int, model: str = "") -> dict:
    """Build Bedrock inferenceConfig, honouring graph_fy_LLM_TEMPERATURE.
...
def _resolve_api_timeout(default: float = 600.0) -> float:
    """Honour graph_fy_API_TIMEOUT env var override, else use default (seconds)."""
    raw = os.environ.get("graph_fy_API_TIMEOUT", "").strip()
    if raw:
        try:
            v = float(raw)
            if v > 0:
...
def _resolve_max_retries(default: int = 6) -> int:
    ...
    Honour graph_fy_MAX_RETRIES; 0 is allowed (disable retries)."""
    raw = os.environ.get("graph_fy_MAX_RETRIES", "").strip()
    if raw:
        try:
            v = int(raw)
            if v >= 0:
...
def _resolve_max_retry_depth(default: int = 3) -> int:
    ...
    with no way for a `graph_fy extract` operator to lower it — or set it to 0 —
    as a mitigation (#2880). Honour graph_fy_MAX_RETRY_DEPTH.
    ...
    """
    raw = os.environ.get("graph_fy_MAX_RETRY_DEPTH", "").strip()
    if raw:
        try:
            v = int(raw)
            if v >= 0:
...
def _thinking_disabled_via_env() -> bool:
    """Opt-in (graph_fy_DISABLE_THINKING) to send ``{"thinking": {"type": "disabled"}}``
    ...
    return os.environ.get("graph_fy_DISABLE_THINKING", "").strip().lower() in ("1", "true", "yes", "on")

_EXTRACTION_SYSTEM = """\
You are a graph_fy semantic extraction agent. Extract a knowledge graph fragment from the files provided.
...
def _file_to_text(path: Path) -> str:
    ...
    if path.suffix.lower() == ".pdf":
        from graph_fy.detect import extract_pdf_text
    ...
    **parent file path** as ``rel`` so every slice of a file shares one
    ...
    for u in units:
        ...
        if safe_path is None:
            print(f"[graph_fy] skipping {p}: symlink target outside corpus root", file=sys.stderr)
            continue
        try:
...
def _build_image_refs(image_files: list[Path], root: Path, *, read_bytes: bool = True) -> list[_ImageRef]:
    ...
    for p in image_files:
        ...
        if abs_path is None:
            print(f"[graph_fy] skipping image {p}: symlink target outside corpus root", file=sys.stderr)
            continue
        try:
            # as_posix, not str: `rel` is handed to the model as the literal
            # source_file to emit, so a native backslash spelling on Windows
        ...
        if read_bytes:
            try:
                ...
                print(f"[graph_fy] could not read image {rel}: {exc}", file=sys.stderr)
                raw = None
            if raw is not None and len(raw) > _MAX_IMAGE_BYTES:
                print(
                    f"[graph_fy] image {rel} is {len(raw) // 1024} KB, over the "
...
def _backend_supports_vision(backend: str) -> bool:
    ...
    opt-in via graph_fy_OLLAMA_VISION=1 once the user selects a vision model
    (e.g. --model llama3.2-vision).
    """
    if backend == "ollama":
        return os.environ.get("graph_fy_OLLAMA_VISION", "").strip() == "1"
...
def _sanitize_fragment(parsed: dict) -> dict:
    ...
    if hyperedges:
        from graph_fy.build import _coerce_hyperedge_member_refs
...
def _parse_llm_json(raw: str) -> dict:
    ...
    if len(raw) > _LLM_JSON_MAX_BYTES:
        print(
            f"[graph_fy] LLM response exceeds {_LLM_JSON_MAX_BYTES} bytes "
    ...
    print(
        f"[graph_fy] LLM returned invalid JSON, skipping chunk "
...
def _mark_hollow(result: dict, raw_content: str | None, backend: str | None) -> dict:
    ...
    *same* chunk with backoff instead.
    ...
    if _response_is_hollow(raw_content, result) and result.get("finish_reason") != "length":
        print(
            f"[graph_fy] {backend or 'backend'} returned a hollow response "
...
def _backend_pkg_hint(pkg: str, extra: str) -> str:
    ...
    `uv tool install graph_fyy` puts graph_fy in an isolated venv, so a plain
    ...
    return (
        ...
        f"Install it with:  uv tool install \"graph_fyy[{extra}]\" --force  "
...
def _call_openai_compat(
    ...
    *,
    ...
    # default. Honour graph_fy_API_TIMEOUT (seconds) for explicit override;
    ...
    # graph_fy_MAX_RETRIES still wins for users who want it.
    _retries = _resolve_max_retries()
    if backend == "ollama" and not os.environ.get("graph_fy_MAX_RETRIES", "").strip():
        _retries = 0
    ...
    if backend == "ollama" and extra_body is None:
        num_ctx_raw = os.environ.get("graph_fy_OLLAMA_NUM_CTX", "").strip()
        ...
        if num_ctx_raw:
            try:
                ...
                print(
                    f"[graph_fy] graph_fy_OLLAMA_NUM_CTX={num_ctx_raw!r} is not a valid integer; "
                ...
                if num_ctx < estimated_input:
                    print(
                        f"[graph_fy] warning: graph_fy_OLLAMA_NUM_CTX={num_ctx} is smaller than "
            ...
            num_ctx = auto_num_ctx
        keep_alive = os.environ.get("graph_fy_OLLAMA_KEEP_ALIVE", "30m")
    ...
    if output_tokens < 50 and backend == "ollama":
        print(
            "[graph_fy] warning: ollama returned very few tokens — likely causes: "
            "(1) VRAM pressure: check `nvidia-smi` and reduce chunk size with "
            "--token-budget (e.g. --token-budget 4096) or set "
            "graph_fy_OLLAMA_NUM_CTX to a smaller value; "
...
# A JSON Schema pinning the top-level shape graph_fy consumes. Passed to
...
def _claude_cli_supports_json_schema(claude_cmd: str) -> bool:
    ...
    reliable than guessing a version boundary — so graph_fy uses structured
...
def _call_claude_cli(user_message: str, max_tokens: int = 8192, *, deep_mode: bool = False, images: list[_ImageRef] | None = None) -> dict:
    ...
    a pay-as-you-go API key just to run graph_fy's semantic pass.
    ...
    # extraction graph_fy performs. graph_fy_CLAUDE_CLI_MODEL=haiku (or
    ...
    cli_model = os.environ.get("graph_fy_CLAUDE_CLI_MODEL", "").strip()
    if cli_model:
...
def _azure_client(api_key: str, endpoint: str):
    ...
    timeout_raw = os.environ.get("graph_fy_API_TIMEOUT", "").strip()
...
def _call_bedrock(model: str, user_message: str, max_tokens: int = 8192, *, deep_mode: bool = False, images: list[_ImageRef] | None = None) -> dict:
    ...
    try:
        ...
        raise ImportError(
            "AWS Bedrock extraction requires boto3. Run: pip install graph_fyy[bedrock]"
        ) from exc

    region = os.environ.get("AWS_REGION") or os.environ.get("AWS_DEFAULT_REGION") or "us-east-1"
    profile = os.environ.get("AWS_PROFILE")
    session = boto3.Session(profile_name=profile, region_name=region)
    # Wire graph_fy_API_TIMEOUT into the botocore read timeout. Without an
...
def extract_files_direct(
    ...
    *,
    ...
    if not key and backend == "ollama":
        ...
        print(
            "[graph_fy] WARNING: ollama backend selected with no OLLAMA_API_KEY set; "
    ...
    if isinstance(result, dict):
        try:
            ...
            if _n_unverified:
                print(
                    f"[graph_fy] {_n_unverified} semantic node(s) had no evidence in "
                    "the source and were flagged verification=unverified",
                    file=sys.stderr,
                )
        except Exception as _exc:  # noqa: BLE001 — evidence-binding is advisory
            print(f"[graph_fy] evidence-binding skipped: {_exc}", file=sys.stderr)
    return result
...
def _extract_with_adaptive_retry(
    ...
    *,
    ...
    - recognized timeout exceptions — dense chunks can take long enough to hit
      `graph_fy_API_TIMEOUT` before returning output. For `claude-cli`,
    ...
    try:
        ...
        for _delay in (_HOLLOW_BACKOFF_S if max_depth > 0 else ()):
            if result.get("finish_reason") != "hollow":
                break
            print(
                f"[graph_fy] retrying the same chunk of {len(chunk)} in {_delay:g}s "
        ...
        if len(chunk) <= 1:
            ...
            if halves is not None:
                print(
                    f"[graph_fy] slice of {unit_path(chunk[0])} {reason} at "
            ...
            print(
                f"[graph_fy] single-file chunk {unit_path(chunk[0])} {fail_desc} "
        ...
        if _depth >= max_depth:
            ...
            print(
                f"[graph_fy] chunk of {len(chunk)} {persist_desc} at "
                f"recursion depth {_depth} (max {max_depth}) — dropping",
                file=sys.stderr,
            )
            return {"nodes": [], "edges": [], "hyperedges": [], "input_tokens": 0, "output_tokens": 0, "model": model, "finish_reason": "stop"}
        print(
            f"[graph_fy] chunk of {len(chunk)} {reason} at depth "
    ...
    if result.get("finish_reason") == "hollow":
        ...
        print(
            f"[graph_fy] chunk of {len(chunk)} still hollow after "
    ...
    if len(chunk) <= 1:
        ...
        if halves is not None:
            print(
                f"[graph_fy] slice of {unit_path(chunk[0])} truncated at depth {_depth}; "
                f"splitting the slice and retrying",
                file=sys.stderr,
            )
            return _merge_two([halves[0]], [halves[1]])
        print(
            f"[graph_fy] single-file chunk {unit_path(chunk[0])} truncated at "
    ...
    if _depth >= max_depth:
        print(
            f"[graph_fy] chunk of {len(chunk)} still truncated at recursion "
    ...
    print(
        f"[graph_fy] chunk of {len(chunk)} truncated at depth {_depth}, "
...
def extract_corpus_parallel(
    ...
    Adaptive retry on truncation:
        - When the LLM returns `finish_reason="length"` (output truncated at
          ...
          the default, overridable by graph_fy_MAX_RETRY_DEPTH so an operator
    ...
    looks, instead of creating an unwanted ``graph_fy_out/`` inside the
    ...
    if backend == "ollama" and os.environ.get("graph_fy_OLLAMA_PARALLEL", "").strip() != "1":
        max_concurrency = 1
    ...
    if backend == "claude-cli" and os.environ.get("graph_fy_CLAUDE_CLI_PARALLEL", "").strip() != "1":
        max_concurrency = 1
    def _checkpoint_chunk(result: dict, chunk: "list[Path | FileSlice]") -> None:
        ...
        if os.environ.get("graph_fy_NO_INCREMENTAL_CACHE"):
            return
        try:
            ...
            print(f"[graph_fy] incremental cache checkpoint failed: {_exc}", file=sys.stderr)
    ...
    if workers == 1:
        ...
        for idx, chunk in enumerate(chunks):
            ...
            if exc is not None:
                print(f"[graph_fy] chunk {idx + 1}/{total} failed: {exc}", file=sys.stderr)
        ...
        with ThreadPoolExecutor(max_workers=workers) as pool:
            ...
            for future in as_completed(futures):
                ...
                if exc is not None:
                    print(
                        f"[graph_fy] chunk {idx + 1}/{total} failed: {exc}",
                        file=sys.stderr,
                    )
                    merged["failed_chunks"] += 1
                    continue
    ...
    if merged["failed_chunks"] > 0:
        print(
            f"[graph_fy] WARNING: {merged['failed_chunks']}/{total} semantic chunk(s) failed"
    ...
    if dropped_node_count:
        ...
        print(
            f"[graph_fy] WARNING: dropped {dropped_node_count} out-of-scope node(s) "
    ...
    if uncovered:
        ...
        print(
            f"[graph_fy] WARNING: {len(uncovered)}/{len(dispatched)} dispatched file(s) "
...
def _call_llm(
    ...
    *,
    ...
    Used by lightweight callers (e.g. `graph_fy.dedup` LLM tiebreaker) that
    ...
    Previously `graph_fy.dedup` imported a `_call_llm` symbol that did not
...
def _ollama_host_is_link_local_or_metadata(host: str) -> bool:
    ...
    import ipaddress
    import socket
    if host in ("metadata.google.internal", "metadata.google.com", "0.0.0.0", "::", "[::]"):  # nosec: B104
        return True
...
def _validate_ollama_base_url(url: str, *, warn: bool = True) -> None:
    ...
    try:
        ...
        if warn:
            print(
                f"[graph_fy] WARNING: OLLAMA_BASE_URL={url!r} is not a parseable URL.",
                file=sys.stderr,
            )
        return
    if parsed.scheme not in ("http", "https"):
        if warn:
            print(
                f"[graph_fy] WARNING: OLLAMA_BASE_URL has unexpected scheme {parsed.scheme!r}; "
    ...
    if warn and not is_loopback:
        ...
        print(
            f"[graph_fy] WARNING: OLLAMA_BASE_URL points to non-loopback host {host!r}{scheme_note}. "
...
def _claude_cli_available() -> bool:
    ...
    if os.environ.get("graph_fy_NO_CLAUDE_CLI") or os.environ.get("PYTEST_CURRENT_TEST"):
        return False
    if platform.system() == "Windows":
...
# ── Community labeling ────────────────────────────────────────────────────────
# When graph_fy runs inside an orchestrating agent (Claude Code / Gemini CLI),
...
# graph_fy is run as a bare CLI (``graph_fy extract . --backend X``), there is no
    ...
    *,
    ...
    try:
        ...
        if len(batch_cids) <= 1 or depth >= max_depth:
            print(
                f"[graph_fy label] batch of {len(batch_cids)} still unparseable "
    ...
    *,
    ...
    def _merge(batch_idx: int, parsed, exc, batch_usage=None) -> None:
        ...
        if exc is not None:
            ...
            print(
                f"[graph_fy label] batch {batch_idx + 1}/{n_batches} "
...
def generate_community_labels(
    ...
    *,
    ...
    if not backend:
        if not quiet:
            print(
                "[graph_fy label] no LLM backend configured; keeping Community N "
    ...
    try:
        ...
        if not quiet:
            print(
                f"[graph_fy label] warning: community labeling failed ({exc}); "
