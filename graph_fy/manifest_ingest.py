...
per package -- keyed by NAME via :func:`graph_fy.ids.make_id` -- plus
...
_TOMLI_REQUIRED = (
    ...
    "(or reinstall graph_fyy, which declares tomli for python_version < '3.11')."
)
...
def _parse_package_json(text: str) -> dict | None:
    import json
    try:
    ...
    for key in ("dependencies", "devDependencies", "peerDependencies"):
        ...
        if isinstance(d, dict):
            deps.extend(str(k) for k in d.keys())
    return {"name": str(name), "version": data.get("version"), "deps": list(dict.fromkeys(deps))}


def _parse_requirements_txt(text: str) -> dict | None:
    deps: list[str] = []
    for line in text.splitlines():
        s = line.strip()
    ...
    if not deps:
        return None
    return {"name": "requirements", "version": None, "deps": list(dict.fromkeys(deps))}


def _parse_poetry_lock(text: str) -> dict | None:
    _toml = _load_toml_module()
    data = _toml.loads(text)
    ...
    if isinstance(pkgs, list):
        for p in pkgs:
            if isinstance(p, dict) and "name" in p:
                deps.append(str(p["name"]))
    return {"name": "poetry.lock", "version": None, "deps": list(dict.fromkeys(deps))}
