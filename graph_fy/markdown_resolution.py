...
from graph_fy.extractors.base import _LANGUAGE_BUILTIN_GLOBALS

MARKDOWN_MENTION_SUFFIXES = frozenset({".md", ".mdx", ".qmd", ".skill", ".txt", ".rst", ".markdown", ".pdf"})
...
def resolve_markdown_mentions(
    ...
    for rc in mentions:
        ...
        if cited:
            ...
            if "." in callee and not rc.get("qualifiers"):
                parts = [p.strip() for p in callee.split(".") if p.strip()]
                if parts:
                    names = parts
                    callee = parts[-1]
            if callee in _LANGUAGE_BUILTIN_GLOBALS:
                continue
            if len(names) > 1:
                leaf_candidates = list(by_label.get(names[-1], []))
                for depth, owner_name in enumerate(reversed(names[:-1]), start=1):
                    owners = set(by_label.get(owner_name, []))
                    narrowed = []
                    for c in leaf_candidates:
                        ...
                        if chain & owners:
                            narrowed.append(c)
                    leaf_candidates = narrowed
                target = leaf_candidates[0] if len(leaf_candidates) == 1 else None
            else:
