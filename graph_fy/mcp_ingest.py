...
and turns the `mcpServers` map into graph_fy nodes and edges.

Symmetry with `serve.py`: graph_fy exposes itself AS an MCP server. This module
indexes MCP servers AS a corpus type, completing the loop — an agent that runs
graph_fy with `--mcp` can now query its own configured MCP layer.

Entry point:
  extract_mcp_config(path: Path) -> dict[str, list[dict]]

  Returns `{"nodes": [...], "edges": [...]}` compatible with graph_fy's
...
def extract_mcp_config(path: Path) -> dict[str, Any]:
    """Parse an MCP config file into graph_fy nodes and edges.
...
# ── Node / edge construction (graph_fy schema) ────────────────────────────────
...
# only graph_fy.ids), so this can no longer drift from extract._file_stem.
from graph_fy.extractors.base import _file_stem  # noqa: E402
