"""Single source of truth for the graph_fy output-directory name.

The output directory is ``graph_fy_out`` by default and overridable with the
``graph_fy_OUT`` env var (worktrees or shared-output setups, #686). It accepts a
relative name (``"graph_fy_out-feature"``) or an absolute path
(``"/shared/graph_fy_out"``).

This used to be duplicated as an identical ``_graph_fy_OUT`` constant in
``__main__``, ``cache``, and ``watch``, while ``security`` and ``callflow_html``
hardcoded the literal ``"graph_fy_out"`` and silently ignored the override
...
``graph_fy_OUT`` before the process starts (the normal worktree/shared-output
...
# Bare directory name even when graph_fy_OUT is an absolute path. Used by path
# guards that walk parents looking for the output directory by name.
graph_fy_OUT_NAME = os.path.basename(os.path.normpath(graph_fy_OUT))


def out_path(*parts: str) -> Path:
    """A path inside the configured output dir, e.g. ``out_path("cache")``.

    ``Path(graph_fy_OUT) / ...`` resolves correctly for both a relative name
    ("graph_fy_out") and an absolute override ("/shared/graph_fy_out").
    Falls back to legacy "graph_fy_out" if graph_fy_out does not exist.
    """
    p = Path(graph_fy_OUT, *parts)
    if not p.exists() and graph_fy_OUT == "graph_fy_out":
        legacy = Path("graph_fy_out", *parts)
        if legacy.exists():
            return legacy
    return p
...
def default_graph_json() -> str:
    ...
    commands so a ``graph_fy_OUT`` override is honoured everywhere, not just where
    the path is passed explicitly (#1423).
    """
    target = out_path("graph.json")
    if not target.exists() and Path("graph_fy_out", "graph.json").exists():
        return str(Path("graph_fy_out", "graph.json"))
    return str(target)
...
def stem_filename_budget(output_dir: "str | Path", *, reserve: int = 0, limit: int = 200) -> int:
    ...
    ``C:\\Users\\me\\projects\\svc\\graph_fy_out\\obsidian`` exceeds MAX_PATH and
...
def load_node_link_graph(path_or_data):
    """Load a graph_fy graph.json into a networkx graph, accepting both writers.
    ...
    if not isinstance(data, dict):
        p = Path(data)
        from graph_fy.security import check_graph_file_size_cap  # lazy: security imports paths
