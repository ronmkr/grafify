"""graph_fy prs — graph-aware PR dashboard.
...
Usage:
  graph_fy prs                   # dashboard of all open PRs
  graph_fy prs <number>          # deep dive on one PR
  graph_fy prs --triage          # Opus ranks your review queue
  graph_fy prs --worktrees       # show worktree → branch → PR mapping
  graph_fy prs --conflicts       # PRs sharing graph communities (merge-order risk)
  graph_fy prs --base <branch>   # filter to PRs targeting this base (default: v8)
"""
...
def render_dashboard(prs: list[PRInfo], base: str = "v8", show_wrong_base: bool = False) -> None:
    ...
    print()
    print(bold(f"  graph_fy prs  ·  base: {base}  ·  {len(actionable)} PRs"))
    print()
...
def triage_with_opus(prs: list[PRInfo], base: str) -> None:
    print("  LLM triage is disabled in local no-LLM build.")
    return
