"""Deterministic "work memory" reflection over graph_fy_out/memory/.

`graph_fy reflect` reads the Q&A memory docs that `graph_fy save-result` files back
...
and a given ``now``. When a graph (`graph.json` + `.graph_fy_analysis.json`) is available
the lessons are also grouped by community label; without it they degrade to a single
flat section.

The artifact lands at ``graph_fy_out/reflections/LESSONS.md`` rather than inside the wiki
because ``graph_fy export wiki`` deletes every ``wiki/*.md`` on each run — a lessons file
...
from graph_fy.ingest import OUTCOMES
...
LEARNING_SIDECAR_NAME = ".graph_fy_learning.json"
...
def _load_node_community(graph_path: Path, analysis_path: Path,
    ...
    Mirrors how `graph_fy export wiki` reads graph.json + .graph_fy_analysis.json +
    .graph_fy_labels.json. Community membership in the analysis sidecar is keyed by
...
def render_lessons_md(agg: dict[str, Any]) -> str:
    ...
    out: list[str] = [
        "# Lessons",
        "",
        f"_Auto-generated by `graph_fy reflect` from {agg['total']} session "
        f"{'memory' if agg['total'] == 1 else 'memories'} in graph_fy_out/memory/. "
...
def lessons_fresh(out_path: Path, memory_dir: Path,
    ...
    Lets ``graph_fy reflect --if-stale`` skip a redundant run — e.g. when the git
...
def reflect(memory_dir: Path, out_path: Path,
    ...
    if graph_path is not None:
        ...
        analysis_path = Path(analysis_path) if analysis_path else (
            graph_path.parent / ".graph_fy_analysis.json")
        labels_path = Path(labels_path) if labels_path else (
            graph_path.parent / ".graph_fy_labels.json")
...
# graph.json as ``.graph_fy_learning.json``. It carries which nodes have proven
...
def _resolve_source_path(src: str, graph_path: Path) -> Path | None:
    ...
    live in ``<root>/graph_fy_out/`` (so its own dir is not the root) or directly
    ...
    Order: the committed ``.graph_fy_root`` marker (#686/#1423 — authoritative for
    an absolute/elsewhere ``graph_fy_OUT`` override); then the layout-appropriate
    root *first* — graph.json's parent's parent for the ``graph_fy_out`` layout,
    ...
    try:
        recorded = (out_dir / ".graph_fy_root").read_text(encoding="utf-8-sig").strip()
        if recorded:
    ...
    if out_dir.name == graph_fy_OUT_NAME:
...
def write_learning_sidecar(agg: dict[str, Any], graph_path: Path,
    ...
    """Write ``.graph_fy_learning.json`` next to ``graph_path`` deterministically.
