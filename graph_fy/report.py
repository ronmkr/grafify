...
def _portable_root_label(root: str) -> str:
    ...
    filesystem, and makes `graph_fy update .`, `graph_fy update ./proj`, and
    `graph_fy update /abs/path/proj` all label the header `proj`. Only the
...
def load_learning_for_report(graph_path) -> dict | None:
    ...
    Reads the ``.graph_fy_learning.json`` overlay (preferred sources) next to
    ...
    try:
        gp = _Path(graph_path)
        from graph_fy.reflect import load_learning_overlay, load_memory_docs, aggregate_lessons
...
def generate(
    ...
    if built_at_commit:
        lines += [
            ...
            "- Run `graph_fy update .` after code changes (no API cost).",
        ]
    ...
    if gap_count > 0 or amb_pct > 20:
        ...
        if thin_communities:
            lines.append(f"- **{len(thin_communities)} thin communities (<{min_community_size} nodes) omitted from report** — run `graph_fy query` to explore isolated nodes.")
        if amb_pct > 20:
    ...
    # Preferred sources come from the .graph_fy_learning.json sidecar; the
