...
graph_fy nodes and edges. NOT a full SCIP protobuf implementation —
...
def ingest_scip_json(
    ...
    """Convert a SCIP-style JSON document into graph_fy nodes and edges.
...
def _scip_relation_for(rel: dict[str, Any]) -> str:
    """Pick the graph_fy relation tag for a SCIP relationship dict.
...
def _make_scip_node_id(symbol: str, source_file: str) -> str:
    """Derive a stable graph_fy node ID from a SCIP symbol identifier.
...
def _scip_kind_to_file_type(kind: str) -> str:
    """Map SCIP symbol kind to a graph_fy file_type."""
