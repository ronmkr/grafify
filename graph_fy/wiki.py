...
from graph_fy.paths import stem_filename_budget
...
# and resolves fine on every filesystem graph_fy targets; stripping it would
...
def _community_article(
    ...
    lines += ["---", "", f"*Part of the graph_fy knowledge wiki. See {_md_link('index', resolver)} to navigate.*"]
    return "\n".join(lines)
...
def _index_md(
    ...
    lines: list[str] = [
        ...
        "",
        "> Auto-generated by graph_fy. Start here — read community articles for context, then drill into god nodes for detail.",
        "",
    ...
    lines += [
        "---",
        "",
        "*Generated by [graph_fy](https://github.com/safishamsi/graph_fy)*",
    ]
    return "\n".join(lines)
...
def to_wiki(
    ...
    if not communities:
        raise ValueError(
            ...
            "Run `graph_fy extract .` or `graph_fy cluster-only .` first."
        )
    ...
    if not communities:
        raise ValueError(
            ...
            "Re-run `graph_fy extract .` to regenerate .graph_fy_analysis.json."
        )
    ...
    # Windows a 200-char slug under an ordinary graph_fy_out/wiki/ overruns
