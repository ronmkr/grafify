...
def dispatch_command(cmd: str) -> None:
    if cmd == "provider":
        print("[graphify] LLM providers are disabled in this fully local, zero-model build.", file=sys.stderr)
        sys.exit(0)
    elif cmd == "prs":
        ...
        if len(sys.argv) < 3:
            print("Usage: graphify query \"<text>\" [--limit N] [--hops N] [--dfs] [--context C] [--budget N] [--graph path] [--terse] [--caveman]", file=sys.stderr)
            sys.exit(1)
        ...
        _is_legacy_text = bool(context_filters or use_dfs or budget != 2000 or force_text or "who calls " in question.lower())
        if _is_legacy_text:
            ...
            if terse:
                from graphify.retrieve import format_query_results
                print(format_query_results(_result, terse=True))
            else:
                print(_result)
            if verbose:
                try:
                    from graphify.mcp import estimate_token_savings
                    nodes_data = []
                    ...
                    root_dir = str(gp.parent.parent) if gp.parent.name == "graphify-out" else str(gp.parent)
            ...
            from graphify.retrieval import query as _retrieval_query
            idx_path = gp.parent / "index.db"
            results = _retrieval_query(question, graph=G, db_path=idx_path, limit=limit, hops=hops, graph_path=gp)
            querylog.log_query(
                kind="query",
                question=question,
                corpus=str(gp),
            ...
            if verbose:
                try:
                    ...
                    if terse:
                        from graphify.retrieve import format_query_results
                        print(format_query_results(results, terse=True))
                    else:
        ...
        if len(sys.argv) < 3:
            print("Usage: graphify impact <target> [--depth N] [--json] [--terse] [--graph path]", file=sys.stderr)
            sys.exit(1)
        ...
        if len(sys.argv) < 3:
            print("Usage: graphify flow <entry_point> [--sink <sink>] [--json] [--graph <path>] [--max-depth N]", file=sys.stderr)
            sys.exit(1)
        from graphify.affected import load_graph
        ...
        if len(sys.argv) < 4:
            print(
                f'Usage: graphify {cmd} "<source>" "<target>" [--graph path] '
                "[--directed|--undirected]",
                file=sys.stderr,
            )
            sys.exit(1)
        ...
        if len(sys.argv) < 3:
            print('Usage: graphify explain "<node>" [--graph path] [--terse] [--caveman]', file=sys.stderr)
            sys.exit(1)
        ...
        if len(sys.argv) > 2 and not sys.argv[2].startswith("-"):
            ...
            if arg_p.is_dir():
                cand = arg_p / "graphify-out" / "GRAPH_REPORT.md"
                report_path = cand if cand.exists() else arg_p / "GRAPH_REPORT.md"
            else:
                report_path = arg_p
        if not report_path.exists():
            print(f"Error: report file not found at {report_path}. Run `graphify build` first.", file=sys.stderr)
            sys.exit(1)
        ...
        if not graph_path.exists():
            print(f"error: graph file not found at {graph_path}. Run `graphify build` first.", file=sys.stderr)
            sys.exit(1)
        ...
        if json_output:
            ...
            print(f"Graphify Lint: {result['status'].upper()}")
        ...
        if incremental_mode:
            ...
            print(
                f"[graphify {cmd}] {len(code_files)} code and doc file(s) changed; "
                f"{unchanged_total} unchanged; {len(deleted_files)} deleted"
                f"{_excl_note}"
            )
        else:
            print(
                f"[graphify {cmd}] found {len(code_files)} code and doc file(s)"
            )
        ...
        if no_cluster:
            ...
            stages.mark("write")
            try:
                from graphify.build import build as _build_bare
                _bare_G = _build_bare([merged], root=target)
                from graphify.retrieval import build_index as _build_fts_index
                _build_fts_index(_bare_G, graphify_out / "index.db")
                print(f"[graphify extract] wrote {graphify_out / 'index.db'} (FTS5 search index)")
            except Exception as _idx_err:
                print(f"[graphify extract] warning: could not build FTS5 index: {_idx_err}", file=sys.stderr)
            cost = _estimate_cost(
        ...
        from graphify.cluster import label_communities_by_hub as _label_by_hub
        ...
        try:
            from graphify.retrieval import build_index as _build_fts_index
            _build_fts_index(G, graphify_out / "index.db")
            print(f"[graphify extract] wrote {graphify_out / 'index.db'} (FTS5 search index)")
        except Exception as _idx_err:
            print(f"[graphify extract] warning: could not build FTS5 index: {_idx_err}", file=sys.stderr)
        ...
        try:
            from graphify.report import generate as _generate_report, load_learning_for_report as _llfr
            ...
            print(f"[graphify extract] warning: could not generate GRAPH_REPORT.md: {_rep_err}", file=sys.stderr)
        ...
        if not ("--no-viz" in sys.argv):
            ...
            try:
                ...
                print(f"[graphify extract] warning: could not write {html_target}: {_viz_err}", file=sys.stderr)
