...
def _skill_registration(skill_path: str = "~/.claude/skills/graphify/SKILL.md") -> str:
    return (
        ...
        "- code knowledge graph and retrieval layer. Trigger: `/graphify`\n"
        "When graphify-out/ exists in the project root, refer to it to understand the codebase without fragmentation (code is the source of truth):\n"
        '- Run `graphify query "<question>"` for codebase questions before reading raw files.\n'
        "- Check graphify-out/GRAPH_REPORT.md and graphify-out/wiki/index.md for overall architecture and community structure.\n"
