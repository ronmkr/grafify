...
def patch_graph_in_memory(
    ...
    *,
    ...
    out = Path(out_dir).resolve() if out_dir is not None else (root_path / _GRAPHIFY_OUT)
    ...
    from graphify.build import _abs_identity, _norm_source_file
    ...
    try:
        from graphify.retrieval import build_index
    ...
    *,
    ...
    if initial_graph is not None:
        ...
        try:
            ...
            print(f"[graphify watch] Loaded in-memory graph: {len(G)} nodes, {G.number_of_edges()} edges")
        except Exception:
        ...
        print("[graphify watch] Initializing graph...")
        _rebuild_code(watch_path)
        if existing_graph.exists():
            data = json.loads(existing_graph.read_text(encoding="utf-8"))
            from graphify.build import build_from_json
    ...
    print(f"[graphify watch --hot] Watching {watch_root} (Hot in-memory patch mode)")
    print("[graphify watch --hot] Code changes patch graph & re-index in <100ms.")
    print(f"[graphify watch --hot] Debounce: {debounce}s")
    ...
    try:
        while True:
            ...
            if pending and (time.monotonic() - last_trigger) >= debounce:
                ...
                batch = list(changed)
                changed.clear()
                print(f"\n[graphify watch --hot] {len(batch)} file(s) changed")
                if _batch_triggers_rebuild(batch):
                    ...
                    if code_batch:
                        ...
                        print(
                            f"[graphify watch --hot] Patched in {latency_ms:.1f}ms: "
