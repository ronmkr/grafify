...
Verifies:
1. `graph_fy build` completes successfully on a folder containing:
   - Helm charts
   - K8s manifests
...
2. `graph_fy lint` correctly flags deliberately introduced drift/lint issues:
...
from graph_fy.extract import extract, collect_files
...
def block_network(monkeypatch):
    ...
    def _guarded_connect(*args, **kwargs):
        raise RuntimeError("Network blocked: unexpected outbound network call during local graph_fy run")
