...
_ARCHITECTURE = Path(__file__).parent.parent / "docs" / "ARCHITECTURE.md"
...
def _documented_symbols() -> list[tuple[str, str]]:
    ...
    for module, cell in _ROW.findall(text[start:end]):
        for func in _FUNC.findall(cell):
            pairs.append((f"graph_fy.{module}", func))
    return pairs
...
def test_the_table_was_actually_parsed():
    ...
    assert {"graph_fy.extract", "graph_fy.build", "graph_fy.serve"} <= modules, modules
