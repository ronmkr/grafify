...
def test_detection_tests_pass_with_every_backend_variable_exported():
    ...
    target_files = [f for f in ["tests/test_ollama.py", "tests/test_provider_registry.py", "tests/test_llm_backends.py"] if (ROOT / f).exists()]
    if not target_files:
        target_files = ["tests/test_backend_env_isolation.py"]
