...
from graph_fy.build import build_from_json, build, build_merge, edge_data, edge_datas, dedupe_edges, dedupe_nodes
...
def test_legacy_node_name_path_aliases_folded():
    ...
    from graph_fy.validate import validate_extraction
...
def test_ghost_merge_wrong_source_file_resolved_by_label():
    ...
    graph_fy_out/memory/*.md query answer, a runbook narrating "see App.tsx")
    ...
    ext = {
        "nodes": [
            ...
            {"id": "app", "label": "App.tsx", "file_type": "code",
             "source_file": "graph_fy_out/memory/query_fake.md", "_origin": "semantic"},
        ...
        "edges": [
            {"source": "some_query_node", "target": "app", "relation": "references",
             "confidence": "EXTRACTED", "source_file": "graph_fy_out/memory/query_fake.md"},
    ...
    ext = {
        "nodes": [
            ...
            {"id": "phantom_index", "label": "index.ts", "file_type": "code",
             "source_file": "graph_fy_out/memory/query_fake2.md", "_origin": "semantic"},
        ],
        "edges": [],
    }
    G = build_from_json(ext, directed=False)
...
def test_build_from_json_ambiguous_old_stem_alias_stays_dangling(tmp_path):
    ...
    extraction = {
        "nodes": [
            ...
            # form, matching what a real graph_fy update run would already have
...
def test_merge_raw_extraction_tier_scoped(tmp_path):
    ...
    from graph_fy.build import merge_raw_extraction
...
def test_graph_has_legacy_ids_detects_old_scheme():
    ...
    from graph_fy.build import graph_has_legacy_ids
...
def test_semantic_rekey_relative_vs_absolute_source_file():
    ...
    from graph_fy.build import _semantic_id_remap
...
def test_derive_prune_root_recovers_root_from_posix_absolute_prune_sources():
    ...
    direction.
    """
    from graph_fy.build import _derive_prune_root
...
def test_build_annotations_all_resolve():
    ...
    from graph_fy import build as build_module
