...
build_merge backs `graph_fy --update`. Two regressions covered here:
...
from graph_fy.build import build_merge, _infer_merge_root
...
def test_prune_without_root_removes_ghost_nodes_via_grandparent_fallback(tmp_path):
    root = tmp_path / "corpus"
    (root / "graph_fy_out").mkdir(parents=True)
    graph_path = root / "graph_fy_out" / "graph.json"
    nodes = [
...
def test_prune_without_root_uses_graph_fy_root_marker(tmp_path):
    # graph.json not under a <root>/graph_fy_out layout, so grandparent wouldn't
    # help — the committed .graph_fy_root marker must be honored instead.
    out = tmp_path / "out"
    out.mkdir()
    graph_path = out / "graph.json"
    real_root = tmp_path / "elsewhere" / "repo"
    real_root.mkdir(parents=True)
    (out / ".graph_fy_root").write_text(str(real_root), encoding="utf-8")
...
def test_prune_matches_across_symlinked_root(tmp_path):
    ...
    real = tmp_path / "real"
    (real / "graph_fy_out").mkdir(parents=True)
    link = tmp_path / "link"
    os.symlink(real, link)
    graph_path = real / "graph_fy_out" / "graph.json"
...
def test_prune_matches_node_stored_absolute_against_relative_delete(tmp_path):
    ...
    # Runbook-style: NO root passed (eff_root inferred from the graph_fy_out
...
def test_graph_fy_root_marker_with_a_utf8_bom_still_resolves(tmp_path):
