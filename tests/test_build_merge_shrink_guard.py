...
- #2446: with `root` omitted and a non-standard layout (graph.json not under
  <root>/graph_fy_out/), the inferred root was wrong, absolute prune_sources
...
import graph_fy.build as buildmod
from graph_fy.build import build_merge, merge_raw_extraction
...
def test_no_existing_graph_path_unchanged(tmp_path):
    ...
    gp = tmp_path / "graph_fy_out" / "graph.json"  # does not exist
...
def test_absolute_prune_custom_layout_derives_root(tmp_path):
    """graph.json directly in <root> (no graph_fy_out dir, no marker): the
...
def test_absolute_prune_standard_layout_marker_still_prunes(tmp_path):
    ...
    .graph_fy_root marker keeps working without root=."""
    root = tmp_path / "repo"
...
def test_zero_match_prune_warns_instead_of_already_clean(tmp_path, capsys):
    ...
    gp = root / "graph_fy_out" / "graph.json"
