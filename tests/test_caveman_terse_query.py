...
def test_cli_query_terse_and_caveman(monkeypatch, tmp_path: Path, capsys):
    ...
    monkeypatch.setattr(
        mainmod.sys,
        "argv",
        ["graph_fy", "query", "login", "--graph", str(graph_path)],
    )
    mainmod.main()
    ...
    monkeypatch.setattr(
        mainmod.sys,
        "argv",
        ["graph_fy", "query", "login", "--graph", str(graph_path), "--terse"],
    )
    mainmod.main()
    ...
    monkeypatch.setattr(
        mainmod.sys,
        "argv",
        ["graph_fy", "query", "login", "--graph", str(graph_path), "--caveman"],
    )
    mainmod.main()
...
def test_cli_explain_terse_and_caveman(monkeypatch, tmp_path: Path, capsys):
    ...
    monkeypatch.setattr(
        mainmod.sys,
        "argv",
        ["graph_fy", "explain", "login", "--graph", str(graph_path)],
    )
    mainmod.main()
    normal_out = capsys.readouterr().out
    ...
    monkeypatch.setattr(
        mainmod.sys,
        "argv",
        ["graph_fy", "explain", "login", "--graph", str(graph_path), "--terse"],
    )
    mainmod.main()
    terse_out = capsys.readouterr().out
    ...
    monkeypatch.setattr(
        mainmod.sys,
        "argv",
        ["graph_fy", "explain", "login", "--graph", str(graph_path), "--caveman"],
    )
    mainmod.main()
    caveman_out = capsys.readouterr().out
    assert caveman_out.strip() == terse_out.strip()
...
def test_cli_query_legacy_text_terse(monkeypatch, tmp_path: Path, capsys):
    ...
    monkeypatch.setattr(
        mainmod.sys,
        "argv",
        ["graph_fy", "query", "who calls login", "--graph", str(graph_path)],
    )
    mainmod.main()
    normal_out = capsys.readouterr().out
    ...
    monkeypatch.setattr(
        mainmod.sys,
        "argv",
        ["graph_fy", "query", "who calls login", "--graph", str(graph_path), "--terse"],
    )
    mainmod.main()
    terse_out = capsys.readouterr().out
    assert "Traversal:" not in terse_out
