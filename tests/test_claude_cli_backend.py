...
def test_extraction_instructions_ride_in_user_turn(fake_claude):
    ...
    assert "graph_fy semantic extraction agent" in sent
...
# `--json-schema`, graph_fy constrains the output shape structurally so the
...
def test_json_schema_flag_added_when_cli_supports_it(monkeypatch, fake_claude):
    ...
    pins the top-level {nodes, edges} shape graph_fy parses."""
