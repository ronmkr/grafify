...
`sed q`) is routine. graph_fy used to keep writing after the reader disconnected,
...
def test_help_survives_reader_closing_pipe_early():
    """`graph_fy --help | head -n1` must leave graph_fy exiting 0, not 255."""
    producer = subprocess.Popen(
        [PYTHON, "-m", "graph_fy", "--help"], stdout=subprocess.PIPE
    )
...
def test_small_buffered_output_survives_reader_that_reads_nothing():
    ...
    producer = subprocess.Popen(
        [PYTHON, "-m", "graph_fy", "--version"], stdout=subprocess.PIPE
    )
    reader = subprocess.Popen(
