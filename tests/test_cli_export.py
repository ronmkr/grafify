"""Integration tests for graph_fy export subcommands and CLI commands.
...
def _make_graph(tmp_path: Path) -> Path:
    """Build a minimal graph.json + analysis/labels files in tmp_path/graph_fy_out/."""
    ...
    from graph_fy.analyze import god_nodes, surprising_connections
    ...
    (out / ".graph_fy_analysis.json").write_text(json.dumps(analysis))
...
def test_export_html_creates_file(tmp_path):
    ...
    html = tmp_path / "graph_fy_out" / "graph.html"
    assert html.exists()
...
def test_export_obsidian_creates_vault(tmp_path):
    ...
    assert r.returncode == 0, r.stderr
    vault = tmp_path / "graph_fy_out" / "obsidian"
    assert vault.exists()
...
# ── graph_fy export falkordb (cypher) ────────────────────────────────────────
...
def test_query_uses_graph_fy_out_env(tmp_path):
    out = _make_graph(tmp_path)
    custom_out = tmp_path / "custom-graph"
    out.rename(custom_out)
    env = os.environ.copy()
    env["graph_fy_OUT"] = custom_out.name
...
def test_extract_writes_to_graph_fy_out_env(tmp_path):
    """#1423: `graph_fy extract` honours graph_fy_OUT for where it WRITES, not only
    where readers look — previously it hardcoded graph_fy_out/ and ignored the
    override. Code-only corpus, so no LLM backend is needed."""
    (tmp_path / "m.py").write_text("def a():\n    return b()\n\n\ndef b():\n    return 1\n")
    env = os.environ.copy()
    env["graph_fy_OUT"] = "custom-out"
    ...
    assert not (tmp_path / "graph_fy_out").exists(), "extract ignored graph_fy_OUT and wrote graph_fy_out/"
...
def test_path_uses_graph_fy_out_env(tmp_path):
    out = _make_graph(tmp_path)
    custom_out = tmp_path / "custom-graph"
    out.rename(custom_out)
    env = os.environ.copy()
...
def test_explain_uses_graph_fy_out_env(tmp_path):
    out = _make_graph(tmp_path)
    custom_out = tmp_path / "custom-graph"
    out.rename(custom_out)
    env = os.environ.copy()
...
# ── graph_fy export unknown format ───────────────────────────────────────────
...
# Regression test for #934 - cluster-only crashes when graph_fy_out/ doesn't exist

def test_cluster_only_creates_output_dir_when_missing(tmp_path):
    """cluster-only must not crash with FileNotFoundError when graph_fy_out/ is absent (#934)."""
    # Build graph.json somewhere other than the default graph_fy_out/ location
    # so we can point --graph at it while graph_fy_out/ doesn't exist yet.
    ...
    assert r.returncode == 0, r.stderr
    assert (tmp_path / "graph_fy_out" / "GRAPH_REPORT.md").exists()


def test_cluster_only_graph_in_graph_fy_out_writes_beside_it(tmp_path):
    """#1747 Case 2: `cluster-only --graph <elsewhere>/graph_fy_out/graph.json`
    must write GRAPH_REPORT.md and the re-clustered graph beside that graph, not
    into a stray graph_fy_out/ in the CWD."""
    project = tmp_path / "project"
    project.mkdir()
    out_dir = _make_graph(project)  # project/graph_fy_out/graph.json
...
def test_extract_out_does_not_pollute_corpus(tmp_path):
    ...
    assert r.returncode == 0, r.stderr
    assert (out / "graph_fy_out" / "graph.json").exists()   # graph in --out
    assert not (corpus / "graph_fy_out").exists()           # corpus untouched
...
def test_cluster_only_persists_analysis_sidecar(tmp_path):
    """cluster-only must refresh .graph_fy_analysis.json alongside graph.json.
    ...
    """
    out = _make_graph(tmp_path)
    analysis_path = out / ".graph_fy_analysis.json"
...
def test_cluster_only_remaps_labels_to_previous_cids(tmp_path):
    ...
    .graph_fy_labels.json keeps tracking the same conceptual communities after
    ...
    labels_json = out / ".graph_fy_labels.json"
...
# ── communities-fallback when .graph_fy_analysis.json is absent ──────────────
# The watch / post-commit rebuild path only writes graph.json + GRAPH_REPORT.md;
# it does NOT regenerate .graph_fy_analysis.json. The full `graph_fy extract`
...
# Without these tests, `graph_fy export html|obsidian|wiki|svg|graphml|neo4j`
...
def test_export_html_falls_back_to_node_community_attribute(tmp_path):
    """When .graph_fy_analysis.json is absent, export html should reconstruct
    ...
    (out / ".graph_fy_analysis.json").unlink()
...
def test_export_html_fallback_recovers_multiple_communities(tmp_path):
    ...
    analysis = json.loads((out / ".graph_fy_analysis.json").read_text(encoding="utf-8"))
...
def test_graph_json_node_ids_are_portable_across_checkout_paths(tmp_path):
    ...
    def _build(root: Path):
        ...
        assert r.returncode == 0, r.stderr
        data = json.loads((root / "graph_fy_out" / "graph.json").read_text())
...
def test_cluster_only_warns_when_labeling_flags_are_ignored(tmp_path):
    """#2534 case 1: with a saved .graph_fy_labels.json the reuse branch never
    ...
    _make_graph(tmp_path)  # persists .graph_fy_labels.json -> reuse branch
...
# ── graph_fy query names its graph (#2789) ───────────────────────────────────
...
def test_query_command_header_names_the_graph(tmp_path):
    ...
    assert first_line.startswith("Graph: graph_fy_out/graph.json ("), first_line
