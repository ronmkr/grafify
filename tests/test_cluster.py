...
from graph_fy.cluster import cluster, cohesion_score, remap_communities_to_previous, score_all
...
def test_cluster_does_not_write_to_stderr(capsys):
    ...
    cluster(G)
    captured = capsys.readouterr()
    # Allow logging output (starts with [graph_fy]) but no raw ANSI codes
...
def test_partition_is_invariant_to_edge_endpoint_orientation():
    ...
    from graph_fy.cluster import _partition
