"""Tests for graph_fy/dedup.py entity deduplication pipeline."""
from __future__ import annotations
import pytest
from graph_fy.dedup import (
...
def test_build_calls_dedup():
    ...
    from graph_fy.build import build
    chunk1 = {
...
def test_dedup_still_merges_real_typos():
    ...
    from graph_fy.dedup import _is_variant_pair, _short_label_blocked
...
def test_variant_pair_helper():
    ...
    from graph_fy.dedup import _is_variant_pair
...
def test_prefix_guard_does_not_block_same_length_typos():
    ...
    from graph_fy.dedup import _norm
...
def test_numeric_tokens_differ_helper():
    ...
    multisets (#1284)."""
    from graph_fy.dedup import _numeric_tokens_differ
...
def test_id_prefixes_preserves_non_latin_segments():
    from graph_fy.dedup import _id_prefixes
...
def test_content_token_swap_helper():
    ...
    from graph_fy.dedup import _content_token_swap
...
def test_same_word_variant_helper():
    ...
    from graph_fy.dedup import _same_word_variant
...
def test_reads_as_file_entity_helper():
    ...
    from graph_fy.dedup import _reads_as_file_entity
...
def test_extract_deduplicates_nodes_and_edges(tmp_path):
    from graph_fy.extract import extract

    f = tmp_path / "sample.py"
    f.write_text("def hello():\n    return 42\n")

    res = extract([f], root=tmp_path)
    nodes = res["nodes"]
    edges = res["edges"]
    ...
    edge_keys = [(e["source"], e["target"], e.get("relation")) for e in edges]
