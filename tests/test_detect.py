...
from graph_fy.detect import classify_file, count_words, detect, detect_incremental, save_manifest, FileType, _is_ignored, _load_graph_fyignore, _is_sensitive
from graph_fy import detect as detect_mod
...
def test_classify_pdf():
    assert classify_file(Path("paper.pdf")) is None
...
def test_classify_image():
    assert classify_file(Path("screenshot.png")) is None
    assert classify_file(Path("design.jpg")) is None
    assert classify_file(Path("diagram.webp")) is None
...
def test_detect_skips_noise_dot_dirs():
    """Noise dot dirs (.next, .nuxt, .graph_fy cache, …) are skipped (#873).
    Non-noise dot dirs (.github, .claude, …) are now allowed through."""
    result = detect(FIXTURES)
    for files in result["files"].values():
        for f in files:
            # graph_fy's own cache is always skipped
            assert "/.graph_fy/" not in f
...
def test_classify_md_paper_by_signals(tmp_path):
    """A .md file with paper signals classifies as DOCUMENT in local no-LLM build."""
    ...
    assert classify_file(paper) == FileType.DOCUMENT
...
def test_classify_attention_paper():
    """The real attention paper file should be classified as DOCUMENT in local build."""
    paper_path = Path("/home/safi/graph_fy_eval/papers/attention_is_all_you_need.md")
    if paper_path.exists():
        result = classify_file(paper_path)
        assert result == FileType.DOCUMENT


def test_graph_fyignore_excludes_file(tmp_path):
    """Files matching .graph_fyignore patterns are excluded from detect()."""
    (tmp_path / ".graph_fyignore").write_text("vendor/\n*.generated.py\n")
    ...
    assert result["graph_fyignore_patterns"] == 2


def test_graph_fyignore_matches_nfd_path_with_nfc_pattern(tmp_path):
    ...
    (tmp_path / ".graph_fyignore").write_text(f"{nfc_name}/\n", encoding="utf-8")
...
def test_graph_fyignore_matches_nfc_path_with_nfd_pattern(tmp_path):
    """The reverse direction also holds: NFD pattern, NFC path on disk."""
    nfc_name = unicodedata.normalize("NFC", "Or\u00e7amento")
    nfd_name = unicodedata.normalize("NFD", nfc_name)

    (tmp_path / ".graph_fyignore").write_text(f"{nfd_name}/\n", encoding="utf-8")
    d = tmp_path / nfc_name
    d.mkdir()
...
def test_graph_fyignore_ascii_patterns_unaffected(tmp_path):
...
def test_graph_fyignore_missing_is_fine(tmp_path):
    """No .graph_fyignore is not an error."""
    (tmp_path / "main.py").write_text("x = 1")
    result = detect(tmp_path)
    assert result["graph_fyignore_patterns"] == 0


def test_graph_fyignore_comments_ignored(tmp_path):
    """Comment lines in .graph_fyignore are not treated as patterns."""
    (tmp_path / ".graph_fyignore").write_text("# this is a comment\n\nmain.py\n")
    (tmp_path / "main.py").write_text("x = 1")
...
def test_graph_fyignore_utf8_bom_first_pattern_honored(tmp_path):
    """A UTF-8 BOM at the start of .graph_fyignore must not corrupt the first
    pattern (#2163): git strips a single leading BOM, so `*.log` on line 1
    must still exclude app.log."""
    (tmp_path / ".graph_fyignore").write_bytes(b"\xef\xbb\xbf*.log\nbuild/\n")
...
def test_graph_fyignore_bom_only_file(tmp_path):
    """A .graph_fyignore containing only a BOM yields zero patterns, not one
    bogus U+FEFF pattern (#2163)."""
    (tmp_path / ".graph_fyignore").write_bytes(b"\xef\xbb\xbf")
    (tmp_path / "main.py").write_text("x = 1")
...
def test_graph_fyignore_bom_then_comment(tmp_path):
    """A BOM followed by a comment must still parse as a comment, not become
    a `\\ufeff# comment` pattern (#2163)."""
    (tmp_path / ".graph_fyignore").write_bytes(b"\xef\xbb\xbf# comment\nmain.py\n")
    (tmp_path / "main.py").write_text("x = 1")
    (tmp_path / "other.py").write_text("x = 2")
    ...
    assert not any("main.py" in f for f in result["files"]["code"])
    assert any("other.py" in f for f in result["files"]["code"])
    assert result["graph_fyignore_patterns"] == 1, "BOM'd comment became a pattern"
...
def test_git_info_exclude_utf8_bom(tmp_path):
    ...
    pattern either (#2163) — second read site in _load_graph_fyignore."""
...
def test_graph_fyignore_hermetic_without_vcs(tmp_path):
    """Without a VCS root, parent .graph_fyignore does NOT apply (hermetic)."""
    (tmp_path / ".graph_fyignore").write_text("vendor/\n")
    ...
    # parent .graph_fyignore must NOT leak into a non-VCS scan
    assert any("vendor" in f for f in code_files)
    assert result["graph_fyignore_patterns"] == 0


def test_graph_fyignore_discovered_from_parent_in_vcs(tmp_path):
    """Inside a VCS repo, parent .graph_fyignore applies to subdirectory scans."""
    (tmp_path / ".git").mkdir()
    (tmp_path / ".graph_fyignore").write_text("vendor/\n")
    sub = tmp_path / "packages" / "mylib"
    sub.mkdir(parents=True)
    ...
    assert result["graph_fyignore_patterns"] >= 1


def test_graph_fyignore_stops_at_git_boundary(tmp_path):
    """Upward search stops at the git repo root (.git directory)."""
    (tmp_path / ".graph_fyignore").write_text("main.py\n")
    repo = tmp_path / "repo"
    repo.mkdir()
    (repo / ".git").mkdir()
    sub = repo / "sub"
...
def test_graph_fyignore_at_git_root_is_included(tmp_path):
    """A .graph_fyignore at the git repo root is included when scanning a subdir."""
    repo = tmp_path / "repo"
    repo.mkdir()
    (repo / ".git").mkdir()
    (repo / ".graph_fyignore").write_text("vendor/\n")
...
def test_graph_fyignore_still_excludes_git_tracked_file(tmp_path):
    ...
    (tmp_path / ".graph_fyignore").write_text("storage/\n", encoding="utf-8")
...
def test_detect_incremental_propagates_follow_symlinks(requires_symlinks, tmp_path, monkeypatch):
    ...
    # Store manifest inside graph_fy_out/ so it is pruned by _SKIP_DIRS
    # and doesn't get re-detected as a code file now that .json is indexed.
    manifest_dir = tmp_path / "graph_fy_out"
    manifest_dir.mkdir()
...
def test_classify_video_extensions():
    """Video and audio file extensions should return None in local no-LLM build."""
    ...
    assert classify_file(Path("recording.wav")) is None
    assert classify_file(Path("webinar.webm")) is None
    assert classify_file(Path("audio.m4a")) is None


def test_classify_google_workspace_shortcuts():
    assert classify_file(Path("notes.gdoc")) is None
    assert classify_file(Path("budget.gsheet")) is None
    assert classify_file(Path("deck.gslides")) is None
...
def test_detect_finds_video_files(tmp_path):
    """Video files are ignored in local no-LLM build."""
    (tmp_path / "lecture.mp4").write_bytes(b"fake video data")
    (tmp_path / "notes.md").write_text("# Notes\nSome content here.")
    result = detect(tmp_path)
    assert len(result["files"]["video"]) == 0
...
def test_collect_files_keeps_coverage_code_namespace(tmp_path):
    ...
    from graph_fy.extract import collect_files
...
def test_detect_allows_github_dir(tmp_path):
    ...
    (gh / "ci.sh").write_text("#!/bin/sh\necho test\n")
    (tmp_path / "main.py").write_text("def run(): pass")
    result = detect(tmp_path)
    all_files = [f for files in result["files"].values() for f in files]
    assert any(".github" in f for f in all_files), "expected .github/workflows/ci.sh to be detected"
...
def test_detect_honors_git_info_exclude(tmp_path):
    ...
    .graph_fyignore — otherwise nested worktree copies get fully indexed (#1810)."""
    (tmp_path / ".git" / "info").mkdir(parents=True)
...
def test_git_info_exclude_ranks_below_gitignore_negation(tmp_path):
    ...
    from graph_fy.detect import _load_graph_fyignore, _is_ignored
    (tmp_path / ".git" / "info").mkdir(parents=True)
    ...
    patterns = _load_graph_fyignore(tmp_path)
...
def test_detect_skips_graph_fy_own_cache(tmp_path):
    """.graph_fy/ (extraction cache) must never be re-indexed as source (#873)."""
    cache = tmp_path / ".graph_fy" / "cache"
    cache.mkdir(parents=True)
    (cache / "abc123.json").write_text('{"nodes": [], "edges": []}')
    (tmp_path / "app.py").write_text("def go(): pass")
    result = detect(tmp_path)
    all_files = [f for files in result["files"].values() for f in files]
    assert not any(".graph_fy" in f for f in all_files)
    assert any("app.py" in f for f in all_files)
...
def test_anchored_root_wildcard_negation_reincludes_subtree(tmp_path):
    ...
    for rel in ("src/app/main.py", "src/lib/util.py", "docs/guide.md", "README.md"):
        ...
        path.write_text("x\n")
    (tmp_path / ".graph_fyignore").write_text("/*\n!/src/\n")
...
def test_path_pattern_single_star_does_not_cross_segment(tmp_path):
    ...
    for pattern in ("/src/*.py", "src/*.py"):
        (tmp_path / ".graph_fyignore").write_text(f"{pattern}\n")
        result = detect(tmp_path)
        files = as_posix_list(
...
def test_anchored_double_star_crosses_path_segments(tmp_path):
    ...
    direct.write_text("x\n")
    nested.write_text("x\n")
    (tmp_path / ".graph_fyignore").write_text("/src/**/generated.py\n")
...
def test_negation_cannot_rescue_file_under_excluded_dir(tmp_path):
    ...
    from graph_fy.detect import _is_ignored, _load_graph_fyignore
    android = tmp_path / "android" / "app" / "src"
    android.mkdir(parents=True)
    victim = android / "Main.kt"
    victim.write_text("fun main() {}")
    (tmp_path / ".graph_fyignore").write_text("android/\n!src/\n")
    patterns = _load_graph_fyignore(tmp_path)
...
def test_negation_works_when_no_ancestor_excluded(tmp_path):
    ...
    keep = src / "keep.py"
    keep.write_text("x = 1")
    (tmp_path / ".graph_fyignore").write_text("*.py\n!src/keep.py\n")
    patterns = _load_graph_fyignore(tmp_path)
...
def test_negation_does_not_disable_directory_pruning(tmp_path, monkeypatch):
    ...
    """
    import os
    import graph_fy.detect as det

    (tmp_path / ".graph_fyignore").write_text("myignored/\n*.md\n!docs/**\n")
...
def test_anchored_dir_not_matched_at_depth(tmp_path):
    ...
    (tmp_path / ".graph_fyignore").write_text("/inbox/\n")
    patterns = _load_graph_fyignore(tmp_path)
    assert not _is_ignored(f, tmp_path, patterns), (
...
def test_anchored_file_not_matched_at_depth(tmp_path):
    ...
    (tmp_path / ".graph_fyignore").write_text("/build\n")
    patterns = _load_graph_fyignore(tmp_path)
...
def test_anchored_multi_segment_pattern(tmp_path):
    ...
    (tmp_path / ".graph_fyignore").write_text("/src/inbox/\n")
    patterns = _load_graph_fyignore(tmp_path)
...
def test_detect_does_not_ignore_scan_root_itself_via_parent_gitignore(tmp_path):
    ...
    corpus_dir = tmp_path / "graph_fy-corpus"
    corpus_dir.mkdir()
    ...
    (tmp_path / ".gitignore").write_text("graph_fy-corpus/\n", encoding="utf-8")
...
def test_is_ignored_cache_evaluates_each_dir_once():
    ...
    """
    from graph_fy.detect import _is_ignored
...
# a 76k-file vault with a 29k-file directory listed in .graph_fyignore pinned a
...
def test_ignored_dir_pruned_walk_never_lists_contents(tmp_path, monkeypatch):
    ...
    a regenerated notes/ dir listed in .graph_fyignore.
    """
    import graph_fy.detect as det

    (tmp_path / ".graph_fyignore").write_text("graph_fy-notes/\n")
    for d in range(5):
        sub = tmp_path / "graph_fy-notes" / f"note-dir-{d}"
        sub.mkdir(parents=True)
        for f in range(4):
    ...
    assert not any("graph_fy-notes" in Path(v).parts for v in visited), (
    ...
    assert not any("graph_fy-notes" in p for p in all_files)
    ...
    assert sum("graph_fy-notes" in p for p in ignored) == 1
...
def test_scan_ignore_cost_is_per_directory_not_per_file(tmp_path, monkeypatch):
    ...
    for d in range(n_ignored_dirs):
        sub = tmp_path / "graph_fy-notes" / f"note-dir-{d:03d}"
        sub.mkdir(parents=True)
    ...
    def counting_scan(path, *args, **kwargs):
        scan_calls["total"] += 1
        if "graph_fy-notes" in path.parts:
...
def test_save_manifest_skips_semantic_hash_for_files_without_cache(tmp_path):
    ...
    import json
    from graph_fy.cache import save_cached
...
def test_save_manifest_clear_semantic_erases_stale_hash_for_omitted_file(tmp_path):
    ...
    doc.parent.mkdir()
    doc.write_text("# Doc\n\ncontent")
    manifest_path = str(tmp_path / "graph_fy_out" / "manifest.json")
...
def test_save_manifest_clear_ast_blanks_both_hashes_for_failed_extra(tmp_path):
    ...
    the next extract re-queues the file without deleting graph_fy_out/."""
    import json
...
# Regression tests for #945 - .gitignore fallback when no .graph_fyignore exists

def test_gitignore_fallback_when_no_graph_fyignore(tmp_path):
    """When no .graph_fyignore exists, .gitignore patterns are honored (#945)."""
    (tmp_path / ".git").mkdir()
...
def test_graph_fyignore_and_gitignore_are_merged(tmp_path):
    """When both exist, their patterns are MERGED — a file excluded only by
    .gitignore stays excluded even though .graph_fyignore says nothing about it
    (#1363). Previously the presence of a .graph_fyignore silently disabled the
    ...
    (tmp_path / ".graph_fyignore").write_text("other.py\n")  # says nothing about main.py
    (tmp_path / "main.py").write_text("x = 1")
    (tmp_path / "other.py").write_text("x = 2")
    ...
    assert not any("other.py" in f for f in code)  # graph_fyignore applied
    assert any("keep.py" in f for f in code)       # neither excludes it


def test_graph_fyignore_negation_overrides_gitignore(tmp_path):
    """.graph_fyignore is evaluated after .gitignore, so a `!` negation in it can
    re-include a file the .gitignore excluded (last-match-wins, #1363)."""
    (tmp_path / ".git").mkdir()
    (tmp_path / ".gitignore").write_text("*.py\n")           # exclude all .py
    (tmp_path / ".graph_fyignore").write_text("!keep.py\n")  # but rescue keep.py
    (tmp_path / "main.py").write_text("x = 1")
    (tmp_path / "keep.py").write_text("x = 2")

    result = detect(tmp_path)
    code = result["files"]["code"]
    assert any("keep.py" in f for f in code)      # rescued by graph_fyignore negation
...
def test_shebang_interpreter_plain(tmp_path):
    ...
    from graph_fy.detect import _shebang_interpreter
...
# relative keys so a committed ``graph_fy_out/`` round-trips across machines
...
def test_save_manifest_relativizes_keys_when_root_given(tmp_path):
    ...
    import json
    from graph_fy.detect import save_manifest, load_manifest
...
def test_load_manifest_absolutizes_relative_keys(tmp_path):
    ...
    import json
    from graph_fy.detect import load_manifest

    manifest_path = tmp_path / "graph_fy_out" / "manifest.json"
...
def test_detect_incremental_portable_across_paths(tmp_path):
    ...
    import json
    from graph_fy.detect import save_manifest, detect_incremental
    ...
    manifest_a = str(repo_a / "graph_fy_out" / "manifest.json")
    files = {
    ...
    (repo_b / "graph_fy_out").mkdir()
    manifest_b = repo_b / "graph_fy_out" / "manifest.json"
...
def test_convert_office_file_sidecar_name_stable_across_checkouts(tmp_path, monkeypatch):
    ...
    produces the same sidecar name (no unbounded duplicates when graph_fy_out/ is
    ...
    def _sidecar(root):
        ...
        out_dir = root / "graph_fy_out" / "converted"
    ...
    fallback = detect_mod.convert_office_file(
        checkout_a / "docs" / "report.xlsx", checkout_a / "graph_fy_out" / "converted"
    )
...
def test_convert_office_file_outside_root_falls_back(tmp_path, monkeypatch):
    ...
    root = tmp_path / "repo"
    (root / "graph_fy_out" / "converted").mkdir(parents=True)
...
@pytest.mark.skip(reason="Office conversion removed in local no-LLM build")
def test_detect_office_conversion_respects_cache_root(tmp_path, monkeypatch):
    ...
    cache_root/graph_fy_OUT/converted, leaving the scanned corpus untouched, while
    ...
    assert not (corpus / detect_mod.graph_fy_OUT).exists(), (
        "detect() must not write graph_fy_out into the scanned corpus tree when cache_root is provided (#2787)"
    )

    # 2. Converted sidecar must exist under cache_root
    converted_dir = cache_out / detect_mod.graph_fy_OUT / "converted"
...
@pytest.mark.parametrize(
    ...
    [
        pytest.param("graph_fy_out/nlp", False, None, id="default-parent"),
...
def test_nested_graph_fy_out_prunes_only_configured_path(
    ...
    probe = subprocess.run(
        ...
        env={**os.environ, "graph_fy_OUT": configured_out},
        check=True,
        capture_output=True,
        text=True,
    )
...
def test_detect_records_unclassified_extensionless_files(tmp_path):
    # #1692: extensionless, non-shebang project files (Gemfile, Makefile, ...)
    ...
    assert unclassified == ["Gemfile", "LICENSE", "Makefile"]
...
def test_graph_fyinclude_is_inert_and_not_unclassified(tmp_path, capsys):
    """#2112: .graph_fyinclude support was removed (dead since #873).

    A leftover .graph_fyinclude must not error, must not surface in the
    ...
    (tmp_path / ".graph_fyinclude").write_text(".github/\ndocs/**\n")
    result = detect(tmp_path)

    # not surfaced as an unclassified scan input
    assert not any(".graph_fyinclude" in p for p in result["unclassified"])
    # real files are indexed exactly as before; the file changes nothing
    assert result["files"] == baseline["files"]
    assert any("main.py" in f for f in result["files"]["code"])
    # one-time stderr note, matching the [graph_fy] warning convention
    err = capsys.readouterr().err
    assert err.count("[graph_fy] WARNING: .graph_fyinclude is no longer supported") == 1
...
def test_save_manifest_noop_skips_disk_write(tmp_path):
    ...
    a = tmp_path / "a.py"
    a.write_text("x = 1\n", encoding="utf-8")
    manifest_path = Path(tmp_path / "graph_fy_out" / "manifest.json")
...
def test_save_manifest_ast_kind_noop_then_change(tmp_path):
    """#2838's literal path: `graph_fy update` calls save_manifest with kind='ast'.
...
def test_sensitive_filter_indexes_topic_prose_and_source(path):
    from graph_fy.detect import _is_sensitive
...
def test_lexical_relative_matches_pathlib_relative_to():
    ...
    from pathlib import Path
    from graph_fy.detect import _lexical_relative, _nfc
...
def test_globstar_matcher_leaves_no_reference_cycle():
    ...
    import gc
    from graph_fy.detect import _match_anchored_ignore_pattern
