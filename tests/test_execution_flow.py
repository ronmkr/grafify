"""Tests for Execution Flow Slicing & Entry-to-Sink Tracing (graph_fy.flow, cli, mcp)."""
...
from graph_fy.mcp import TOOL_SPEC, get_execution_flow
...
def test_cli_flow_basic(monkeypatch, tmp_path, capsys):
    """Verify graph_fy flow <entry_point> CLI output."""
    graph_path = _write_graph_file(tmp_path)
    monkeypatch.setattr(mainmod, "_check_skill_version", lambda _: None)
    monkeypatch.setattr(
        mainmod.sys,
        "argv",
        ["graph_fy", "flow", "POST /login", "--graph", str(graph_path)],
    )
...
def test_cli_flow_with_sink_filter(monkeypatch, tmp_path, capsys):
    """Verify graph_fy flow <entry_point> --sink <sink> CLI output."""
    graph_path = _write_graph_file(tmp_path)
    monkeypatch.setattr(mainmod, "_check_skill_version", lambda _: None)
    monkeypatch.setattr(
        mainmod.sys,
...
def test_cli_flow_json_output(monkeypatch, tmp_path, capsys):
    """Verify graph_fy flow --json outputs valid JSON with structured flow data."""
    graph_path = _write_graph_file(tmp_path)
    monkeypatch.setattr(mainmod, "_check_skill_version", lambda _: None)
    monkeypatch.setattr(
        mainmod.sys,
        "argv",
        ["graph_fy", "flow", "POST /login", "--json", "--graph", str(graph_path)],
    )
...
def test_cli_flow_missing_args(monkeypatch, capsys):
    """Verify graph_fy flow with no args prints usage and exits with code 1."""
    monkeypatch.setattr(mainmod, "_check_skill_version", lambda _: None)
    monkeypatch.setattr(mainmod.sys, "argv", ["graph_fy", "flow"])
    ...
    assert "Usage: graph_fy flow" in err


# --- MCP Tool Tests ---

def test_mcp_get_execution_flow_tool():
    """Verify graph_fy.mcp.get_execution_flow works via function call and dict."""
    G = _create_sample_graph()
