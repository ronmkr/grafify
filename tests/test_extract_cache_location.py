...
graph_fy_out/cache/ inside it. It now defaults to the current working directory;
...
import graph_fy.cache as cache
from graph_fy.cache import load_cached, file_hash
...
def test_default_cache_lands_in_cwd_not_source_tree(tmp_path, monkeypatch):
    ...
    # the key-root, leaving a stray graph_fy_out/ in a writable corpus, #1774).
    assert not (corpus / "graph_fy_out").exists(), (
        "cache/stat-index written into the analyzed source tree (#1774)"
    )
    assert (work / "graph_fy_out" / "cache").is_dir(), "cache should land under CWD"
...
def test_default_cache_does_not_leave_stat_index_in_source_tree(tmp_path, monkeypatch):
    ...
    assert not (corpus / "graph_fy_out").exists(), "stat-index leaked into the corpus"
    assert (work / "graph_fy_out" / "cache" / "stat-index.json").exists(), (
...
def test_explicit_cache_root_still_wins(tmp_path, monkeypatch):
    ...
    assert (out / "graph_fy_out" / "cache").is_dir()
    assert not (corpus / "graph_fy_out").exists()
    assert not (work / "graph_fy_out").exists()
