"""Tests for `graph_fy extract` CLI dispatch path in graph_fy.__main__."""
...
def test_extract_exits_nonzero_when_ast_extraction_raises(
    ...
    import graph_fy.extract as extractmod
    ...
    monkeypatch.setattr(
        mainmod.sys,
        "argv",
        ["graph_fy", "extract", str(corpus), "--code-only",
         "--out", str(out_dir)],
    )
    ...
    assert (
        "[graph_fy extract] AST extraction failed: worker pool failed"
        in capsys.readouterr().err
    )
    assert not (out_dir / "graph_fy_out" / "graph.json").exists(), (
...
def test_extract_allow_partial_continues_past_ast_failure(
    ...
    monkeypatch.setattr(
        "graph_fy.llm.extract_corpus_parallel", _one_chunk_succeeded
    )
    monkeypatch.setattr(mainmod, "_check_skill_version", lambda _: None)
    monkeypatch.setattr(
        mainmod.sys,
        "argv",
        ["graph_fy", "extract", str(corpus), "--backend", "claude",
    ...
    assert (out_dir / "graph_fy_out" / "graph.json").exists(), (
...
def test_extract_exits_nonzero_when_all_semantic_chunks_fail(
    ...
    The bug this guards: `pip install graph_fyy` doesn't pull in `anthropic`,
    so `graph_fy extract --backend claude` would print per-chunk errors and
    ...
    monkeypatch.setattr(
        "graph_fy.llm.extract_corpus_parallel", _all_chunks_failed
    )
    monkeypatch.setattr(mainmod, "_check_skill_version", lambda _: None)
    monkeypatch.setattr(
        mainmod.sys,
...
def test_extract_succeeds_when_at_least_one_chunk_completes(
    ...
    monkeypatch.setattr(
        "graph_fy.cache.save_semantic_cache", _capture_semantic_cache
    )
    monkeypatch.setattr(mainmod, "_check_skill_version", lambda _: None)
    monkeypatch.setattr(
        mainmod.sys,
...
def test_incremental_partial_run_preserves_untouched_semantic_hash(
    ...
    monkeypatch.setattr(
        "graph_fy.llm.extract_corpus_parallel", _stamp_everything_sent
    )
    monkeypatch.setattr(mainmod, "_check_skill_version", lambda _: None)
    ...
    _run_extract()
    manifest_path = out_dir / "graph_fy_out" / "manifest.json"
...
def test_truncated_doc_semantic_hash_is_cleared_for_requeue(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("graph_fy.llm.extract_corpus_parallel", _extract)
    monkeypatch.setattr(mainmod, "_check_skill_version", lambda _: None)
...
def test_manifest_stamps_freshly_extracted_semantic_docs(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("graph_fy.llm.extract_corpus_parallel", _fresh_relative)
    monkeypatch.setattr(mainmod, "_check_skill_version", lambda _: None)
    monkeypatch.setattr(
        mainmod.sys, "argv",
        ["graph_fy", "extract", str(corpus), "--backend", "claude",
...
def test_manifest_stamps_hyperedge_only_docs(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("graph_fy.llm.extract_corpus_parallel", _hyperedge_only)
    monkeypatch.setattr(mainmod, "_check_skill_version", lambda _: None)
    monkeypatch.setattr(
        mainmod.sys, "argv",
        ["graph_fy", "extract", str(corpus), "--backend", "claude",
    ...
    manifest = json.loads((out_dir / "graph_fy_out" / "manifest.json").read_text())
...
def test_extract_mode_deep_dispatches_over_warm_cache(monkeypatch, tmp_path):
    ...
    calls: list[dict] = []
    monkeypatch.setattr("graph_fy.llm.extract_corpus_parallel",
                        _recording_extractor(calls))
    monkeypatch.setattr(mainmod, "_check_skill_version", lambda _: None)

    # No --out: the default layout (graph_fy_out/ beside the sources) keeps the
    ...
    base = ["graph_fy", "extract", str(corpus), "--backend", "claude",
            "--no-cluster"]
    ...
    assert any((corpus / "graph_fy_out" / "cache" / "semantic-deep").glob("**/*.json"))
...
def test_extract_force_flag_redispatches_and_stamps_manifest(monkeypatch, tmp_path):
    ...
    assert any((corpus / "graph_fy_out" / "cache" / "semantic").glob("**/*.json"))
    manifest = json.loads(
        (corpus / "graph_fy_out" / "manifest.json").read_text()
    )
    assert manifest.get("README.md", {}).get("semantic_hash"), (
...
def test_extract_graph_fy_force_env_redispatches(monkeypatch, tmp_path):
    """graph_fy_FORCE=1 behaves like --force (env parity with `update`)."""
    corpus = _make_corpus(tmp_path)
    monkeypatch.setenv("ANTHROPIC_API_KEY", "sk-test-fake-key")
    monkeypatch.delenv("graph_fy_FORCE", raising=False)
    calls: list[dict] = []
    ...
    monkeypatch.setenv("graph_fy_FORCE", "1")
    _run_extract(monkeypatch, base)
    assert len(calls) == 2, "graph_fy_FORCE=1 must force a re-dispatch"
...
def test_cache_check_mode_deep_reads_deep_namespace(monkeypatch, tmp_path, capsys):
    ...
    _run_extract(monkeypatch, ["graph_fy", "cache-check", str(files_from),
                               "--root", str(tmp_path)])
...
def test_extract_codeonly_succeeds_without_api_key(monkeypatch, tmp_path):
    ...
    Regression: graph_fy extract validated a backend upfront and exited 1 with
    ...
    monkeypatch.setattr(
        mainmod.sys, "argv",
        ["graph_fy", "extract", str(corpus), "--out", str(out_dir)],
    )
    ...
    graph = out_dir / "graph_fy_out" / "graph.json"
...
def test_missing_manifest_code_only_preserves_semantic_layer(monkeypatch, tmp_path):
    """#1925: `graph_fy extract --code-only` with a MISSING manifest.json must
    ...
    (corpus / "README.md").write_text("# Notes\nCurated docs.\n")
    out_dir = tmp_path / "out"
    graph_fy_out = out_dir / "graph_fy_out"
    _clear_backend_keys(monkeypatch)
    monkeypatch.setattr(mainmod, "_check_skill_version", lambda _: None)
    ...
    _run_extract(monkeypatch, ["graph_fy", "extract", str(corpus),
                               "--code-only", "--out", str(out_dir)])
    graph_path = graph_fy_out / "graph.json"
    ...
    (graph_fy_out / "manifest.json").unlink()
    ...
    (graph_fy_out / "manifest.json").unlink(missing_ok=True)
    _run_extract(monkeypatch, ["graph_fy", "extract", str(corpus),
                               "--code-only", "--out", str(out_dir)])
...
def test_extract_out_keeps_project_root_clean(monkeypatch, tmp_path):
    """`extract --out DIR` routes every artifact to DIR/graph_fy_out/ and the
    scanned project must not grow a graph_fy_out/ (or anything else) beside
    its sources.
    ...
    monkeypatch.setattr(
        mainmod.sys, "argv",
        ["graph_fy", "extract", ".", "--out", str(external)],
    )
    ...
    assert not (corpus / "graph_fy_out").exists(), (
        "scanned project must not grow a graph_fy_out/ when --out is set"
    )
...
def test_extract_without_key_still_errors_when_docs_present(
    ...
    monkeypatch.setattr("graph_fy.llm.detect_backend", lambda: None)
    monkeypatch.setattr(mainmod, "_check_skill_version", lambda _: None)
    monkeypatch.setattr(
        mainmod.sys, "argv",
        ["graph_fy", "extract", str(corpus), "--out", str(out_dir)],
...
def test_extract_timing_flag_emits_stage_timings(monkeypatch, tmp_path, capsys):
    """--timing prints per-stage `[graph_fy timing]` lines to stderr (#1490); omitting
    ...
    monkeypatch.setattr(
        mainmod.sys, "argv",
        ["graph_fy", "extract", str(code), "--no-cluster", "--out", str(tmp_path / "o1"), "--timing"],
    )
    with pytest.raises(SystemExit) as exc:
        mainmod.main()
    assert exc.value.code == 0
    err = capsys.readouterr().err
    assert "[graph_fy timing] detect:" in err
    assert "[graph_fy timing] total:" in err

    # without --timing => no timing lines
    monkeypatch.setattr(
        mainmod.sys, "argv",
        ["graph_fy", "extract", str(code), "--no-cluster", "--out", str(tmp_path / "o2")],
    )
    with pytest.raises(SystemExit) as exc2:
        mainmod.main()
    assert exc2.value.code == 0
    assert "graph_fy timing" not in capsys.readouterr().err
...
def test_pathless_postgres_extract_initializes_empty_detection(
    ...
    graph_path = out_root / "graph_fy_out" / "graph.json"
    manifest = out_root / "graph_fy_out" / "manifest.json"
    monkeypatch.setattr(mainmod, "_check_skill_version", lambda _: None)
    monkeypatch.setattr("graph_fy.pg_introspect.introspect_postgres", _introspect)
    ...
    (out_root / "graph_fy_out" / ".graph_fy_semantic_marker").write_text(
        '{"output_tokens": 1}'
    )
    ...
    backups = [
        path
        for path in (out_root / "graph_fy_out").iterdir()
...
def test_incremental_extract_prunes_newly_excluded_file_not_in_manifest(
    ...
    .graph_fyignore, re-run extract: x.py's nodes must be gone even though it
    ...
    _run_extract(
        monkeypatch,
        ["graph_fy", "extract", str(project), "--out", str(out_dir)],
    )
    graph_path = out_dir / "graph_fy_out" / "graph.json"
    manifest_path = out_dir / "graph_fy_out" / "manifest.json"
    ...
    (project / ".graph_fyignore").write_text("x.py\n")
    _run_extract(
        monkeypatch,
        ["graph_fy", "extract", str(project), "--out", str(out_dir)],
    )
...
def test_no_cluster_incremental_prunes_newly_excluded_file(
    ...
    monkeypatch.setattr(
        mainmod.sys, "argv",
        ["graph_fy", "extract", str(project), "--no-cluster", "--out", str(out_dir)],
    )
    with pytest.raises(SystemExit) as exc:
        mainmod.main()
    assert exc.value.code == 0
...
# only `rm -rf graph_fy_out` recovers. The extractor is swapped through
...
def test_failed_extra_is_retried_and_recovers(monkeypatch, tmp_path, capsys):
    ...
    graph_path = out_dir / "graph_fy_out" / "graph.json"
    manifest_path = out_dir / "graph_fy_out" / "manifest.json"
    argv = ["graph_fy", "extract", str(project), "--out", str(out_dir)]
...
def test_cache_check_prompt_file_scopes_hits_to_that_prompt(monkeypatch, tmp_path, capsys):
    ...
    base = ["graph_fy", "cache-check", str(files_from), "--root", str(tmp_path)]
...
def test_edge_only_semantic_extraction_not_stamped_and_retried(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("graph_fy.llm.extract_corpus_parallel", _mock_llm)
    argv = ["graph_fy", "extract", str(corpus), "--backend", "claude",
            "--no-cluster", "--out", str(out_dir)]
    ...
    from graph_fy.cache import file_hash
    cache_sem_dir = out_dir / "graph_fy_out" / "cache" / "semantic"
...
def test_stale_poisoned_manifest_semantic_source_healed(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr("graph_fy.llm.extract_corpus_parallel", _mock_llm)
    argv = ["graph_fy", "extract", str(project), "--backend", "claude",
            "--no-cluster", "--out", str(out_dir)]
...
def test_incremental_stray_attribution_preserves_undispatched_file(
    ...
    monkeypatch.setattr(
        "graph_fy.llm.extract_corpus_parallel", _seed_extraction
    )
    monkeypatch.setattr(mainmod, "_check_skill_version", lambda _: None)
    argv = ["graph_fy", "extract", str(project), "--out", str(out_dir)]
    _run_extract(monkeypatch, argv)
    ...
    monkeypatch.setattr(
        "graph_fy.llm.extract_corpus_parallel", _straying_extraction
    )
    _run_extract(monkeypatch, argv)
