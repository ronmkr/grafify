"""`graph_fy extract --code-only` indexes code without an LLM key (#1734).
...
def _run(repo: Path, *extra: str):
    ...
    env["graph_fy_OUT"] = str(repo / "graph_fy_out")
    return subprocess.run(
        [PYTHON, "-m", "graph_fy", "extract", ".", *extra],
...
def test_code_only_succeeds_without_key(tmp_path):
    ...
    graph = repo / "graph_fy_out" / "graph.json"
...
def test_extract_usage_advertises_code_only(tmp_path):
    ...
    by triggering the no-key error. `graph_fy extract` with no path prints usage."""
    r = subprocess.run(
        [PYTHON, "-m", "graph_fy", "extract"],
...
def _run_relative_out(repo: Path, *extra: str):
    """Like _run but with a RELATIVE graph_fy_OUT so --out/--output controls the
    parent dir (an absolute graph_fy_OUT would override the flag)."""
    env = {k: v for k, v in os.environ.items() if k not in _KEY_VARS}
    env["graph_fy_OUT"] = "graph_fy_out"
    return subprocess.run(
        [PYTHON, "-m", "graph_fy", "extract", ".", *extra],
        cwd=repo, capture_output=True, text=True, env=env,
    )
...
def test_output_flag_is_alias_of_out(tmp_path):
    ...
    to the default `<path>/graph_fy_out/`). It is now an alias of `--out`."""
    repo = tmp_path / "repo"
    repo.mkdir()
    (repo / "app.py").write_text("def hello():\n    return 1\n")
    ...
    assert (custom / "graph_fy_out" / "graph.json").exists(), "--output was ignored (#2004)"
    assert not (repo / "graph_fy_out").exists(), "output must not go to the default dir"
...
def test_output_flag_inline_form(tmp_path):
    ...
    assert r.returncode == 0, r.stderr
    assert (custom / "graph_fy_out" / "graph.json").exists()


def test_no_gitignore_indexes_vcs_ignored_code_but_keeps_graph_fyignore(tmp_path):
    repo = tmp_path / "repo"
    ...
    (repo / "proj" / ".graph_fyignore").write_text("hidden/\n")
    ...
    graph = json.loads((repo / "graph_fy_out" / "graph.json").read_text())
...
def test_no_gitignore_setting_persists_across_flagless_extract(tmp_path):
    ...
    `graph_fy extract` must NOT clobber it back to honoring .gitignore (which
    ...
    def _sources():
        g = json.loads((repo / "graph_fy_out" / "graph.json").read_text())
...
def test_explicit_exclude_replaces_persisted_setting_with_custom_out(tmp_path):
    ...
    graph_out = out_root / "graph_fy_out"
    def _sources():
    ...
    assert json.loads((graph_out / ".graph_fy_build.json").read_text()) == {
        "excludes": ["generated"]
    }
...
def test_code_only_force_rescan_re_resolves_tsconfig_paths_and_preserves_semantics(tmp_path: Path):
    ...
    assert r1.returncode == 0, r1.stderr
    graph_path = repo / "graph_fy_out" / "graph.json"
