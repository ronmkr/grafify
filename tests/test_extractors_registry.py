...
own ``graph_fy/extractors/<lang>.py`` module, kept a verbatim re-export in
``graph_fy.extract`` (the facade every existing importer uses), and seeded a
``graph_fy.extractors.LANGUAGE_EXTRACTORS`` registry. Three things must stay
true for that split to be behavior-preserving:

- the function is importable from its new per-language module,
- ``graph_fy.extract`` still re-exports the SAME function object (facade
...
import graph_fy.extract as facade
from graph_fy.extractors import LANGUAGE_EXTRACTORS
...
def test_every_registry_extractor_is_reexported_from_facade():
    ...
    assert not missing, f"registry extractors not re-exported from graph_fy.extract: {missing}"
