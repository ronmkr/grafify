"""A graph_fy skip must not end the whole hook (#2986).
...
entire hook process — so anything after graph_fy's end marker was silently
dropped on every root commit (HEAD~1 does not exist), every rebase/merge,
every linked worktree and every graph_fy_SKIP_HOOK=1.
"""
...
from graph_fy.hooks import _CHECKOUT_MARKER_END, _HOOK_MARKER_END, install
...
def test_a_skipped_post_commit_still_runs_what_follows(tmp_path):
    ...
    r = _run(hook, repo, graph_fy_SKIP_HOOK="1")
    assert r.returncode == 0, r.stderr
    assert "AFTER_graph_fy" in r.stdout
...
def test_a_skipped_checkout_hook_still_runs_what_follows(tmp_path):
    repo = _repo(tmp_path)
    hook = _install_with_trailer(repo, "post-checkout", _CHECKOUT_MARKER_END)
    r = _run(hook, repo, "abc", "def", "1", graph_fy_SKIP_HOOK="1")
    assert r.returncode == 0, r.stderr
    assert "AFTER_graph_fy" in r.stdout
...
def test_a_preexisting_hook_before_the_block_still_runs_first(tmp_path):
    ...
    hook.write_text("#!/bin/sh\necho BEFORE_graph_fy\n", encoding="utf-8", newline="\n")
    install(repo)
    r = _run(hook, repo, graph_fy_SKIP_HOOK="1")
    assert r.returncode == 0, r.stderr
    assert "BEFORE_graph_fy" in r.stdout
