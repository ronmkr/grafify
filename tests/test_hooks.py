...
import pytest
from graph_fy.hooks import install, uninstall, status, _hooks_dir, _HOOK_MARKER, _CHECKOUT_MARKER
...
def test_hook_skips_head_on_exe():
    ...
    from graph_fy.hooks import _PYTHON_DETECT
...
def test_install_embeds_pinned_interpreter(tmp_path):
    ...
    graph_fy launcher on PATH (uv tool / pipx isolation, #1127).

    When graph_fy is installed via `uv tool install graph_fyy` or `pipx install
    graph_fyy`, the interpreter lives in an isolated venv and the launcher is in
    ~/.local/bin.  GUI git clients and CI runners often run with a minimal PATH
    that omits that directory, so `command -v graph_fy` fails, the python3/python
    fallbacks cannot import graph_fy (wrong venv), and the hook silently exits 0.
...
def test_hook_check_no_additionalContext(tmp_path):
    """graph_fy hook-check must not emit additionalContext — Codex Desktop rejects it."""
    import sys
    ...
    result = subprocess.run(
        [sys.executable, "-m", "graph_fy", "hook-check"],
        cwd=tmp_path,
        capture_output=True,
        text=True,
    )
...
from graph_fy.hooks import (  # noqa: E402
    _HOOK_SCRIPT,
    _CHECKOUT_SCRIPT,
...
def test_hooks_limit_windows_workers_by_default(name, script):
    ...
    assert 'export graph_fy_MAX_WORKERS="${graph_fy_MAX_WORKERS:-1}"' in script


def _launcher_payload(script: str) -> str:
    """Extract the `python -c "<payload>"` the hook hands to graph_fy_PYTHON.
...
def test_rebuild_bodies_read_graph_fy_root(name, body):
    """The rebuild must honour the persisted scan root rather than hardcoding the
    repo top (#1173). Both bodies read <output-dir>/.graph_fy_root and pass the
    recovered root to _rebuild_code instead of the bare Path('.')."""
    assert ".graph_fy_root" in body, f"{name} ignores .graph_fy_root (#1173)"
    # The output dir is resolved from graph_fy_OUT at hook-run time, not hardcoded
    # to graph_fy_out/, so a renamed output dir is still found (#1423).
    assert "graph_fy_OUT" in body, f"{name} ignores the graph_fy_OUT override (#1423)"
...
def test_rebuild_bodies_with_graph_fy_root_are_valid_python():
    """The .graph_fy_root snippet must parse so a quoting slip can't ship a hook
...
def _extract_root_resolution(body: str) -> str:
    """Pull the `.graph_fy_root` -> `_root` snippet out of a rebuild body, so a
...
def test_rebuild_bodies_reject_an_out_of_repo_graph_fy_root(name, body, tmp_path, monkeypatch):
    """#3265: `.graph_fy_root` sits inside graph_fy_out/, a directory the
    ...
    what gets read, and what gets written into the same committed graph_fy_out/
    ...
    (repo / "graph_fy_out").mkdir(parents=True)
    outside.mkdir()
    (repo / "graph_fy_out" / ".graph_fy_root").write_text(str(outside), encoding="utf-8")
...
def test_rebuild_bodies_honour_an_in_repo_graph_fy_root(name, body, tmp_path, monkeypatch):
    """The legitimate case the #3265 guard must not break: a subdirectory-scoped
    root (#1173) is still recovered."""
    repo = tmp_path / "repo"
    (repo / "graph_fy_out").mkdir(parents=True)
    (repo / "backend").mkdir()
    (repo / "graph_fy_out" / ".graph_fy_root").write_text("backend", encoding="utf-8")
    monkeypatch.chdir(repo)
    ns = {"Path": Path, "os": os}
    exec(compile(_extract_root_resolution(body), "<rebuild_body>", "exec"), ns)
...
def test_rebuild_bodies_survive_a_graph_fy_root_symlink_loop(name, body, tmp_path, monkeypatch):
    """A committed `.graph_fy_root` naming a path that resolves through a
    ...
    (repo / "graph_fy_out" / ".graph_fy_root").write_text("loop_a", encoding="utf-8")
    monkeypatch.chdir(repo)
    ns = {"Path": Path, "os": os}
    exec(compile(_extract_root_resolution(body), "<rebuild_body>", "exec"), ns)
...
def test_rebuild_bodies_arm_a_timeout_without_sigalrm(name, body):
    ...
    prefixes = set(re.findall(r"print\(f'\[([a-z_ ]+)\]", body))
...
def test_detached_launch_targets_graph_fy_python():
    """The launcher must run via the resolved $graph_fy_PYTHON, not a bare
    `python`, so it uses the same interpreter the detection block selected."""
    snippet = _detached_launch(_REBUILD_BODY_COMMIT)
    assert snippet.startswith('"$graph_fy_PYTHON" -c "')
...
def test_windows_hookspath_rejected_no_junk_dir_on_posix(tmp_path, monkeypatch, winpath):
    ...
    monkeypatch.setattr("graph_fy.hooks.os.name", "posix")
    repo = _make_git_repo(tmp_path)
...
def test_probes_use_find_spec_not_full_import():
    """`python -c "import graph_fy"` executes the FULL package import — 10s+ on a
    ...
    from graph_fy.hooks import _PYTHON_DETECT
    assert '-c "import graph_fy"' not in _PYTHON_DETECT, (
...
def test_shebang_read_is_null_byte_safe():
    """On Windows, `command -v graph_fy` can return the launcher path WITHOUT its
...
def test_probe_prefers_sibling_python_exe_on_windows_layouts():
    """pip on Windows puts Scripts/graph_fy(.exe) beside ..\\python.exe (or
...
def _detect_run(tmp_path, home, stub_bin, env_extra=None):
    ...
    environment — dead pin, no .graph_fy_python, an unparseable launcher first
    on PATH, and a python3 that cannot import graph_fy (#2852's uv-tool
    Windows machine reproduced on POSIX) — and report what graph_fy_PYTHON
...
def _broken_uv_machine(tmp_path):
    """#2852's machine: the only graph_fy-importable python lives in the uv
    tool venv; the launcher on PATH is a binary trampoline reached WITHOUT its
    .exe suffix (Git-Bash command -v); ambient python3 cannot import graph_fy.
    Returns (home, stub_bin)."""
    home = tmp_path / "home"
    stub_bin = tmp_path / "stubbin"
    stub_bin.mkdir(parents=True)
    launcher = stub_bin / "graph_fy"
    launcher.write_bytes(b"MZ\x90\x00\x03" + bytes(range(60)))
    launcher.chmod(0o755)
    # Ambient pythons answer the probe with "no module named graph_fy" —
...
def _tool_venv(home, tool, rel, ok):
    ...
    ok=False simulates a venv without graph_fy (the probe must reject it)."""
...
def test_uv_tool_env_rescues_hook_when_pin_and_launcher_fail(tmp_path):
    """#2852: `uv tool install` puts graph_fy in an isolated venv no ambient
    ...
    mine = _tool_venv(home, "graph_fyy", "bin/python", ok=True)
...
def test_uv_tool_env_honors_uv_tool_dir_and_windows_layout(tmp_path):
    ...
    py = tools / "graph_fyy" / "Scripts" / "python.exe"
...
@pytest.mark.skipif(shutil.which("sh") is None, reason="sh required to run emitted probe chain")
def test_uv_tool_env_without_graph_fy_still_fails_loudly(tmp_path):
    """The scan must not adopt a tool env whose python lacks graph_fy; the
...
def test_file_path_allowlist_accepts_windows_backslash_path(winpath, tmp_path):
    """#2126: the .graph_fy_python FILE allowlist must accept real Windows paths
...
def test_shebang_allowlist_accepts_windows_backslash_path(shebang_path, tmp_path):
    ...
    pattern = _extract_case_pattern('graph_fy_PYTHON="" ;;')
    _assert_harness_can_reject(pattern, tmp_path)
...
def test_python_detect_allowlists_still_reject_shell_metacharacters(dangerous, tmp_path):
    ...
    for marker in ('_FROM_FILE=""', 'graph_fy_PYTHON="" ;;'):
...
def test_hooks_honor_skip_env(name, script):
    """graph_fy_SKIP_HOOK=1 must suppress BOTH hooks. post-checkout previously
    lacked the check, so the var stopped commit rebuilds but not branch-switch
    ones (#1809)."""
    assert '[ "${graph_fy_SKIP_HOOK:-0}" = "1" ] && exit 0' in script, (
        f"{name} does not honor graph_fy_SKIP_HOOK"
    )
...
def test_checkout_hook_skips_same_head_noop_at_runtime():
    ...
    from graph_fy.hooks import _CHECKOUT_SCRIPT
    ...
    # graph_fy_out check / detached launch so nothing is actually rebuilt.
...
def _worktree_guard_snippet() -> str:
    from graph_fy.hooks import _WORKTREE_GUARD
...
def test_install_registers_merge_driver(tmp_path):
    """install() must set merge.graph_fy.* via git config and add the
    .gitattributes line that README/CHANGELOG 0.7.0 document (#1902)."""
    repo = _make_git_repo(tmp_path)
    result = install(repo)
    res = subprocess.run(
        ["git", "-C", str(repo), "config", "--get", "merge.graph_fy.driver"],
        capture_output=True, text=True,
    )
    ...
    assert any(
        "graph.json" in line and "merge=graph_fy" in line
...
def test_install_merge_driver_idempotent(tmp_path):
    ...
    matches = [l for l in lines if "merge=graph_fy" in l]
...
def test_install_preserves_existing_gitattributes(tmp_path):
    ...
    assert "merge=graph_fy" in content


def test_uninstall_removes_merge_driver_keeps_other_attrs(tmp_path):
    """uninstall() must unset merge.graph_fy.* and remove only the graph_fy
    ...
    content = (repo / ".gitattributes").read_text(encoding="utf-8")
    assert "*.png binary" in content
    assert "merge=graph_fy" not in content


@pytest.mark.parametrize("exe", [
    r"C:\Users\First Last\AppData\Roaming\uv\tools\graph_fyy\Scripts\python.exe",
    r"C:\Program Files\Python312\python.exe",
    "/home/first last/.local/share/uv/tools/graph_fyy/bin/python",
])
def test_pinned_python_accepts_paths_containing_spaces(exe, monkeypatch):
    ...
    from graph_fy.hooks import _pinned_python
...
def test_merge_driver_quotes_interpreter_with_spaces(tmp_path, monkeypatch):
    ...
    from graph_fy.hooks import install

    exe = r"C:\Users\First Last\AppData\Roaming\uv\tools\graph_fyy\Scripts\python.exe"
    repo = _make_git_repo(tmp_path)
    monkeypatch.setattr(_sys, "executable", exe)
    install(repo)
    ...
    assert driver.endswith("-m graph_fy merge-driver %O %A %B")
...
def test_graph_fyrc_parsing(tmp_path):
    """Test 1: .graph_fyrc parsing for valid and invalid values."""
    from graph_fy.hooks import _load_graph_fyrc

    rc = tmp_path / ".graph_fyrc"
    rc.write_text("# comment\nviz_node_limit=0\n", encoding="utf-8")
    cfg = _load_graph_fyrc(tmp_path)
    ...
    with pytest.raises(ValueError, match="Invalid viz_node_limit"):
        _load_graph_fyrc(tmp_path)


def test_no_config_preserves_existing_hook(tmp_path):
    """Test 2: Without .graph_fyrc, generated hooks omit graph_fy_VIZ_NODE_LIMIT export."""
    repo = _make_git_repo(tmp_path)
    install(repo)
    commit_hook = (repo / ".git" / "hooks" / "post-commit").read_text()
    checkout_hook = (repo / ".git" / "hooks" / "post-checkout").read_text()
    assert "graph_fy_VIZ_NODE_LIMIT" not in commit_hook
    assert "graph_fy_VIZ_NODE_LIMIT" not in checkout_hook


def test_config_baked_into_generated_hook(tmp_path):
    """Test 3: viz_node_limit from .graph_fyrc is baked into both hooks."""
    repo = _make_git_repo(tmp_path)
    (repo / ".graph_fyrc").write_text("viz_node_limit=0\n", encoding="utf-8")
    install(repo)

    commit_hook = (repo / ".git" / "hooks" / "post-commit").read_text()
    checkout_hook = (repo / ".git" / "hooks" / "post-checkout").read_text()

    assert 'export graph_fy_VIZ_NODE_LIMIT="${graph_fy_VIZ_NODE_LIMIT:-0}"' in commit_hook
    assert 'export graph_fy_VIZ_NODE_LIMIT="${graph_fy_VIZ_NODE_LIMIT:-0}"' in checkout_hook
...
def test_baked_viz_limit_yields_to_an_explicit_per_run_override(tmp_path):
    ...
    graph_fy_VIZ_NODE_LIMIT: the baked line uses the `${VAR:-<n>}` default form,
    so an already-set env value wins (mirrors graph_fy_MAX_WORKERS)."""
    repo = _make_git_repo(tmp_path)
    (repo / ".graph_fyrc").write_text("viz_node_limit=100\n", encoding="utf-8")
    install(repo)
    commit_hook = (repo / ".git" / "hooks" / "post-commit").read_text()

    # default form, not an unconditional assignment that would override the env
    assert 'export graph_fy_VIZ_NODE_LIMIT="${graph_fy_VIZ_NODE_LIMIT:-100}"' in commit_hook
    assert 'export graph_fy_VIZ_NODE_LIMIT="100"' not in commit_hook
    # prove the shell semantics: an explicit env value survives the export line
    line = 'export graph_fy_VIZ_NODE_LIMIT="${graph_fy_VIZ_NODE_LIMIT:-100}"'
    out = subprocess.run(
        ["sh", "-c", f'graph_fy_VIZ_NODE_LIMIT=7; {line}; echo "$graph_fy_VIZ_NODE_LIMIT"'],
        capture_output=True, text=True, check=True,
    )
    assert out.stdout.strip() == "7"


def test_status_survives_a_malformed_graph_fyrc(tmp_path):
    """A typo in the committed .graph_fyrc must not turn the read-only `status`
    diagnostic into a traceback; it reports the problem and continues."""
    repo = _make_git_repo(tmp_path)
    install(repo)
    (repo / ".graph_fyrc").write_text("viz_node_limit=not-an-int\n", encoding="utf-8")
...
def test_changing_config_updates_existing_hook(tmp_path):
    """Test 4: Re-running install updates existing graph_fy hook block with new config."""
    repo = _make_git_repo(tmp_path)
    rc = repo / ".graph_fyrc"
    ...
    commit_hook = (repo / ".git" / "hooks" / "post-commit").read_text()
    assert 'export graph_fy_VIZ_NODE_LIMIT="${graph_fy_VIZ_NODE_LIMIT:-0}"' in commit_hook
    assert 'export graph_fy_VIZ_NODE_LIMIT="${graph_fy_VIZ_NODE_LIMIT:-5000}"' not in commit_hook
    assert commit_hook.count("# graph_fy-hook-start") == 1


def test_user_hook_content_survives_update(tmp_path):
    """Test 5: User hook content outside graph_fy markers survives hook update."""
    repo = _make_git_repo(tmp_path)
    ...
    assert 'export graph_fy_VIZ_NODE_LIMIT="${graph_fy_VIZ_NODE_LIMIT:-0}"' in content
    assert "old block" not in content


def test_status_reports_configuration(tmp_path):
    """Test 6: graph_fy hook status exposes configured viz node limit and detects out-of-date hooks."""
    repo = _make_git_repo(tmp_path)
    rc = repo / ".graph_fyrc"
    rc.write_text("viz_node_limit=0\n", encoding="utf-8")
    install(repo)
...
def test_both_hooks_configured(tmp_path):
    ...
    repo = _make_git_repo(tmp_path)
    (repo / ".graph_fyrc").write_text("viz_node_limit=42\n", encoding="utf-8")
    install(repo)

    for name in ("post-commit", "post-checkout"):
        hook_text = (repo / ".git" / "hooks" / name).read_text()
        assert 'export graph_fy_VIZ_NODE_LIMIT="${graph_fy_VIZ_NODE_LIMIT:-42}"' in hook_text
...
def test_rebuild_bodies_tolerate_a_bom_in_graph_fy_root(name, body):
    ...
    assert "encoding='utf-8-sig'" in body, (
        f"{name} rebuild body must read .graph_fy_root BOM-tolerantly"
    )
...
@pytest.mark.parametrize("exe", [
    "/home/dev/snap/code/259/.local/share/uv/tools/graph_fyy/bin/python",
    "/home/dev/snap/code/current/.local/share/uv/tools/graph_fyy/bin/python",
    "/root/snap/pycharm-community/42/.local/share/uv/tools/graph_fyy/bin/python",
])
def test_pinned_python_refuses_a_rotating_snap_revision(exe, monkeypatch):
    ...
    graph_fy installed from inside a snap-confined editor lives under
    ...
    not locate a Python with graph_fy installed" and the graphs quietly stopped
    tracking the code.
...
def test_pinned_python_still_pins_a_stable_path(monkeypatch):
    ...
    for exe in (
        "/home/dev/.local/share/uv/tools/graph_fyy/bin/python",
        "/usr/bin/python3",
...
def test_hook_probes_snap_confined_uv_tool_dirs():
    ...
    """
    from graph_fy.hooks import _HOOK_SCRIPT

    assert '"$HOME"/snap/*/[0-9]*/.local/share/uv/tools' in _HOOK_SCRIPT
    assert '"$HOME"/snap/*/current/.local/share/uv/tools' in _HOOK_SCRIPT


# ── graph_fy_OUT: the shell gates in front of the rebuild must honour it ─────
#
# #1423 moved the Python rebuild bodies onto graph_fy_OUT, but the sh that runs
# BEFORE them still hardcoded graph_fy_out/: post-checkout exited unless a
# literal graph_fy_out/ directory existed, and post-commit's "only graph
# artifacts changed" filter only recognised graph_fy_out/ paths. With the
...
def _emitted_hook_run(repo: Path, script: str, args: list[str], env_extra: dict[str, str]):
    ...
    for key in ("GIT_DIR", "graph_fy_OUT", "graph_fy_SKIP_HOOK"):
        env.pop(key, None)
    env.update(env_extra)
    return subprocess.run(
...
@_SH_ONLY
def test_checkout_hook_rebuilds_when_graph_fy_out_is_renamed(tmp_path):
    repo = tmp_path / "repo"
    repo.mkdir()
    _git(repo, "init", "-q")
    (repo / "custom-out").mkdir()  # the configured output dir; no graph_fy_out/
    result = _emitted_hook_run(repo, _CHECKOUT_SCRIPT, ["aaa", "bbb", "1"], {"graph_fy_OUT": "custom-out"})
    assert result.returncode == 0, result.stderr
    assert _LAUNCH_LINE in result.stdout, (
        "post-checkout exited before the launch: it gates on a literal graph_fy_out/ "
        f"instead of $graph_fy_OUT\nstdout={result.stdout!r}\nstderr={result.stderr!r}"
    )
...
@_SH_ONLY
def test_commit_hook_skips_graph_only_commit_when_graph_fy_out_is_renamed(tmp_path):
    repo = _repo_with_graph_only_commit(tmp_path, "custom-out")
    result = _emitted_hook_run(repo, _HOOK_SCRIPT, [], {"graph_fy_OUT": "custom-out"})
    assert result.returncode == 0, result.stderr
    assert _LAUNCH_LINE not in result.stdout, (
...
def test_commit_hook_skips_graph_only_commit_under_default_dir(tmp_path):
    ...
    repo = _repo_with_graph_only_commit(tmp_path, "graph_fy_out")
