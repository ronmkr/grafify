...
`_load_dir_own_ignore` / `_load_graph_fyignore` read .gitignore,
.graph_fyignore and $GIT_DIR/info/exclude with `errors="ignore"`, which turns a
...
def test_ansi_encoded_rule_still_excludes(tmp_path):
    """The reported case: a cp1252 .graph_fyignore must still exclude."""
...
def test_ansi_encoded_rule_warns_once_naming_the_file(tmp_path, capsys):
    import graph_fy.detect as detect_mod
    detect_mod._warned_ignore_encodings.clear()
    _corpus(tmp_path, f"{NAME}/\n".encode("cp1252"))
    detect(tmp_path)
    err = capsys.readouterr().err
    assert ".graph_fyignore" in err and "UTF-8" in err, err
...
def test_ascii_rules_are_untouched(tmp_path):
    (tmp_path / ".graph_fyignore").write_bytes(b"vendor/\n")
...
def test_utf16_bom_encoded_rule_still_excludes(tmp_path):
    """A UTF-16 (BOM) .graph_fyignore — what PowerShell Set-Content and Notepad
