...
from graphify.impact import compute_impact, format_impact_report, resolve_targets
...
def test_cli_impact_and_blast(monkeypatch, tmp_path, capsys):
    ...
    # 1. Test "graphify impact AuthService"
    monkeypatch.setattr(
        mainmod.sys,
        "argv",
        ["graphify", "impact", "AuthService", "--graph", str(graph_path)],
    )
    mainmod.main()
    ...
    # 2. Test "graphify blast AuthService --terse"
    monkeypatch.setattr(
        mainmod.sys,
        "argv",
        ["graphify", "blast", "AuthService", "--terse", "--graph", str(graph_path)],
    )
    mainmod.main()
    out_terse = capsys.readouterr().out
    assert out_terse.startswith("Impact for AuthService:")

    # 3. Test "graphify impact AuthService --json"
    monkeypatch.setattr(
        mainmod.sys,
        "argv",
        ["graphify", "impact", "AuthService", "--json", "--graph", str(graph_path)],
    )
    mainmod.main()
...
def test_cli_impact_missing_target(monkeypatch, capsys):
    monkeypatch.setattr(mainmod, "_check_skill_version", lambda _: None)
    monkeypatch.setattr(mainmod.sys, "argv", ["graphify", "impact"])

    with pytest.raises(SystemExit) as exc:
        mainmod.main()
    assert exc.value.code == 1
    err = capsys.readouterr().err
    assert "Usage: graphify impact <target>" in err


def test_graphify_mcp_module_impact(tmp_path):
    from graphify.mcp import get_impact_analysis, get_blast_radius, IMPACT_TOOL_SPEC, BLAST_TOOL_SPEC
