"""Tests for the incomplete-build shrink-guard on `graph_fy extract`.
...
def _seed_to_json_recorder(monkeypatch, *, returns=True):
    ...
    monkeypatch.setattr("graph_fy.export.to_json", _stub)
    return rec
...
def _arm_extract(monkeypatch, tmp_path, *, chunk_total, chunk_succeeded, extra_argv=()):
    ...
    monkeypatch.setattr("graph_fy.llm.extract_corpus_parallel", _stub_corpus)
...
def test_partial_extraction_refuses_to_shrink_existing_graph(monkeypatch, tmp_path, capsys):
    ...
    assert not (out_dir / "graph_fy_out" / "manifest.json").exists()
...
def _arm_no_cluster(monkeypatch, tmp_path, *, extra_argv=()):
    ...
    monkeypatch.setattr(
        mainmod.sys, "argv",
        ["graph_fy", "extract", str(corpus), "--backend", "claude", "--no-cluster",
         "--out", str(out_dir), *extra_argv],
    )
