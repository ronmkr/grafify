"""Integration tests for incremental graph_fy extract behavior."""
...
def test_manifest_written_after_extract(tmp_path):
    ...
    manifest = docs / "graph_fy_out" / "manifest.json"
...
def test_extract_no_cluster_incremental_noop_preserves_existing_graph(tmp_path):
    ...
    graph_path = project / "graph_fy_out" / "graph.json"
...
def test_incremental_python_relative_import_target_canonicalizes(tmp_path):
    ...
    from graph_fy.extract import extract, _make_id
...
def test_update_prunes_a_removed_imports_edge(tmp_path):
    """#1521: when an import is deleted from a file, `graph_fy update` must prune
