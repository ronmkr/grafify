...
A plain `graph_fy install` refreshes only the detected platform, so a stale
`.graph_fy_version` at ANOTHER platform's destination made the warning
permanent: it named no path, its advice ("run graph_fy install") did not
...
from graph_fy.install import __version__
from graph_fy.install import _copy_skill_file, _platform_skill_destination
...
def _stale(tmp_path):
    d = tmp_path / "skills" / "graph_fy"
    d.mkdir(parents=True)
    (d / "SKILL.md").write_text("---\nname: graph_fy\n---\nbody\n", encoding="utf-8")
    (d / ".graph_fy_version").write_text("0.0.0", encoding="utf-8")
    return d / "SKILL.md"
...
def test_the_warning_names_the_stale_destination_and_the_exact_command(tmp_path, capsys):
    ...
    assert "0.0.0" in err and __version__ in err
    assert str(tmp_path / "skills" / "graph_fy") in err, "the stale path must be named"
    assert "graph_fy install --platform antigravity" in err
...
def test_without_a_platform_name_the_generic_command_is_kept(tmp_path, capsys):
    ...
    err = capsys.readouterr().err
    assert "graph_fy install" in err


def test_a_current_skill_stays_silent(tmp_path, capsys):
    skill = _stale(tmp_path)
    (skill.parent / ".graph_fy_version").write_text(__version__, encoding="utf-8")
...
def test_the_version_stamp_is_written_beside_the_skill(sandbox_home):
    _copy_skill_file("claude")
    dst = _platform_skill_destination("claude")
    assert (dst.parent / ".graph_fy_version").read_text(encoding="utf-8") == __version__
