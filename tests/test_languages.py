...
def test_cpp_string_tests_punctuation_only_names_stay_distinct(tmp_path):
    ...
    r = extract_cpp(f)
    from graph_fy.extract import _make_id, _file_stem
...
def test_kotlin_emits_in_file_calls():
    ...
    `identifier` rename — see graph_fy-kmp's PythonParityTest."""
    r = extract_kotlin(FIXTURES / "sample.kt")
    calls = _calls(r)
...
def test_swift_extension_across_files_merges_into_canonical_type():
    ...
    r = extract(paths, cache_root=Path("/tmp/graph_fy-test-no-cache"))
...
from graph_fy.extract import extract_elixir
...
# ── Objective-C ──────────────────────────────────────────────────────────────
from graph_fy.extract import extract_objc
...
def test_objc_header_dispatch_routes_objc_not_c(tmp_path):
    ...
    from graph_fy.extract import _get_extractor, extract_objc as _eo, extract_c as _ec
...
from graph_fy.extract import extract_markdown
...
def test_markdown_frontmatter_fallback_parses_flat_keys():
    ...
    from graph_fy.extractors.markdown import _parse_frontmatter_fallback
...
def test_markdown_heading_id_is_stable_regardless_of_frontmatter():
    ...
    from graph_fy.extractors.base import _make_id, _file_stem
    r = _md_extract("---\ntitle: Doc\ntags: [a, b]\n---\n\n# Overview\n\n## Details\n")
...
# ── Zig ───────────────────────────────────────────────────────────────────────
from graph_fy.extract import extract_zig
...
def test_cl_ids_are_path_qualified_across_directories(tmp_path):
    ...
    they land in separate extract batches (what `graph_fy update` does) build()
...
def test_robot_keyword_call_edges_incl_fixtures_and_loops():
    from graph_fy.extractors.robot import _kw_id
    r = extract_robot(FIXTURES / "sample.robot")
...
def test_robot_curdir_and_execdir_imports_resolve_without_double_prefix():
    ...
    from graph_fy.extractors.robot import _resolve_robot_import
...
def test_robot_extractor_degrades_gracefully_without_robotframework():
    ...
    # graph_fy.extract must work and the extractor must return the
    ...
    script = (
        ...
        "from pathlib import Path\n"
        "from graph_fy.extract import extract_robot\n"
