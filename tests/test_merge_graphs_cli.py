"""`graph_fy merge-graphs` tolerates inputs that disagree on graph type (#1606).
...
def _run(args, cwd):
    return subprocess.run([PYTHON, "-m", "graph_fy"] + args, cwd=cwd,
...
def test_merge_graphs_mixed_directed_and_multigraph(tmp_path):
    a = tmp_path / "r1" / "graph_fy_out" / "graph.json"
    b = tmp_path / "r2" / "graph_fy_out" / "graph.json"
    c = tmp_path / "r3" / "graph_fy_out" / "graph.json"
...
def test_merge_graphs_same_named_repo_dirs_do_not_collapse(tmp_path):
    # #1729: two graphs under a same-named repo dir (src/graph_fy_out and
    # frontend/src/graph_fy_out both → tag "src") share the `src::` prefix, so a
    ...
    a = tmp_path / "src" / "graph_fy_out" / "graph.json"
    b = tmp_path / "frontend" / "src" / "graph_fy_out" / "graph.json"
...
def test_distinct_repo_tags_unit(tmp_path):
    ...
    assert distinct_repo_tags([
        Path("backend/graph_fy_out/graph.json"),
        Path("web/graph_fy_out/graph.json"),
    ]) == ["backend", "web"]
    # same-named repo dirs are widened to stay distinct
    tags = distinct_repo_tags([
        Path("proj/src/graph_fy_out/graph.json"),
        Path("proj/frontend/src/graph_fy_out/graph.json"),
    ])
    ...
    tags3 = distinct_repo_tags([
        Path("a/src/graph_fy_out/graph.json"),
        Path("b/src/graph_fy_out/graph.json"),
        Path("c/src/graph_fy_out/graph.json"),
    ])
...
def test_merge_graphs_preserves_import_edge_direction(tmp_path):
    ...
    a = tmp_path / "repo1" / "graph_fy_out" / "graph.json"
    b = tmp_path / "repo2" / "graph_fy_out" / "graph.json"
    a.parent.mkdir(parents=True, exist_ok=True)
    b.parent.mkdir(parents=True, exist_ok=True)
...
def test_merge_graphs_carries_hyperedges_from_all_inputs(tmp_path):
    ...
    a = tmp_path / "alpha" / "graph_fy_out" / "graph.json"
    b = tmp_path / "beta" / "graph_fy_out" / "graph.json"
