...
#1506 added `.graph_fy_obsidian_manifest.json` so a re-export can update its own
notes while refusing to touch the user's. A vault created by an EARLIER graph_fy
has no manifest, so `_owned` starts empty and every note graph_fy itself wrote
last time reads as a user file: the re-export writes fresh notes beside the
stale ones, and warns that graph_fy "did not create" files it did (#2863).

The fix adopts, once, the notes carrying graph_fy's own signature:

  * node notes    - YAML frontmatter with a tag in the ``graph_fy/`` namespace
  * community notes - no frontmatter at all, so matched on graph_fy's own
                      filename prefix AND the Dataview query it writes

`.obsidian/graph.json` is deliberately NOT adopted: graph_fy writes one, but so
...
from graph_fy.export import to_obsidian

try:
    from graph_fy.export import _adopt_pre_manifest_notes, _is_graph_fy_note
except ImportError:  # pre-fix tree
    _adopt_pre_manifest_notes = _is_graph_fy_note = None

MANIFEST = ".graph_fy_obsidian_manifest.json"
...
def pre_manifest_vault(tmp_path):
    """A vault as an older graph_fy would have left it: its notes, no manifest."""
...
def test_graph_fy_no_longer_claims_it_did_not_write_its_own_notes(pre_manifest_vault):
...
def test_a_note_merely_mentioning_graph_fy_is_not_adopted(pre_manifest_vault):
    mine = pre_manifest_vault / "Notes.md"
    mine.write_text("# Notes\n\nI use graph_fy/document tags manually.\n", encoding="utf-8")
    _export(pre_manifest_vault, ["Alpha", "Beta", "Gamma"])
    assert "I use graph_fy" in mine.read_text(encoding="utf-8")
...
def test_obsidian_config_is_not_adopted(pre_manifest_vault):
    """graph_fy writes .obsidian/graph.json, but so does Obsidian. With no
...
@pytest.mark.skipif(_is_graph_fy_note is None, reason="pre-fix tree")
def test_detector_accepts_a_graph_fy_node_note(tmp_path):
    p = tmp_path / "n.md"
    p.write_text('---\nsource_file: "a.md"\ntags:\n  - graph_fy/document\n---\n\n# N\n',
                 encoding="utf-8")
    assert _is_graph_fy_note(p)
...
def test_detector_rejects_everything_else(tmp_path, body):
    ...
    assert not _is_graph_fy_note(p)
