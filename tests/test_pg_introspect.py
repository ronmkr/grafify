...
def test_pg_introspect_import_error():
    ...
    # #1906: the PyPI package is graph_fyy (double-y), so the install hint must match
    assert "graph_fyy[postgres]" in str(exc_info.value)
...
def test_pg_introspect_grammar_error_surfaces():
    ...
    with patch.dict("sys.modules", {"psycopg": mock_psycopg}):
        with patch(
            "graph_fy.pg_introspect.extract_sql",
            return_value={
                "nodes": [],
                "edges": [],
