...
On graph_fy's own package, pruning `graph_fy/callflow_html.py` removed 137 of its
139 nodes and stranded `graph_fy_callflow_html_py_path` (label `Path`) and
`graph_fy_callflow_html_py_counter` (label `Counter`) permanently.
...
from graph_fy.build import build_from_json, build_merge
