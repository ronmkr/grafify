"""Tests for `graph_fy reflect` and the work-memory reflection layer.

`graph_fy reflect` reads the outcome-tagged Q&A docs that `graph_fy save-result`
files into graph_fy_out/memory/ and writes a deterministic lessons artifact
(graph_fy_out/reflections/LESSONS.md) an agent can load next session: preferred
...
from graph_fy.ingest import save_query_result
from graph_fy.reflect import (
    aggregate_lessons,
    lessons_fresh,
    load_memory_docs,
    parse_memory_doc,
...
def _write_raw_doc(mem: Path, filename: str, date: str, *, outcome="dead_end",
    ...
    lines = ["---", 'type: "query"', f'date: "{date}"', f'question: "{question}"',
             'contributor: "graph_fy"', f'outcome: "{outcome}"']
    if nodes:
...
def test_cli_reflect_end_to_end(tmp_path):
    ...
    lessons = cwd / "graph_fy_out" / "reflections" / "LESSONS.md"
...
def test_cli_save_result_reads_answer_from_file(tmp_path):
    ...
    docs = list((tmp_path / "graph_fy_out" / "memory").glob("*.md"))
...
def test_cli_reflect_cold_start_writes_empty_lessons(tmp_path):
    """First run with no graph_fy_out/memory/ still succeeds and writes a valid doc."""
    r = _run(["reflect"], tmp_path)
    assert r.returncode == 0, r.stderr
    assert "Reflected 0 memories" in r.stdout
    lessons = tmp_path / "graph_fy_out" / "reflections" / "LESSONS.md"
    assert lessons.exists()
...
def _make_graph(tmp_path: Path) -> Path:
    """Build a minimal graph.json + analysis/labels in tmp_path/graph_fy_out/.
    ...
    (out / ".graph_fy_analysis.json").write_text(json.dumps({
...
@pytest.mark.parametrize("sidecar_name", [".graph_fy_analysis.json", ".graph_fy_labels.json"])
def test_lessons_fresh_false_when_graph_sidecar_newer(tmp_path, sidecar_name):
    ...
    out = tmp_path / "LESSONS.md"; out.write_text("y", encoding="utf-8")
    graph = tmp_path / "graph.json"; graph.write_text("{}", encoding="utf-8")
    analysis = tmp_path / ".graph_fy_analysis.json"; analysis.write_text("{}", encoding="utf-8")
    labels = tmp_path / ".graph_fy_labels.json"; labels.write_text("{}", encoding="utf-8")
...
# --- work-memory overlay sidecar (.graph_fy_learning.json) --------------------
#
...
from graph_fy.reflect import (  # noqa: E402
...
def test_sidecar_write_classifies_and_keys_by_canonical_id(tmp_path):
    """reflect with a graph writes .graph_fy_learning.json next to graph.json with
...
def test_relative_source_file_not_spuriously_stale_in_graph_fy_out_layout(tmp_path):
    """Regression: with a RELATIVE source_file and graph.json under graph_fy_out/,
    ...
    (graph_fy_out/) — otherwise every node looked unfindable and was marked stale.
    The edit case must still flip stale=True."""
    out = tmp_path / "graph_fy_out"          # graph.json lives here
...
def test_relative_source_file_resolved_via_graph_fy_root_marker(tmp_path):
    """When a committed .graph_fy_root marker records the project root (e.g. a
    graph_fy_OUT override pointing the output dir elsewhere), the fingerprint
    ...
    (out / ".graph_fy_root").write_text(str(proj), encoding="utf-8")  # the marker
    mem = out / "memory"
    _write_raw_doc(mem, "a.md", "2026-05-01", outcome="useful", nodes=["login()"])
    _write_raw_doc(mem, "b.md", "2026-05-10", outcome="useful", nodes=["login()"])
    reflect(mem, out / "reflections" / "LESSONS.md",
...
def test_flat_layout_does_not_match_same_named_file_one_dir_up(tmp_path):
    ...
    # Flat layout: graph.json sits directly in proj/ (not a graph_fy_out subdir).
