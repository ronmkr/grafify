"""#1688 - graph_fy's shared-file section update must not destroy user content.

_replace_or_append_section used to locate its marker (`## graph_fy`) as a
...
from graph_fy.__main__ import _replace_or_append_section

MARKER = "## graph_fy"
NEW = "## graph_fy\n\nThis project has a knowledge graph at graph_fy_out/.\n"
...
def test_inline_reference_to_marker_is_not_treated_as_the_section():
    before = (
        "# My Project\n\n"
        "## Setup\n"
        "- See the `## graph_fy` section for graph usage.\n\n"
        "## Release Process\n"
        "Critical steps that must not be lost.\n"
    )
    after = _replace_or_append_section(before, MARKER, NEW)
    assert "See the `## graph_fy` section" in after       # bullet preserved
    assert "Critical steps that must not be lost" in after  # later section preserved
    assert "knowledge graph at graph_fy_out/" in after      # section still added
...
def test_real_section_is_replaced_in_place():
    ...
    assert "knowledge graph at graph_fy_out/" in after
