...
Since #522 it runs as the shell-agnostic `graph_fy hook-guard search` subcommand
...
from graph_fy.__main__ import _claude_pretooluse_hooks
...
def _env():
    e = dict(os.environ)
    e.pop("graph_fy_OUT", None)
    return e


def _run(command, cwd, *, graph: bool):
    if graph:
        (cwd / "graph_fy_out").mkdir(parents=True, exist_ok=True)
        (cwd / "graph_fy_out" / "graph.json").write_text("{}", encoding="utf-8")
    stdin = json.dumps({"tool_input": {"command": command}})
    return subprocess.run(
        [sys.executable, "-m", "graph_fy", "hook-guard", "search"],
...
def test_hook_command_has_no_backslashes(monkeypatch):
    ...
    # escape character and strips it (C:\Users\me\graph_fy.EXE -> C:Usersme...),
    # breaking every guard. The emitted command must use forward slashes.
    from graph_fy.__main__ import _resolve_graph_fy_exe
    monkeypatch.setattr("shutil.which", lambda _name: r"C:\Users\me\graph_fy.EXE")
    assert _resolve_graph_fy_exe() == "C:/Users/me/graph_fy.EXE"
...
def test_command_has_no_shell_syntax():
    ...
    assert "graph_fy" in cmd and "hook-guard search" in cmd
...
def test_nudges_on_search_commands_with_graph(tmp_path):
    for command in (
        ...
        assert "graph_fy query" in out, f"{command!r} should nudge"
...
def test_nudge_payload_is_valid_pretooluse_json(tmp_path):
    ...
    assert "graph_fy query" in payload["hookSpecificOutput"]["additionalContext"]


def test_fails_open_on_malformed_stdin(tmp_path):
    (tmp_path / "graph_fy_out").mkdir()
    (tmp_path / "graph_fy_out" / "graph.json").write_text("{}", encoding="utf-8")
    r = subprocess.run(
        [sys.executable, "-m", "graph_fy", "hook-guard", "search"],
...
def test_honors_graph_fy_out_override(tmp_path):
    """The guard resolves the graph via graph_fy_OUT, not a hardcoded path."""
    custom = tmp_path / "custom-out"
    custom.mkdir()
    (custom / "graph.json").write_text("{}", encoding="utf-8")
    env = dict(os.environ, graph_fy_OUT=str(custom))
    stdin = json.dumps({"tool_input": {"command": "grep -rn foo ."}})
    r = subprocess.run(
        [sys.executable, "-m", "graph_fy", "hook-guard", "search"],
        input=stdin, capture_output=True, text=True, cwd=tmp_path, env=env,
    )
    assert "graph_fy query" in r.stdout
...
def test_grep_tool_input_nudges_with_graph(tmp_path):
    for tool_input in (
        ...
        assert "graph_fy query" in out, f"Grep input {tool_input!r} should nudge"
...
def test_bash_non_search_with_stray_pattern_key_does_not_nudge(tmp_path):
    ...
    (tmp_path / "graph_fy_out").mkdir(parents=True, exist_ok=True)
    (tmp_path / "graph_fy_out" / "graph.json").write_text("{}", encoding="utf-8")
