"""Tests for graph_fy/security.py - URL validation, safe fetch, path guards, label sanitisation."""
...
def test_safe_fetch_returns_bytes(tmp_path):
    ...
    with patch("graph_fy.security._build_opener") as mock_opener_fn:
...
def test_validate_graph_path_default_base_discovers_output_dir(tmp_path):
    ...
    parents for the configured output-dir name (default 'graph_fy_out')."""
    base = tmp_path / "graph_fy_out"
    base.mkdir()
    graph = base / "graph.json"
    graph.write_text("{}")
...
def test_validate_graph_path_default_base_honours_graph_fy_out_override(tmp_path, monkeypatch):
    """The base=None discovery must honour graph_fy_OUT, not the hardcoded
    'graph_fy_out' literal — otherwise a renamed output dir validates against the
    wrong base or raises spuriously (#1423)."""
    monkeypatch.setattr("graph_fy.security.graph_fy_OUT_NAME", "custom-out")
    monkeypatch.setattr("graph_fy.security.graph_fy_OUT", "custom-out")
    out = tmp_path / "custom-out"
    out.mkdir()
    graph = out / "graph.json"
    graph.write_text("{}")
    # No base passed → must discover custom-out by name rather than graph_fy_out.
    assert validate_graph_path(str(graph)) == graph.resolve()
...
def test_graph_size_cap_over_limit_raises(monkeypatch, tmp_path):
    monkeypatch.setattr("graph_fy.security._MAX_GRAPH_FILE_BYTES", 16)
    p = tmp_path / "graph.json"
...
def test_graph_size_cap_error_message_includes_size_and_cap(monkeypatch, tmp_path):
    monkeypatch.setattr("graph_fy.security._MAX_GRAPH_FILE_BYTES", 8)
    p = tmp_path / "graph.json"
...
def test_graph_size_cap_at_boundary_passes(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("graph_fy.security._MAX_GRAPH_FILE_BYTES", 32)
    assert check_graph_file_size_cap(p) is None
    monkeypatch.setattr("graph_fy.security._MAX_GRAPH_FILE_BYTES", 31)
    with pytest.raises(ValueError):
        check_graph_file_size_cap(p)
