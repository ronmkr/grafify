...
#1990 — `graph_fy extract --out` saves recovery checkpoints in the wrong directory.
...
def _semantic_dir(root: Path, mode: str | None = None) -> Path:
    ...
    return root / "graph_fy_out" / "cache" / kind
...
def test_save_semantic_cache_no_corpus_graph_fy_out_created(tmp_path):
    """With cache_root set, no graph_fy_out/ dir should be created inside corpus."""
    corpus = tmp_path / "corpus"
    out = tmp_path / "out"
    corpus.mkdir()
    out.mkdir()
    ...
    assert not (corpus / "graph_fy_out").exists(), (
        "graph_fy_out/ must not be created inside the corpus when cache_root is set"
    )
