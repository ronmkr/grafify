...
def test_score_nodes_multiword_exact_label_outranks_superset():
    ...
    Regression for the `graph_fy path` "No path found" bug: every node sharing
...
def _force_full_scan(monkeypatch):
    ...
    monkeypatch.setattr("graph_fy.serve._trigram_candidates", lambda *a, **k: None)
...
def test_query_terms_filters_only_short_english_terms(monkeypatch):
    import graph_fy.serve as serve_mod
...
def test_load_graph_missing_file(tmp_path):
    graph_fy_dir = tmp_path / "graph_fy_out"
    graph_fy_dir.mkdir()
    with pytest.raises(SystemExit):
        _load_graph(str(graph_fy_dir / "nonexistent.json"))
...
def test_load_graph_corrupted_json_prints_recovery_message(tmp_path, capsys):
    ...
    assert "Re-run /graph_fy to rebuild" in err
...
def test_load_graph_accepts_under_cap(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("graph_fy.security._MAX_GRAPH_FILE_BYTES", 10 * 1024 * 1024)
    G2 = _load_graph(str(p))
    assert G2.number_of_nodes() == G.number_of_nodes()
...
def test_query_graph_text_context_filter_aliases_resolve():
    import networkx as nx
    from graph_fy.serve import _normalize_context_filters
...
def test_serve_import_is_clean_under_syntax_warnings(tmp_path):
    ...
    result = subprocess.run(
        [
            ...
            "-c", "import graph_fy.serve as serve; assert serve._jieba is not None",
        ],
        capture_output=True,
        text=True,
    )
...
def test_score_query_matches_legacy_under_full_scan_fallback(monkeypatch):
    ...
    monkeypatch.setattr(
        "graph_fy.serve._trigram_candidates", lambda G, needles: None
    )
...
def test_query_graph_text_makes_exactly_one_score_query_call(monkeypatch):
    ...
    monkeypatch.setattr("graph_fy.serve._score_query", counting_sq)
    monkeypatch.setattr("graph_fy.serve._score_nodes", counting_sn)
...
def test_snake_case_identifier_still_matches_itself():
    ...
    G = nx.Graph()
    G.add_node("n1", label="_query_terms", source_file="graph_fy/serve.py",
...
def test_resolve_single_node_shared_by_get_node_and_get_neighbors():
    ...
    from graph_fy.serve import _resolve_single_node
...
def test_score_nodes_rationale_does_not_count_toward_term_coverage():
    ...
    from graph_fy.serve import _EXACT_MATCH_BONUS
    G = nx.Graph()
