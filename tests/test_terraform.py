"""Tests for the Terraform/HCL extractor (graph_fy/extract.py, issue #187)."""
from __future__ import annotations

from pathlib import Path

from graph_fy.build import build_from_json
from graph_fy.extract import extract_terraform
...
def test_reference_edges(tmp_path):
    ...
    assert ("AWS:aws_instance.web", "data.aws_ami.ubuntu") in refs
    assert ("AWS:aws_instance.web", "var.region") in refs
    assert ("module.vpc", "local.cidr") in refs
    assert ("output.ip", "AWS:aws_instance.web") in refs


def test_depends_on_edge(tmp_path):
    r = extract_terraform(_write(tmp_path, "main.tf", SAMPLE))
    assert ("AWS:aws_instance.web", "AWS:aws_security_group.sg") in _rel_pairs(r, "depends_on")


def test_file_contains_blocks(tmp_path):
    r = extract_terraform(_write(tmp_path, "main.tf", SAMPLE))
    contains = _rel_pairs(r, "contains")
    assert ("main.tf", "AWS:aws_instance.web") in contains
...
"""
    r = extract_terraform(_write(tmp_path, "main.tf", body))
    node = next(n for n in r["nodes"] if n["label"] == "AWS:aws_s3_bucket.example")
...
"""
    r = extract_terraform(_write(tmp_path, "main.tf", body))
    node = next(n for n in r["nodes"] if n["label"] == "AWS:aws_security_group.sg")
    attrs = node.get("attributes")
    assert attrs is not None
...
"""
    r = extract_terraform(_write(tmp_path, "main.tf", body))
    bucket_node = next(n for n in r["nodes"] if n["label"] == "AWS:aws_s3_bucket.example")
    ...
    refs = _rel_pairs(r, "references")
    assert ("AWS:aws_s3_bucket.example", "var.bucket_name") in refs
    assert ("AWS:aws_s3_bucket.example", "data.aws_ami.ubuntu") in refs
...
"""
    r = extract_terraform(_write(tmp_path, "main.tf", body))
    node = next(n for n in r["nodes"] if n["label"] == "AWS:aws_instance.web")
    attrs = node.get("attributes")
    assert attrs is not None
...
def test_terraform_attributes_context_rendering(tmp_path):
    from graph_fy.serve import _subgraph_to_text
...
"""
    r = extract_terraform(_write(tmp_path, "main.tf", body))
    G = build_from_json(r)
    node_id = next(n["id"] for n in r["nodes"] if n["label"] == "AWS:aws_s3_bucket.example")
...
def test_terraform_attributes_round_trip_serialization(tmp_path):
    ...
    from graph_fy.serve import _load_graph
...
"""
    ...
    node = next(n for n in r["nodes"] if n["label"] == "AWS:aws_db_instance.main")
...
def test_terraform_redact_value_recurses_into_lists():
    ...
    from graph_fy.extractors.terraform import _redact_value
...
"""
    ...
    node = next(n for n in r["nodes"] if n["label"] == "AWS:aws_x.y")
