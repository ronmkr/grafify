"""Tests for graph_fy tree / tree_html.build_tree.
...
from graph_fy.tree_html import build_tree
...
def test_tree_cli_absolute_root_mismatch_exits_1(tmp_path):
    ...
    assert not (tmp_path / "graph_fy_out" / "GRAPH_TREE.html").exists()
...
def test_tree_cli_default_root_succeeds(tmp_path):
    ...
    html = (tmp_path / "graph_fy_out" / "GRAPH_TREE.html").read_text(encoding="utf-8")
...
def test_tree_cli_partial_match_root_succeeds(tmp_path):
    ...
    assert r.returncode == 0, r.stderr
    assert (tmp_path / "graph_fy_out" / "GRAPH_TREE.html").exists()
