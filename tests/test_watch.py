...
def test_notify_only_creates_flag(tmp_path):
    ...
    flag = tmp_path / "graph_fy_out" / "needs_update"
    assert flag.exists()
    assert flag.read_text() == "1"

def test_notify_only_creates_flag_dir(tmp_path):
    # graph_fy_out dir does not exist yet
    assert not (tmp_path / "graph_fy_out").exists()
    _notify_only(tmp_path)
    assert (tmp_path / "graph_fy_out").is_dir()
...
def test_watched_extensions_includes_docs():
    ...
    assert ".pdf" not in _WATCHED_EXTENSIONS

def test_watched_extensions_excludes_images():
    assert ".png" not in _WATCHED_EXTENSIONS
    assert ".jpg" not in _WATCHED_EXTENSIONS
...
def test_batch_doc_only_deletion_triggers_rebuild(tmp_path):
    """#2580: deleting ONLY doc files while `graph_fy watch` runs must trigger
...
def test_doc_only_deletion_full_rebuild_evicts_md_nodes(tmp_path):
    ...
    graph_path = corpus / "graph_fy_out" / "graph.json"
...
def test_watch_raises_without_watchdog(tmp_path, monkeypatch):
    ...
    from graph_fy.watch import watch
...
def test_graph_fy_root_preserves_relative_when_invoked_with_relative_path(tmp_path, monkeypatch):
    """#777: ``.graph_fy_root`` stores the user-supplied path (``.``), not the
    resolved absolute, so a committed ``graph_fy_out/.graph_fy_root`` is
    ...
    saved = (corpus / "graph_fy_out" / ".graph_fy_root").read_text(encoding="utf-8")
    assert saved == ".", (
        f".graph_fy_root must preserve the user-supplied path; got {saved!r}"
    )


def test_rebuild_code_writes_community_name(tmp_path):
    """#1808: `graph_fy update` / _rebuild_code must forward community_labels to
    ...
    graph = json.loads((corpus / "graph_fy_out" / "graph.json").read_text(encoding="utf-8"))
...
def test_rebuild_code_keeps_a_visualization_when_over_the_viz_cap(tmp_path, monkeypatch):
    ...
    assert _rebuild_code(corpus, acquire_lock=False) is True
    html = corpus / "graph_fy_out" / "graph.html"
    ...
    monkeypatch.setenv("graph_fy_VIZ_NODE_LIMIT", str(cap))
    ...
    with monkeypatch.context() as failed_render:
        failed_render.setattr("graph_fy.paths.os.replace", fail_html_publish)
        assert _rebuild_code(corpus, acquire_lock=False) is True

    assert html.read_text(encoding="utf-8") == before, (
        "a failed aggregate publish must preserve the previous complete HTML"
    )
    assert (corpus / "graph_fy_out" / ".graph.html.stale").exists()
    ...
    assert not (corpus / "graph_fy_out" / ".graph.html.stale").exists()
    ...
    stable_paths = [
        corpus / "graph_fy_out" / "graph.json",
        corpus / "graph_fy_out" / "GRAPH_REPORT.md",
        corpus / "graph_fy_out" / ".graph_fy_labels.json",
        corpus / "graph_fy_out" / ".graph_fy_labels.json.sig",
    ]
    ...
    with monkeypatch.context() as recovery_patch:
        recovery_patch.setattr("graph_fy.cluster.cluster", fail_cluster)
        assert _rebuild_code(corpus, acquire_lock=False) is True
    ...
    monkeypatch.setenv("graph_fy_VIZ_NODE_LIMIT", "0")
    assert _rebuild_code(corpus, acquire_lock=False) is True
    assert not html.exists(), "graph_fy_VIZ_NODE_LIMIT=0 must disable the HTML viz outright"
...
def test_missing_html_recovery_preserves_multigraph_edge_counts(tmp_path, monkeypatch):
    ...
    from graph_fy.watch import _reconcile_graph_html
    ...
    monkeypatch.setenv("graph_fy_VIZ_NODE_LIMIT", "3")
    ...
    with monkeypatch.context() as failed_render:
        ...
        failed_render.setattr("graph_fy.paths.os.replace", fail_replace)
...
def test_update_rebuilds_with_nested_star_gitignore(tmp_path):
    """#1880: `graph_fy update` must not emit 0 nodes (and then refuse to
...
def test_update_discovers_newly_added_files_and_dirs(tmp_path):
    """#1837: after an initial build, a plain `graph_fy update` (full re-scan, no
    ...
    sources = {n.get("source_file", "") for n in
               json.loads((corpus / "graph_fy_out" / "graph.json").read_text())["nodes"]}
...
def test_rebuild_honors_persisted_excludes(tmp_path):
    ...
    import json
    from graph_fy.watch import _rebuild_code, _write_build_config
    ...
    _write_build_config(corpus / "graph_fy_out", excludes=["vendor"])
...
def test_rebuild_honors_persisted_no_gitignore(tmp_path):
    ...
    _write_build_config(
        corpus / "graph_fy_out", excludes=None, gitignore=False
    )

    assert _rebuild_code(corpus, no_cluster=True, acquire_lock=False) is True

    graph = json.loads((corpus / "graph_fy_out" / "graph.json").read_text())
...
def test_graph_fy_root_preserves_absolute_when_user_supplied(tmp_path):
    """When the caller supplies an absolute path, ``.graph_fy_root`` stores
...
def test_rebuild_code_deleted_cwd_without_repo_root_returns_false(tmp_path, monkeypatch, capsys):
    ...
    Without graph_fy_REPO_ROOT, the rebuild should fail cleanly before creating
    relative graph_fy_out queue/lock files.
    """
    from graph_fy.watch import _rebuild_code
    ...
    gone = tmp_path / "gone"
    gone.mkdir()
    monkeypatch.delenv("graph_fy_REPO_ROOT", raising=False)
...
def test_rebuild_code_deleted_cwd_uses_graph_fy_repo_root(tmp_path, monkeypatch):
    """graph_fy_REPO_ROOT lets detached hook rebuilds recover from a deleted CWD."""
    from graph_fy.watch import _rebuild_code
    ...
    gone = tmp_path / "gone"
    gone.mkdir()
    monkeypatch.setenv("graph_fy_REPO_ROOT", str(corpus))
    ...
    try:
        ...
        assert (corpus / "graph_fy_out" / "graph.json").exists()
    finally:
        os.chdir(old_cwd)


def test_rebuild_code_evicts_nodes_from_deleted_files(tmp_path):
    """#1007: graph_fy update (_rebuild_code with no changed_paths) must remove
...
def test_rebuild_code_evicts_removed_symbol_from_surviving_file(tmp_path):
    """#1116: graph_fy update (_rebuild_code with no changed_paths) must prune a
    ...
    # shrink, so the shrink-guard must let `graph_fy update` refresh the graph
...
def test_rebuild_code_preupgrade_marker_less_node_one_cycle_lag(tmp_path):
    ...
    markers. On the first `graph_fy update` after upgrading, a symbol removed
...
def test_rebuild_code_is_idempotent_when_cluster_ids_flap(tmp_path, monkeypatch):
    from graph_fy import cluster as cluster_mod
    from graph_fy.watch import _rebuild_code
    ...
    graph_path = tmp_path / "graph_fy_out" / "graph.json"
    report_path = tmp_path / "graph_fy_out" / "GRAPH_REPORT.md"
...
# --- .graph_fyignore honored in watch handler (gh-928) ---
...
def test_watch_handler_honors_graph_fyignore(tmp_path, monkeypatch):
    """gh-928: the watch Handler must short-circuit paths matching
    .graph_fyignore so busy volumes (node_modules churn, build artefacts,
    Time Machine writes, …) don't wake the rebuild pipeline.
    """
    import threading
    from graph_fy import watch as watch_mod

    watch_root = tmp_path / ".hidden-parent" / "corpus"
    watch_root.mkdir(parents=True)
    (watch_root / ".graph_fyignore").write_text("node_modules/\nbuild/\n", encoding="utf-8")
...
@pytest.mark.skipif(not _watchdog_available(), reason="watchdog not installed")
def test_watch_loads_graph_fyignore_once(tmp_path, monkeypatch):
    """gh-928: .graph_fyignore must be parsed exactly once at watch() startup,
    ...
    (tmp_path / ".graph_fyignore").write_text("ignored/\n", encoding="utf-8")
    (tmp_path / "ignored").mkdir()

    calls = {"n": 0}
    real_loader = detect_mod._load_graph_fyignore
    ...
    monkeypatch.setattr(watch_mod, "_load_graph_fyignore", counting_loader)
    ...
    time.sleep(0.7)
    assert calls["n"] == 1, f"_load_graph_fyignore called {calls['n']} times; expected 1"
...
def test_rebuild_code_accepts_repo_relative_changed_path_for_subdir_root(tmp_path):
    ...
    try:
        ...
        graph_path = src / "graph_fy_out" / "graph.json"
        before = json.loads(graph_path.read_text(encoding="utf-8"))
...
def test_rebuild_code_does_not_update_root_marker_when_write_is_refused(tmp_path, monkeypatch):
    ...
    try:
        ...
        marker = src / "graph_fy_out" / ".graph_fy_root"
...
def test_rebuild_code_incremental_rename_preserves_symlink_source_path(tmp_path):
    ...
    real = corpus / "real"
    real.mkdir()
    (corpus / ".graph_fyignore").write_text("real/\n", encoding="utf-8")
    old = real / "old.py"
...
def test_queue_and_drain_pending_round_trip(tmp_path):
    ...
    from graph_fy.watch import _queue_pending, _drain_pending, _PENDING_FILENAME
...
def test_drain_pending_dedupes_and_skips_blank_lines(tmp_path):
    ...
    from graph_fy.watch import _queue_pending, _drain_pending
...
def test_queue_pending_noop_on_empty_list(tmp_path):
    ...
    from graph_fy.watch import _queue_pending, _PENDING_FILENAME
...
def test_rebuild_code_queues_on_lock_contention(tmp_path, monkeypatch, capsys):
    ...
    from graph_fy.watch import _rebuild_code, _rebuild_lock, _PENDING_FILENAME
...
def test_rebuild_code_drains_late_arrivals(tmp_path, monkeypatch):
    ...
    from graph_fy import watch as watch_mod
    from graph_fy.watch import _rebuild_code as orig_rebuild
...
def test_merge_changed_paths_dedupes_in_order():
    ...
    from graph_fy.watch import _merge_changed_paths
...
def test_rebuild_code_preserves_nodes_from_excluded_but_alive_file(tmp_path, capsys):
    ...
    (#2495): only .graph_fyignore/--exclude matches evict on the hook path.
    """
    import json
    from graph_fy.watch import _rebuild_code
...
# ── #2495: `graph_fy update` purges newly-ignored files that still exist ───────


def test_update_prunes_newly_graph_fyignored_file(tmp_path, monkeypatch, capsys):
    """#2495: a file added to .graph_fyignore while still on disk must be purged
    from the graph by a full `graph_fy update` — nodes, edges, and hyperedges —
    instead of being preserved forever by the fail-closed keep. The legitimate
    shrink passes without --force/graph_fy_FORCE, unchanged in-corpus files stay
    byte-identical, and the prune is reported distinctly from the fail-closed
    keep line."""
    from graph_fy.watch import _rebuild_code

    monkeypatch.delenv("graph_fy_FORCE", raising=False)
    corpus = tmp_path / "corpus"
    corpus.mkdir()
    ...
    (corpus / ".graph_fyignore").write_text("b.py\n", encoding="utf-8")
    capsys.readouterr()
...
def test_update_still_prunes_deleted_file_with_ignore_rules_present(tmp_path):
    ...
    corpus = tmp_path / "corpus"
    corpus.mkdir()
    (corpus / ".graph_fyignore").write_text("unrelated/\n", encoding="utf-8")
    (corpus / "a.py").write_text("def alpha():\n    return 1\n", encoding="utf-8")
    (corpus / "b.py").write_text("def helper():\n    return 2\n", encoding="utf-8")
...
def test_update_evicts_semantic_nodes_from_newly_ignored_non_ast_source(tmp_path, capsys):
    ...
    (corpus / ".graph_fyignore").write_text("notes.txt\n", encoding="utf-8")
    capsys.readouterr()
...
def test_gitignore_eviction_gated_on_full_rebuild(tmp_path, capsys):
    ...
    `graph_fy update` honors it and purges the file."""
    from graph_fy.watch import _rebuild_code
...
def test_graph_fyignore_eviction_applies_to_incremental_rebuilds(tmp_path, capsys):
    """#2495 policy split, other half: a .graph_fyignore match is unambiguous
    ...
    (corpus / ".graph_fyignore").write_text("vendor.py\n", encoding="utf-8")
    capsys.readouterr()
    ...
    assert "vendor.py" not in sources, (
        ".graph_fyignore match must evict on an incremental rebuild too (#2495)"
    )
...
def _seed_semantic_doc_graph(corpus):
    ...
    Mimics a graph produced by the CLI ``graph_fy . --update`` path: code AST
...
def test_rebuild_code_evicts_semantic_nodes_from_deleted_non_ast_source(tmp_path):
    """#2051: a full `graph_fy update` must evict semantic nodes whose non-AST
...
def test_rebuild_code_preserves_remote_source_across_repeated_updates(tmp_path):
    ...
    (gdoc://, s3://, http://) must survive REPEATED `graph_fy update`s. Path
    ...
    collapse."""
    from graph_fy.watch import _rebuild_code, _is_remote_source
...
def test_rebuild_refuses_overwrite_when_existing_graph_over_size_cap(
    ...
    """#2251: an existing graph.json over graph_fy_MAX_GRAPH_BYTES could not be
    ...
    monkeypatch.setenv("graph_fy_MAX_GRAPH_BYTES", "100")
    assert _rebuild_code(
...
# --- #2342: `graph_fy update` rebuild must inherit the on-disk directed flag ---

def test_rebuild_code_inherits_directed_flag_clustered(tmp_path):
    """#2342: the clustered `graph_fy update` rebuild path built the graph via
    ...
    assert after.get("directed") is True, (
        "graph_fy update (clustered) must preserve an existing directed=True graph"
    )
    edge = next(
...
def test_rebuild_code_inherits_directed_flag_no_cluster(tmp_path):
    ...
    assert after.get("directed") is True, (
        "graph_fy update --no-cluster must preserve an existing directed=True graph"
    )
...
def _2406_graph(corpus):
    import json
    return json.loads(
        (corpus / "graph_fy_out" / "graph.json").read_text(encoding="utf-8")
    )
...
def test_incremental_rebuild_does_not_reparse_unchanged_targets(tmp_path, monkeypatch):
    ...
    import graph_fy.extract as extract_mod
    from graph_fy.watch import _rebuild_code
...
# --- #2603: absolute .graph_fy_root must not re-anchor cwd-relative sources ---
...
def test_subfolder_root_marker_preserves_unchanged_nodes(tmp_path, monkeypatch):
    ...
    while the skill writes an ABSOLUTE subfolder path into .graph_fy_root.
    ...
    (out / ".graph_fy_root").write_text(str(src.resolve()), encoding="utf-8")
...
def test_read_only_events_are_ignored():
    ...
    from graph_fy.watch import _is_read_only_event
...
def test_relativize_source_files_relativizes_definition_file(tmp_path):
    ...
    from graph_fy.watch import _relativize_source_files
...
def test_rebase_relative_source_files_rebases_definition_file(tmp_path):
    ...
    from graph_fy.watch import _rebase_relative_source_files
