...
from graphify.watch import patch_graph_in_memory, watch, watch_hot
...
def test_patch_graph_in_memory_add_function(tmp_path: Path):
    ...
    cur = conn.cursor()
    cur.execute("SELECT id FROM nodes_fts WHERE nodes_fts MATCH 'world'")
    rows = cur.fetchall()
    conn.close()
    assert len(rows) >= 1
...
def test_watch_hot_flag_dispatch(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
    ...
    monkeypatch.setattr("graphify.watch.watch_hot", mock_watch_hot)
