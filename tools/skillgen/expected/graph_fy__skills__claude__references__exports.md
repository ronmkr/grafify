# graph_fy reference: extra exports and benchmark
...
```bash
graph_fy export neo4j
```

**If `--neo4j-push <uri>`** - push directly to a running Neo4j instance. Ask the user for credentials if not provided:

```bash
graph_fy export neo4j --push bolt://localhost:7687 --user neo4j --password PASSWORD
```
...
```bash
graph_fy export falkordb --push falkordb://localhost:6379
```
...
```bash
$(cat graph_fy_out/.graphify_python) -m graph_fy.serve graph_fy_out/graph.json
```

This starts a stdio MCP server that exposes tools: `query_graph`, `get_node`, `get_neighbors`, `get_community`, `god_nodes`, `graph_stats`, `shortest_path`. Add to Claude Desktop or any MCP-compatible agent orchestrator so other agents can query the graph live.

To configure in Claude Desktop, add to `claude_desktop_config.json`. Claude Desktop can't run `$(...)`, and under `uv tool install` the system `python3` can't import graph_fy — so set `command` to the **absolute interpreter path** printed by `cat graph_fy_out/.graphify_python`:
```json
{
  "mcpServers": {
    "graph_fy": {
      "command": "<absolute path from: cat graph_fy_out/.graphify_python>",
      "args": ["-m", "graph_fy.serve", "/absolute/path/to/graph_fy_out/graph.json"]
    }
  }
}
```
...
If `total_words` from `graph_fy_out/.graphify_detect.json` is greater than 5,000, run:
