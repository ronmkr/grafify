...
cat > "${PROJECT_ROOT}/graph_fy_out/.graphify_chunk_0N.json" <<'CHUNK_JSON'
PASTED_JSON
CHUNK_JSON
```

Repeat for every chunk. Each chunk's JSON must land in its own `graph_fy_out/.graphify_chunk_NN.json` before Step B3 runs.
