# graph_fy reference: query, path, explain

Load this when the user asks a question against an existing graph, or runs `/graph_fy path` or `/graph_fy explain`. The core's query stub points here for the full traversal flow. These flows use the `graph_fy query` CLI when it is available and fall back to an inline NetworkX traversal otherwise.
...
| Skeleton | `--skeleton` | AST code skeleton elision — signatures, types, docstrings without bodies |
| RAG Prompt | `--rag-prompt` | Formatted prompt-ready context block with Personalized PageRank reranking |

First check the graph exists:
```bash
$(cat graph_fy_out/.graphify_python) -c "
from pathlib import Path
if not Path('graph_fy_out/graph.json').exists():
    print('ERROR: No graph found. Run /graph_fy <path> first to build the graph.')
    raise SystemExit(1)
"
```
If it fails, stop and tell the user to run `/graph_fy <path>` first.
...
import json, re
from pathlib import Path
data = json.loads(Path('graph_fy_out/graph.json').read_text(encoding='utf-8'))
vocab = set()
for n in data['nodes']:
    for c in re.findall(r'[^\W\d_]+', n.get('label','') or '', re.UNICODE):
        ...
        for p in parts:
            ...
            if 3 <= len(t) <= 30:
                vocab.add(t)
Path('graph_fy_out/.vocab.txt').write_text('\n'.join(sorted(vocab)), encoding='utf-8')
print(f'vocab: {len(vocab)} tokens')
"
```

2. Read `graph_fy_out/.vocab.txt`. Then for the user's question, select **up to 12 tokens from this exact list** that semantically match the query intent. Hard constraints:
...
# or: graph_fy query "QUESTION" --dfs --budget 3000
# or: graph_fy query "QUESTION" --skeleton           # return AST code skeletons (signatures only)
# or: graph_fy query "QUESTION" --rag-prompt --budget 2000 # emit prompt-ready context block
```

If the CLI is unavailable, load `graph_fy_out/graph.json` and run the traversal inline:
...
```bash
$(cat graph_fy_out/.graphify_python) -m graph_fy save-result --question "ORIGINAL_QUESTION" --answer "ANSWER" --type query --nodes NODE1 NODE2
```
...
```bash
$(cat graph_fy_out/.graphify_python) -m graph_fy save-result --question "Path from NODE_A to NODE_B" --answer "ANSWER" --type path_query --nodes NODE_A NODE_B
```

---

## For /graph_fy explain

Give a plain-language explanation of a single node - everything connected to it. Prefer the CLI when installed:

```bash
graph_fy explain "NODE_NAME"
```
...
```bash
$(cat graph_fy_out/.graphify_python) -m graph_fy save-result --question "Explain NODE_NAME" --answer "ANSWER" --type explain --nodes NODE_NAME
```
