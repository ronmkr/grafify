# graph_fy reference: incremental update and cluster-only
...
import sys, json
from graph_fy.detect import detect_incremental, save_manifest
from pathlib import Path

result = detect_incremental(Path('INPUT_PATH'))
new_total = result.get('new_total', 0)
print(json.dumps(result, indent=2, ensure_ascii=False))
Path('graph_fy_out/.graphify_incremental.json').write_text(json.dumps(result, ensure_ascii=False), encoding=\"utf-8\")
...
import json
from pathlib import Path
r = json.loads(Path('graph_fy_out/.graphify_incremental.json').read_text(encoding=\"utf-8\"))
Path('graph_fy_out/.graphify_detect.json').write_text(json.dumps({
...
result = json.loads(open('graph_fy_out/.graphify_incremental.json', encoding='utf-8').read()) if Path('graph_fy_out/.graphify_incremental.json').exists() else {}
...
If `code_only` is True: print `[graph_fy update] Code-only changes detected - skipping semantic extraction (no LLM needed)`, run only Step 3A (AST) on the changed files, skip Step 3B entirely (no subagents), then go straight to merge and Steps 4–8.

If `code_only` is False (any changed file is a doc/paper/image/video): **first, if any changed file is in `new_files['video']`, transcribe those files (Step 2.5), then rewrite `.graphify_detect.json` to move the resulting transcript paths into `files['document']` and drop `files['video']`** — otherwise raw `.mp4/.mp3` paths are fed to semantic subagents as unreadable media (#1392). Then run the full Steps 3A–3C pipeline as normal.


If no new files exist (only deletions), create an empty extraction so the merge step can prune:

```bash
if [ ! -f graph_fy_out/.graphify_extract.json ]; then
    echo '[graph_fy update] Only deletions -- creating empty extraction for merge.'
    $(cat graph_fy_out/.graphify_python) -c "
import json
from pathlib import Path
Path('graph_fy_out/.graphify_extract.json').write_text(json.dumps({'nodes':[],'edges':[],'hyperedges':[],'input_tokens':0,'output_tokens':0}), encoding='utf-8')
"
fi
```
...
import json
from pathlib import Path
from graph_fy.build import build_merge
from graph_fy.detect import save_manifest

# Load new extraction and incremental state
new_extraction = json.loads(Path('graph_fy_out/.graphify_extract.json').read_text(encoding=\"utf-8\"))
incremental = json.loads(Path('graph_fy_out/.graphify_incremental.json').read_text(encoding=\"utf-8\"))
...
print(f'[graph_fy update] Merged: {G.number_of_nodes()} nodes, {G.number_of_edges()} edges')
...
Path('graph_fy_out/.graphify_extract.json').write_text(json.dumps(merged_out, ensure_ascii=False), encoding=\"utf-8\")
print(f'[graph_fy update] Merged extraction written ({len(merged_out[\"nodes\"])} nodes, {len(merged_out[\"edges\"])} edges)')
...
print('[graph_fy update] Manifest saved.')
"
```
...
$(cat graph_fy_out/.graphify_python) -c "
import json
from graph_fy.analyze import graph_diff
...
old_data = json.loads(Path('graph_fy_out/.graphify_old.json').read_text(encoding=\"utf-8\")) if Path('graph_fy_out/.graphify_old.json').exists() else None
new_extract = json.loads(Path('graph_fy_out/.graphify_extract.json').read_text(encoding=\"utf-8\"))
...
Before the merge step, save the old graph: `cp graph_fy_out/graph.json graph_fy_out/.graphify_old.json`
Clean up after: `rm -f graph_fy_out/.graphify_old.json`
