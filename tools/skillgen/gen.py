"""skillgen: render graph_fy's committed skill artifacts from edited fragments.
...
files under ``graph_fy/skill*.md`` and ``graph_fy/skills/<platform>/references/``
...
except ModuleNotFoundError:  # Python 3.10 - graph_fy supports >=3.10
...
# straight from git instead of from disk. claude's v8 body is graph_fy/skill.md;
# every other split host has its own graph_fy/skill-<host>.md. Auditing each host
...
def _v8_baseline_ref(platform_key: str) -> str:
    ...
    if platform_key == "claude":
        return f"{_V8_BASELINE_SHA}:graph_fy/skill.md"
    if platform_key == "agents":
        ...
        return f"{_V8_BASELINE_SHA}:graph_fy/skill-amp.md"
    return f"{_V8_BASELINE_SHA}:graph_fy/skill-{platform_key}.md"

# Immutable baseline for --always-on-roundtrip. The six always-on instruction
# blocks used to be triple-quoted constants in graph_fy/__main__.py; they are now
# packaged graph_fy/always_on/*.md files the module reads at load. This ref points
...
ALWAYS_ON_BASELINE_REF = f"{_V8_BASELINE_SHA}:graph_fy/__main__.py"

# The always-on instruction blocks: rendered-file basename -> the __main__.py
# constant it must reproduce. Rendered to graph_fy/always_on/<basename>.md from
...
ALWAYS_ON_SANCTIONED_EDITS: dict[str, tuple[tuple[str, str], ...]] = {
    ...
    # invoke a literal `skill` tool with `skill: "graph_fy"`, which is
    # host-specific and not valid in every environment.
    "_AGENTS_MD_SECTION": (
        (
            "When the user types `/graph_fy`, invoke the `skill` tool with "
            '`skill: "graph_fy"` before doing anything else.',
            "When the user types `/graph_fy`, use the installed graph_fy skill or instructions "
...
# always-on via `graph_fy claude install` (the shared body). The agents-md hosts
# (trae, trae-cn, amp) read AGENTS.md and wire it via `graph_fy <host> install`.
...
_TRAE_PRETOOLUSE_NOTE = (
    ...
    "rebuild on tool use. Run `/graph_fy --update` manually after code changes if "
...
_AGENTS_MD_HOOKS: dict[str, dict[str, str]] = {
    "trae": {
        "heading_suffix": " (Trae)",
        "host_display": "Trae",
        "install_block": "graph_fy trae install       # or: graph_fy trae-cn install",
        "uninstall_block": "graph_fy trae uninstall     # or: graph_fy trae-cn uninstall   # remove the section",
        "pretooluse_note": _TRAE_PRETOOLUSE_NOTE,
    },
    "amp": {
        "heading_suffix": "",
        "host_display": "Amp",
        "install_block": "graph_fy amp install",
        "uninstall_block": "graph_fy amp uninstall  # remove the section",
        "pretooluse_note": "",
    },
    "agents": {
        ...
        # pointing at `graph_fy agents install` (which wires AGENTS.md, like amp).
        "heading_suffix": "",
        "host_display": "your agent",
        "install_block": "graph_fy agents install",
        "uninstall_block": "graph_fy agents uninstall  # remove the section",
        "pretooluse_note": "",
    },
}
...
# them. Two classes:
#
# 1. SHARED_INTRO_ALLOWLIST — the lean intro consolidation. "## What graph_fy is
...
SHARED_INTRO_ALLOWLIST: frozenset[str] = frozenset({
    "## What graph_fy is for",  # lean intro; v8 hosts had verbose intro prose, no heading.
})
...
#   * the inline ``$(cat graph_fy_out/.graph_fy_python) -c "..."`` python blocks
#     become single-quoted here-strings piped to the interpreter's stdin
#     (``@'...'@ | & (Get-Content graph_fy_out\.graph_fy_python) -``). A
...
_PY_INVOKE_POSIX = '$(cat graph_fy_out/.graph_fy_python) -c "'
_PY_INVOKE_PS_OPEN = "@'"
_PY_INVOKE_PS_CLOSE = "'@ | & (Get-Content graph_fy_out\\.graph_fy_python) -"
_MKDIR_POSIX = "mkdir -p graph_fy_out"
_MKDIR_PS = "New-Item -ItemType Directory -Force -Path graph_fy_out | Out-Null"
_FIND_CHUNKS_POSIX = "find graph_fy_out -maxdepth 1 -name '.graph_fy_chunk_*.json' -delete 2>/dev/null"
_FIND_CHUNKS_PS = (
    "Get-ChildItem graph_fy_out -Filter '.graph_fy_chunk_*.json' -File "
...
def _translate_bash_block(lines: list[str]) -> list[str]:
    ...
    for line in lines:
        ...
        elif not line.strip() or line.lstrip().startswith("#") or line.startswith("graph_fy "):
            out.append(line)  # blank lines, comments, and graph_fy CLI calls are shell-neutral
        else:
...
def render_always_on() -> list[RenderedArtifact]:
    """Render the six always-on instruction blocks to graph_fy/always_on/*.md.
    ...
    for basename in sorted(ALWAYS_ON_BLOCKS):
        ...
        out.append(RenderedArtifact(f"graph_fy/always_on/{basename}.md", body))
    return out
...
def audit_coverage(platform: Platform) -> list[str]:
    ...
    The audit reads the host's OWN v8 skill body (graph_fy/skill.md for claude,
    graph_fy/skill-<host>.md otherwise) and checks that every v8 heading lands in
    ...
    v8 headings are exempt and documented as deltas, not holes:
      - waves 2-3 consolidations (the lean "## What graph_fy is for" intro and the
...
def _is_frontmatter_description_line(line: str) -> bool:
    ...
    The unified description (graph_fy #1106) rewrites the frontmatter
...
def _is_chunk_cleanup_line(line: str) -> bool:
    ...
    The bare glob ``.graph_fy_chunk_*.json`` in the v8 cleanup line aborts the
    whole ``rm`` under fish/zsh when no chunk files exist (no-match is a hard
    error there, unlike bash). The fix (graph_fy #1172) drops the glob from the
    ...
    Both the v8 form (bare ``.graph_fy_chunk_*.json`` glob, removed) and the fixed
    ...
    if not s.startswith("rm -f"):
        return False
    return ".graph_fy_chunk_*.json" in line or ("find " in line and "-name '.graph_fy_chunk_" in line)
...
def _is_cache_unlink_fix_line(line: str) -> bool:
    ...
    ``.graph_fy_cached.json`` for Part C to merge. The miss branch now deletes it.
    """
    return (
        ".graph_fy_cached.json').unlink(missing_ok=True)" in line
        or line.strip() == "else:"
        or "Always (re)write the cache file" in line
        or "stale .graph_fy_cached.json" in line
    )
...
def _is_zero_node_guard_fix_line(line: str) -> bool:
    ...
    return (
        ...
        or s == "if not wrote:"
        or "refused to shrink graph_fy_out/graph.json" in line
...
def _is_manifest_root_fix_line(line: str) -> bool:
    ...
    relativized to the scan root, matching the native ``graph_fy update`` path.
    Both the old bare call (removed) and the new rooted call (added) match here;
    the ``import`` guard avoids matching the ``from graph_fy.detect import
    save_manifest`` line.
    """
...
def _is_manifest_stamp_fix_line(line: str) -> bool:
    ...
    ``graph_fy extract`` path. The rooted ``save_manifest`` call itself is
...
def _is_no_api_key_fix_line(line: str) -> bool:
    ...
    stating that graph_fy needs no API key, and (like the subagent-host skills)
    ...
    """
    return "graph_fy needs no API key" in line
...
def _is_shebang_allowlist_fix_line(line: str) -> bool:
    ...
    detection fell through to a bare ``python3`` that lacked graph_fy. ``@`` is now
...
def _is_obsidian_usage_comment_line(line: str) -> bool:
    """Whether a line is part of the ``/graph_fy`` usage-comment fix (#1681).

    The Usage block's bare ``/graph_fy`` comment said "full pipeline on current
...
def _is_uv_from_interpreter_fix_line(line: str) -> bool:
    ...
    Step 1's POSIX interpreter probe ran ``uv tool run graph_fyy python -c ...``,
    but ``graph_fyy`` exposes its executable as ``graph_fy``, so uv treated
    ``python`` as a missing ``graph_fyy`` command and the probe silently failed
    (the ``2>/dev/null`` swallowed uv's "use --from" hint), leaving PYTHON on a
    graph_fy-less system interpreter. The probe now runs
    ``uv tool run --from graph_fyy python -c ...``. Both the old (removed) and new
    (added) forms match here.
    """
    return "uv tool run" in line and "graph_fyy python" in line
...
def _is_semantic_cache_scope_fix_line(line: str) -> bool:
    ...
    return (
        ...
        and ".graph_fy_uncached.txt" in stripped
...
def _is_community_label_export_fix_line(line: str) -> bool:
    ...
    ``.graph_fy_labels.json`` sidecar carried the names). Step 5 now imports
    ...
    return (
        ...
        or stripped == "from graph_fy.export import to_json"
...
def always_on_roundtrip() -> list[str]:
    ...
    for basename, const_name in sorted(ALWAYS_ON_BLOCKS.items()):
        path = f"graph_fy/always_on/{basename}.md"
...
def _parse_args(argv: list[str]) -> argparse.Namespace:
    p = argparse.ArgumentParser(
        ...
        description="Render and guard graph_fy's committed skill artifacts.",
    )
